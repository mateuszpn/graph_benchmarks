...
std::vector<DeviceSelection>
MultiProcessHelper::getSubDevicesForExecution(DeviceSelection subDevices,
                                              size_t processesPerDevice,
                                              size_t actualSubDevicesCount) {
  // Split bitfield of merged subDevices to a vector of single subDevices
  std::vector<DeviceSelection> subDevicesForExecution =
      DeviceSelectionHelper::split(subDevices);

  // Validate if we're not using an illegal subDevice
  for (DeviceSelection subDevice : subDevicesForExecution) {
    if (DeviceSelectionHelper::getSubDeviceIndex(subDevice) >=
        actualSubDevicesCount) {
      return {};
    }
  }

  // Duplicate all subDevices respectively if we have more than one
  // processPerDevice
...
std::string MultiProcessHelper::createAffinityMask(size_t rootDeviceIndex,
  ...
  for (auto i = 0u; i < subDevicesSplit.size(); i++) {
    const auto subDeviceIndex =
        DeviceSelectionHelper::getSubDeviceIndex(subDevicesSplit[i]);
...
std::string MultiProcessHelper::createProcessName(
    const std::vector<DeviceSelection> &subDevicesForExecution,
    size_t processIndex) {
  std::ostringstream processName{};
  processName << "process" << processIndex << " ("
              << DeviceSelectionHelper::toString(
                     subDevicesForExecution[processIndex])
              << ")";
