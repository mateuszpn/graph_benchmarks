...
static TestResult run(const SetKernelArgSvmPointerArguments &arguments,
  ...
  // Create kernels
  const auto spirvModule = FileHelper::loadBinaryFile(
  ...
  for (auto i = 0u; i < arguments.allocationsCount; ++i) {
    ...
    ASSERT_ZE_RESULT_SUCCESS(zeKernelSetIndirectAccess(
        kernels[i], ZE_KERNEL_INDIRECT_ACCESS_FLAG_HOST |
  ...
  for (auto i = 0u; i < arguments.allocationsCount; ++i) {
    ASSERT_ZE_RESULT_SUCCESS(
        L0::UsmHelper::allocate(UsmMemoryPlacement::Shared, levelzero,
                                arguments.allocationSize, &allocations[i]));
  }

  // Warmup
  for (auto i = 0u; i < arguments.allocationsCount; ++i) {
    ASSERT_ZE_RESULT_SUCCESS(zeKernelSetArgumentValue(
        kernels[i], 0, sizeof(void *), &allocations[i]));
  }

  // Reallocate if argument is set
  if (arguments.reallocate) {
    for (auto i = 0u; i < arguments.allocationsCount; ++i) {
      ASSERT_ZE_RESULT_SUCCESS(L0::UsmHelper::deallocate(
          UsmMemoryPlacement::Shared, levelzero, allocations[i]));
      ASSERT_ZE_RESULT_SUCCESS(
          L0::UsmHelper::allocate(UsmMemoryPlacement::Shared, levelzero,
                                  arguments.allocationSize, &allocations[i]));
    }
  ...
  for (auto i = 0u; i < arguments.iterations; i++) {
    ...
    for (auto j = 0u; j < arguments.allocationsCount; ++j) {
      ASSERT_ZE_RESULT_SUCCESS(zeKernelSetArgumentValue(
          kernels[j], 0, sizeof(void *), &allocations[j]));
    }
    timer.measureEnd();
...
static RegisterTestCaseImplementation<SetKernelArgSvmPointer>
