...
void enqueueSvmCopy(cl_command_queue queue, void *src, void *dst, size_t size,
                    std::shared_mutex *barrier,
                    pfn_clEnqueueMemcpyINTEL clEnqueueMemcpyINTEL) {
  // warmup
  EXPECT_CL_SUCCESS(clEnqueueMemcpyINTEL(queue, CL_FALSE, dst, src, size, 0,
                                         nullptr, nullptr));
...
static TestResult run(const SvmCopyArguments &arguments,
  ...
  for (auto i = 0u; i < arguments.numberOfThreads; i++) {
    ...
    ASSERT_CL_SUCCESS(UsmHelperOcl::allocate(opencl, UsmMemoryPlacement::Host,
                                             bufferForCopySize, srcAlloc));
    srcAllocs.push_back(srcAlloc);

    UsmHelperOcl::Alloc dstAlloc{};
    ASSERT_CL_SUCCESS(UsmHelperOcl::allocate(opencl, UsmMemoryPlacement::Device,
                                             bufferForCopySize, dstAlloc));
  ...
  // Warmup
  {
    ...
    for (auto i = 0u; i < arguments.numberOfThreads; i++) {
      threads.push_back(std::unique_ptr<std::thread>(new std::thread(
          enqueueSvmCopy, commandQueues[i % commandQueues.size()],
          srcAllocs[i].ptr, dstAllocs[i].ptr, bufferForCopySize, &barrier,
  ...
  for (auto i = 0u; i < arguments.iterations; i++) {
    ...
    for (auto j = 0u; j < arguments.numberOfThreads; j++) {
      threads.push_back(std::unique_ptr<std::thread>(new std::thread(
          enqueueSvmCopy, commandQueues[j % commandQueues.size()],
          srcAllocs[j].ptr, dstAllocs[j].ptr, bufferForCopySize, &barrier,
          clEnqueueMemcpyINTEL)));
    }
    timer.measureStart();
    lock.unlock();
...
static RegisterTestCaseImplementation<SvmCopy> registerTestCase(run,
                                                                Api::OpenCL);
