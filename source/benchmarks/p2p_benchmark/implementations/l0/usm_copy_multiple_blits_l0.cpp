...
static TestResult run(const UsmP2PCopyMultipleBlitsArguments &arguments,
  ...
  LevelZero levelzero(QueueProperties::create(), ContextProperties::create());
  if (levelzero.commandQueue == nullptr) {
    return TestResult::DeviceNotCapable;
  }

  if (static_cast<size_t>(arguments.srcDeviceId) >=
          levelzero.rootDevices.size() ||
      static_cast<size_t>(arguments.dstDeviceId) >=
          levelzero.rootDevices.size()) {
  ...
  ASSERT_ZE_RESULT_SUCCESS(
      zeDeviceGetProperties(srcDevice, &srcDeviceProperties));
  ze_device_properties_t dstDeviceProperties = {};
  ASSERT_ZE_RESULT_SUCCESS(
      zeDeviceGetProperties(dstDevice, &dstDeviceProperties));

  ze_bool_t hasAccess = false;
  ASSERT_ZE_RESULT_SUCCESS(
      zeDeviceCanAccessPeer(srcDevice, dstDevice, &hasAccess));
  if (hasAccess == false) {
  ...
  ASSERT_ZE_RESULT_SUCCESS(zeDeviceGetCommandQueueGroupProperties(
      srcDevice, &numQueueGroups, nullptr));
  if (numQueueGroups == 0) {
    return TestResult::DeviceNotCapable;
  }
  ...
  std::vector<ze_command_queue_group_properties_t> queueProperties(
      numQueueGroups);
  ASSERT_ZE_RESULT_SUCCESS(zeDeviceGetCommandQueueGroupProperties(
      srcDevice, &numQueueGroups, queueProperties.data()));
  ...
  for (size_t blitterIndex : arguments.blitters.getEnabledBits()) {
    ...
    if (isMainCopyEngine) {
      ...
      cmdQueueDesc.index = static_cast<uint32_t>(
          blitterIndex - queueProperties[mainCopyOrdinal].numQueues);
      blitSizeAssigner.addLinkCopyEngine();
    }

    ze_command_queue_handle_t queue;
    ASSERT_ZE_RESULT_SUCCESS(zeCommandQueueCreate(levelzero.context, srcDevice,
                                                  &cmdQueueDesc, &queue));

    cmdListDesc.commandQueueGroupOrdinal = cmdQueueDesc.ordinal;
    ze_command_list_handle_t list;
    ASSERT_ZE_RESULT_SUCCESS(
        zeCommandListCreate(levelzero.context, srcDevice, &cmdListDesc, &list));
    ...
    queues.push_back(
        PerQueueData{queue, list, queueName, isMainCopyEngine, event});
  }

  // Create buffers
  void *srcBuffer{}, *dstBuffer{};
  ASSERT_ZE_RESULT_SUCCESS(
      UsmHelper::allocate(UsmRuntimeMemoryPlacement::Device, levelzero,
                          srcDevice, arguments.size, &srcBuffer));
  ASSERT_ZE_RESULT_SUCCESS(
      UsmHelper::allocate(UsmRuntimeMemoryPlacement::Device, levelzero,
                          dstDevice, arguments.size, &dstBuffer));
  ...
  for (PerQueueData &queue : queues) {
    ASSERT_ZE_RESULT_SUCCESS(
        zeCommandListAppendMemoryCopy(queue.list, queue.copyDst, queue.copySrc,
                                      queue.copySize, queue.event, 0, nullptr));
  ...
  for (PerQueueData &queue : queues) {
    ASSERT_ZE_RESULT_SUCCESS(zeCommandQueueExecuteCommandLists(
        queue.queue, 1, &queue.list, nullptr));
    ASSERT_ZE_RESULT_SUCCESS(zeCommandQueueSynchronize(
        queue.queue, std::numeric_limits<uint64_t>::max()));
  }
  ...
  for (auto i = 0u; i < arguments.iterations; i++) {
    ...
    for (PerQueueData &queue : queues) {
      ...
      ASSERT_ZE_RESULT_SUCCESS(
          zeEventQueryKernelTimestamp(queue.event, &timestampResult));
      ...
      statistics.pushValue(commandTime, queue.copySize, typeSelector.getUnit(),
                           typeSelector.getType(), queue.name);
    }

    // Report total results
    statistics.pushValue(endGpuTime - startGpuTime, arguments.size,
...
static RegisterTestCaseImplementation<UsmCopyMultipleBlits>
