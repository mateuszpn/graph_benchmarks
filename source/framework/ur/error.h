...
#define ASSERT_UR_RESULT_SUCCESS(retVal)                                       \
  {                                                                            \
    ...
    if (tempVarForDefine != UR_RESULT_SUCCESS) {                               \
      ...
      ss << tempVarForDefine;                                                  \
      NON_FATAL_ERROR("ASSERT_UR_RESULT_SUCCESS", #retVal,                     \
...
#define EXPECT_UR_RESULT_SUCCESS(retVal)                                       \
  {                                                                            \
    ...
    if (tempVarForDefine != UR_RESULT_SUCCESS) {                               \
      std::ostringstream ss;                                                   \
      ss << tempVarForDefine;                                                  \
      NON_FATAL_ERROR("EXPECT_UR_RESULT_SUCCESS", #retVal,                     \
                      std::to_string(tempVarForDefine).c_str(),                \
                      ss.str().c_str());                                       \
    }                                                                          \
  }
...
#define UR_RESULT_SUCCESS_OR_ERROR(retVal)                                     \
  {                                                                            \
    const auto tempVarForDefine = (retVal);                                    \
    if (tempVarForDefine != UR_RESULT_SUCCESS) {                               \
      const auto _message = std::string("Fatal UR error occurred, retVal=") +  \
