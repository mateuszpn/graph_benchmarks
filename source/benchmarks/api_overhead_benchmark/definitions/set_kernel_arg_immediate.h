...
struct KernelSetArgumentValueImmediateArguments : TestCaseArgumentContainer {
  ...
  KernelSetArgumentValueImmediateArguments()
      : argumentSize(*this, "argSize", "Kernel argument size in bytes"),
        differentValues(*this, "differentValues",
                        "Use different values for arguments each iteration") {}
};

struct KernelSetArgumentValueImmediate
    : TestCase<KernelSetArgumentValueImmediateArguments> {
  ...
  std::string getHelp() const override {
    return "measures time spent in zeKernelSetArgumentValue for immediate "
           "arguments on CPU.";
  }
};
