...
static TestResult run(const SubmitKernelArguments &arguments,
  ...
  // Setup
  auto queueProperties = QueueProperties::create()
                             .setProfiling(arguments.useProfiling)
                             .setOoq(!arguments.inOrderQueue);
  ...
  cl_program program = clCreateProgramWithIL(opencl.context, spirvModule.data(),
                                             spirvModule.size(), &retVal);
  ...
  for (auto iteration = 0u; iteration < arguments.numKernels; iteration++) {
    // Note: this test calls clSetKernelArg each time to be closer to the SYCL
    // behavior!
    ASSERT_CL_SUCCESS(
    ...
    ASSERT_CL_SUCCESS(clEnqueueNDRangeKernel(opencl.commandQueue, kernel, 1,
                                             ...
                                             eventPtr));
    // Note: this test calls clReleaseEvent immediately after enqueuing the
    // kernel to be closer to the SYCL behavior!
