  ...
  ImmediateCmdListCopyWorkloadArguments()
      : ordinal(*this, "ordinal", "ordinal of engine group to be used"),
        engineIndex(*this, "engineIndex",
                    "physical engine index inside the engine group"),
        copySize(*this, "copySize",
                 "size in bytes to be used for copy operation") {}
};

struct ImmediateCmdListCopyWorkload
    : Workload<ImmediateCmdListCopyWorkloadArguments> {};

TestResult run(const ImmediateCmdListCopyWorkloadArguments &arguments,
  ...
  ZE_RESULT_SUCCESS_OR_RETURN_ERROR(zeDeviceGetCommandQueueGroupProperties(
  ...
  if (arguments.engineIndex >=
      queueGroupProperties[arguments.ordinal].numQueues) {
    return TestResult::InvalidArgs;
  }
  ...
  ZE_RESULT_SUCCESS_OR_RETURN_ERROR(zeEventPoolCreate(
  ...
  ZE_RESULT_SUCCESS_OR_RETURN_ERROR(zeCommandListCreateImmediate(
  ...
  ZE_RESULT_SUCCESS_OR_RETURN_ERROR(UsmHelper::allocate(
      UsmMemoryPlacement::Host, levelzero, arguments.copySize, &hostSrcMemory));
  ZE_RESULT_SUCCESS_OR_RETURN_ERROR(
      UsmHelper::allocate(UsmMemoryPlacement::Device, levelzero,
                          arguments.copySize, &deviceDstMemory));

  // Warmup
  for (auto i = 0u; i < 5; i++) {
    ZE_RESULT_SUCCESS_OR_RETURN_ERROR(
        zeCommandListAppendMemoryCopy(cmdList, deviceDstMemory, hostSrcMemory,
                                      arguments.copySize, event, 0, nullptr));
