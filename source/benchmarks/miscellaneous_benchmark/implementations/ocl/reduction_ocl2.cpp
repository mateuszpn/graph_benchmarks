...
TestResult run(const ReductionArguments2 &arguments, Statistics &statistics) {
  ...
  ASSERT_CL_SUCCESS(clEnqueueReadBuffer(opencl.commandQueue, buffer, true,
                                        lastIndexOffset, 4u, &actualSum, 0u,
                                        nullptr, nullptr));
  ASSERT_CL_SUCCESS(clFinish(opencl.commandQueue));
  ...
  for (auto i = 0u; i < arguments.iterations; i++) {
    ...
    if (i + 1 < arguments.iterations) {
      statistics.pushValue(std::chrono::nanoseconds{timeNs},
                           typeSelector.getUnit(), MeasurementType::Gpu,
                           "time");
      statistics.pushValue(std::chrono::nanoseconds{timeNs}, sizeInBytes,
                           MeasurementUnit::GigabytesPerSecond,
                           MeasurementType::Gpu, "bw");
...
static RegisterTestCaseImplementation<Reduction2> registerTestCase(run,
                                                                   Api::OpenCL);
