...
static TestResult run(const SeparateAtomicsExplicitArguments &arguments,
  ...
  const size_t atomicBufferSize =
      cachelinesCount * MemoryConstants::cachelineSize;
  const size_t otherArgumentsBufferEntryCount =
      4u; // we only need 1 value, but storing in multiple can prevent some
          // compiler opts
  const size_t otherArgumentsBufferSize =
      otherArgumentsBufferEntryCount * data.sizeOfDataType;

  // Create kernel
  cl_program program = nullptr;
  const char *programName = "atomic_benchmark_kernel.cl";
  const std::string compilerOptions = KernelHelper::getCompilerOptionsExplicit(
      arguments.dataType, arguments.atomicOperation, arguments.memoryOrder,
      arguments.scope, otherArgumentsBufferEntryCount);
  ...
  // Create and initialize the atomic buffer with kernel (we need to use
  // atomic_init for explicit atomics)
  const size_t gwsForInitialize =
      cachelinesCount * (MemoryConstants::cachelineSize / data.sizeOfDataType);
  cl_mem atomicBuffer = clCreateBuffer(opencl.context, CL_MEM_READ_WRITE,
                                       atomicBufferSize, nullptr, &retVal);
  ASSERT_CL_SUCCESS(retVal);
  ASSERT_CL_SUCCESS(
      clSetKernelArg(initializeKernel, 0, sizeof(atomicBuffer), &atomicBuffer));
  ASSERT_CL_SUCCESS(clSetKernelArg(initializeKernel, 1, data.sizeOfDataType,
                                   data.initialValue));
  ASSERT_CL_SUCCESS(
      clEnqueueNDRangeKernel(opencl.commandQueue, initializeKernel, 1, nullptr,
                             &gwsForInitialize, nullptr, 0, nullptr, nullptr));
  ASSERT_CL_SUCCESS(clFinish(opencl.commandQueue));

  // Create and initialize the buffer with value for the other argument of
  // atomic operation
  cl_mem otherArgumentsBuffer =
      clCreateBuffer(opencl.context, CL_MEM_READ_WRITE,
                     otherArgumentsBufferSize, nullptr, &retVal);
  ASSERT_CL_SUCCESS(retVal);
  ASSERT_CL_SUCCESS(clEnqueueFillBuffer(
      opencl.commandQueue, otherArgumentsBuffer, data.otherArgument,
      data.sizeOfDataType, 0, otherArgumentsBufferSize, 0, nullptr, nullptr));
  ASSERT_CL_SUCCESS(clFinish(opencl.commandQueue));
  ...
  // Warmup
  ASSERT_CL_SUCCESS(
      clSetKernelArg(kernel, 0, sizeof(atomicBuffer), &atomicBuffer));
  ...
  ASSERT_CL_SUCCESS(clSetKernelArg(
      kernel, 3, sizeof(arguments.atomicsPerCacheline.getSizeOf()),
      arguments.atomicsPerCacheline.getAddressOf()));
  ...
  ASSERT_CL_SUCCESS(clEnqueueReadBuffer(opencl.commandQueue, atomicBuffer,
                                        CL_BLOCKING, 0, atomicBufferSize,
...
static RegisterTestCaseImplementation<SeparateAtomicsExplicit>
