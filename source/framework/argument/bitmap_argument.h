...
struct BitmaskArgument : Argument {
  ...
  operator Bitset() const { return value; }
  ...
  bool validate() const override { return this->valid; }

protected:
  std::string toStringValue() const override { return value.to_string(); }
  ...
  void parseImpl(const std::string &valueToParse) override {
    ...
    if (std::any_of(valueToParse.begin(), valueToParse.end(),
                    isInvalidCharacter)) {
      return;
    }
