...
Opencl::Opencl(const QueueProperties &queueProperties,
  ...
  if (platformIndex == -1) {
    for (uint32_t localPlatformIndex = 0u; localPlatformIndex < numPlatforms;
         localPlatformIndex++) {
      if (clGetDeviceIDs(platforms[localPlatformIndex], CL_DEVICE_TYPE_GPU, 0,
                         nullptr, &numDevices) == CL_SUCCESS) {
  ...
  if (platformIndex >= numPlatforms) {
    FATAL_ERROR("Invalid OCL platform index. platformIndex=", platformIndex,
  ...
  if (deviceIndex >= numDevices) {
    FATAL_ERROR("Invalid OCL device index. deviceIndex=", deviceIndex,
                " numDevices=", numDevices);
  }
  auto devices = std::make_unique<cl_device_id[]>(numDevices);
  EXPECT_CL_SUCCESS(clGetDeviceIDs(platform, CL_DEVICE_TYPE_GPU, numDevices,
                                   devices.get(), nullptr));
...
cl_command_queue Opencl::createQueue(QueueProperties queueProperties) {
  ...
  const cl_device_id deviceForQueue =
  ...
  // Create queue
  if (queueProperties.fillQueueProperties(deviceForQueue, properties,
                                          maxPropertiesCount)) {
    queue = clCreateCommandQueueWithProperties(this->context, deviceForQueue,
                                               properties, &retVal);
  }
...
cl_context Opencl::createContext(const ContextProperties &contextProperties) {
  ...
  std::vector<cl_device_id> devicesForContext =
      getDevices(contextProperties.deviceSelection, false);
  if (devicesForContext.size() == 0) {
    FATAL_ERROR_IF(contextProperties.requireCreationSuccess,
                   "Failed getting devices for context");
    return nullptr;
  }

  cl_int retVal{};
  cl_context createdContext =
      clCreateContext(nullptr, static_cast<cl_uint>(devicesForContext.size()),
                      devicesForContext.data(), nullptr, nullptr, &retVal);
...
cl_device_id Opencl::getDevice(DeviceSelection deviceSelection) {
  FATAL_ERROR_IF(DeviceSelectionHelper::hasHost(deviceSelection),
                 "Cannot get cl_device_id for host");
...
std::vector<cl_device_id> Opencl::getDevices(DeviceSelection deviceSelection,
                                             bool requireSuccess) {
  FATAL_ERROR_IF(DeviceSelectionHelper::hasHost(deviceSelection),
                 "Cannot get cl_device_id for host");
  ...
  if ((deviceSelection & DeviceSelection::Root) == DeviceSelection::Root) {
    FATAL_ERROR_IF(this->rootDevice == nullptr,
                   "Root device has not been created yet");
  ...
  for (DeviceSelection subDevice : DeviceSelectionHelper::subDevices) {
    if (DeviceSelectionHelper::hasDevice(deviceSelection, subDevice)) {
      const auto subDeviceIndex =
          DeviceSelectionHelper::getSubDeviceIndex(subDevice);
      FATAL_ERROR_IF(subDeviceIndex >= subDevices.size() && requireSuccess,
                     "Invalid subDevice selected")
  ...
  const auto expectedCount =
      DeviceSelectionHelper::getDevicesCount(deviceSelection);
  const auto actualCount = result.size();
  if (expectedCount != actualCount) {
    FATAL_ERROR_IF(expectedCount != actualCount && requireSuccess,
                   "Invalid number of devices accumulated");
    return {};
  }
...
bool Opencl::createSubDevices(bool requireSuccess) {
  ...
  EXPECT_CL_SUCCESS(clGetDeviceInfo(this->rootDevice,
                                    CL_DEVICE_PARTITION_AFFINITY_DOMAIN,
                                    sizeof(domain), &domain, NULL));
  if ((domain & CL_DEVICE_AFFINITY_DOMAIN_NEXT_PARTITIONABLE) == 0) {
    FATAL_ERROR_IF(requireSuccess,
                   "SubDevice was selected, but device is not partitionable");
    return false;
  }
  if ((domain & CL_DEVICE_AFFINITY_DOMAIN_NUMA) == 0) {
    FATAL_ERROR_IF(requireSuccess, "SubDevice was selected, but device is not "
  ...
  EXPECT_CL_SUCCESS(clCreateSubDevices(this->rootDevice, properties, 0, nullptr,
                                       &numSubDevices));
  this->subDevices.resize(numSubDevices);
  EXPECT_CL_SUCCESS(clCreateSubDevices(this->rootDevice, properties,
                                       numSubDevices, this->subDevices.data(),
                                       nullptr));
  return true;
}
