...
static TestResult run(const EnqueueBarrierWithEmptyWaitlistArguments &arguments,
  ...
  QueueProperties queueProperties =
      QueueProperties::create().setOoq(arguments.outOfOrderQueue);
  ...
  if (auto result = ProgramHelperOcl::buildProgramFromSourceFile(
          opencl.context, opencl.device, programName, nullptr, program);
  ...
  ASSERT_CL_SUCCESS(
      clSetKernelArg(kernel, 0, sizeof(size_t), &operationsCount));
  ...
  // Warmup
  {
    ...
    for (auto j = 0u; j < enqueueCount - 1; ++j) {
      ASSERT_CL_SUCCESS(clEnqueueNDRangeKernel(opencl.commandQueue, kernel, 1,
                                               ...
                                               nullptr));
      ASSERT_CL_SUCCESS(clEnqueueBarrierWithWaitList(opencl.commandQueue, 0,
                                                     nullptr, nullptr));
    }
    ASSERT_CL_SUCCESS(clEnqueueNDRangeKernel(opencl.commandQueue, kernel, 1,
                                             nullptr, &gws, &lws, 0, nullptr,
                                             nullptr));
    ASSERT_CL_SUCCESS(
        clEnqueueBarrierWithWaitList(opencl.commandQueue, 0, nullptr, &event));
...
static RegisterTestCaseImplementation<EnqueueBarrierWithEmptyWaitlist>
