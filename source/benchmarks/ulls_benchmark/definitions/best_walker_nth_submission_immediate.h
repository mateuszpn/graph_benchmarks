...
struct BestWalkerNthSubmissionImmediate
    : TestCase<BestWalkerNthSubmissionImmediateArguments> {
  ...
  std::string getHelp() const override {
    return "enqueues n kernels, which updates system memory location and then "
           "busy-loops on CPU "
           "until the update of nth kernel becomes visible. Kernel is enqueued "
           "using low-latency immediate "
