...
static TestResult run(const LifecycleCommandListArguments &arguments,
  ...
  ze_command_list_desc_t commandListDesc = {ZE_STRUCTURE_TYPE_COMMAND_LIST_DESC,
                                            nullptr,
                                            levelzero.commandQueueDesc.ordinal};
  ...
  for (auto i = 0u; i < arguments.cmdListCount; i++) {
    ...
    ASSERT_ZE_RESULT_SUCCESS(
        zeCommandQueueSynchronize(levelzero.commandQueue, UINT64_MAX));
...
static RegisterTestCaseImplementation<LifecycleCommandList>
