...
static TestResult run(const StreamMemoryArguments &arguments,
  ...
  if (arguments.partialMultiplier > 1u) {
    if (arguments.type == StreamMemoryType::Scale ||
        arguments.type == StreamMemoryType::Triad) {
      return TestResult::NoImplementation;
    }
    if (arguments.type == StreamMemoryType::Write &&
        arguments.contents != BufferContents::Zeros) {
      return TestResult::NoImplementation;
    }
  }
  ...
  case StreamMemoryType::Read:
    ...
    buffers[buffersCount++] = clCreateBuffer(opencl.context, bufferFlags,
                                             bufferSize, nullptr, &retVal);
    bufferSizes[buffersCount] = 16u;
    buffers[buffersCount++] =
        clCreateBuffer(opencl.context, bufferFlags, 16u, nullptr, &retVal);
    break;
  ...
  for (auto i = 0u; i < buffersCount; i++) {
    ASSERT_CL_SUCCESS(BufferContentsHelperOcl::fillBuffer(
        opencl.commandQueue, buffers[i], bufferSizes[i], arguments.contents));
  ...
  if ((arguments.type == StreamMemoryType::Write &&
       arguments.contents != BufferContents::Random) ||
      arguments.type == StreamMemoryType::Read) {
    ASSERT_CL_SUCCESS(
        clSetKernelArg(kernel,
                       static_cast<cl_uint>(setScalarArgument ? buffersCount + 1
                                                              : buffersCount),
                       4u, &multiplier));
  }
...
static RegisterTestCaseImplementation<StreamMemory>
