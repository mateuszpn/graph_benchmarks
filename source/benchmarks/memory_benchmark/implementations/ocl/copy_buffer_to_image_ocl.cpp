...
static TestResult run(const CopyBufferToImageArguments &arguments,
  ...
  if (!QueueFamiliesHelper::validateCapability(
          opencl.commandQueue, CL_QUEUE_CAPABILITY_TRANSFER_IMAGE_INTEL)) {
    return TestResult::DeviceNotCapable;
  }
  if (!ImageHelperOcl::validateImageDimensions(opencl.device,
                                               arguments.region)) {
    return TestResult::DeviceNotCapable;
  }
  ...
  const cl_image_format imageFormat =
      ImageHelperOcl::getImageFormat(channelOrder, channelFormat);
  cl_image_desc imageDescription = {};
  imageDescription.image_type =
      ImageHelperOcl::getOclImageTypeFromDimensions(arguments.region);
  ...
  auto size = imageDescription.image_width * imageDescription.image_height *
              imageDescription.image_depth;
  cl_mem srcBuffer =
      clCreateBuffer(opencl.context, CL_MEM_READ_WRITE, size, nullptr, &retVal);
  ASSERT_CL_SUCCESS(retVal);
  cl_mem dstImage = clCreateImage(opencl.context, 0, &imageFormat,
                                  &imageDescription, nullptr, &retVal);
  ASSERT_CL_SUCCESS(retVal);
  const auto imageSizeInBytes = ImageHelperOcl::getImageSizeInBytes(
      channelOrder, channelFormat, arguments.region);

  // Warmup
  const size_t origin[] = {0, 0, 0};
  const size_t region[] = {arguments.region[0], arguments.region[1],
                           arguments.region[2]};

  ASSERT_CL_SUCCESS(clEnqueueCopyBufferToImage(opencl.commandQueue, srcBuffer,
                                               dstImage, 0, origin, region, 0,
  ...
  ASSERT_CL_SUCCESS(clReleaseMemObject(srcBuffer));
  ASSERT_CL_SUCCESS(clReleaseMemObject(dstImage));
  return TestResult::Success;
}

static RegisterTestCaseImplementation<CopyBufferToImage>
