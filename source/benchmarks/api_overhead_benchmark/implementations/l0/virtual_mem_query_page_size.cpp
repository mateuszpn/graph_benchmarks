...
static TestResult run(const VirtualMemQueryPageSizeArguments &arguments,
  ...
  ASSERT_ZE_RESULT_SUCCESS(zeVirtualMemQueryPageSize(
      levelzero.context, levelzero.device, (MemoryConstants::gigaByte * 1) - 1u,
      &pageSize));
  ...
  for (auto i = 0u; i < arguments.iterations; i++) {
    ...
    for (auto j = 0u; j < implicitIterationCount; j++) {
      ASSERT_ZE_RESULT_SUCCESS(zeVirtualMemQueryPageSize(
          levelzero.context, levelzero.device,
          (MemoryConstants::gigaByte * 1) - 1u, &pageSize));
    }
    timer.measureEnd();
    statistics.pushValue(timer.get() / implicitIterationCount,
...
static RegisterTestCaseImplementation<VirtualMemQueryPageSize>
