...
static void printDeviceInfo() {
  ...
  CL_SUCCESS_OR_ERROR(clGetPlatformInfo(opencl.platform, CL_PLATFORM_NAME,
                                        sizeof(bufferString), bufferString,
                                        nullptr),
                      "clGetPlatformInfo failed");
  std::cout << "OpenCL Platform: " << bufferString << std::endl;

  CL_SUCCESS_OR_ERROR(clGetDeviceInfo(opencl.device, CL_DEVICE_NAME,
                                      sizeof(bufferString), bufferString,
                                      nullptr),
                      "clGetDeviceInfo failed");
  std::cout << "\tDevice: " << bufferString << std::endl;

  CL_SUCCESS_OR_ERROR(clGetDeviceInfo(opencl.device, CL_DRIVER_VERSION,
                                      sizeof(bufferString), bufferString,
                                      nullptr),
                      "clGetDeviceInfo failed");
  std::cout << "\t\tdriverVersion: " << bufferString << std::endl;

  CL_SUCCESS_OR_ERROR(clGetDeviceInfo(opencl.device,
                                      CL_DEVICE_MAX_COMPUTE_UNITS,
                                      sizeof(bufferUint), &bufferUint, nullptr),
                      "clGetDeviceInfo failed");
  ...
  if (intelProduct != IntelProduct::Unknown) {
    std::cout << "\t\tintelProduct:  " << std::to_string(intelProduct)
              << " (intelGen: " << std::to_string(intelGen) << ")\n";
  }
...
static void printAvailableDevices() {
  ...
  for (cl_uint platformIndex = 0; platformIndex < numPlatforms;
    ...
    EXPECT_CL_SUCCESS(clGetPlatformInfo(platform, CL_PLATFORM_NAME,
                                        sizeof(bufferString), bufferString,
                                        nullptr));
    ...
    if (retVal != CL_SUCCESS) {
      std::cout << "(no GPU devices found, clGetDeviceIDs returned "
                << oclErrorToString(retVal) << ")\n";
      continue;
    } else {
    ...
    for (cl_uint deviceIndex = 0; deviceIndex < numDevices; deviceIndex++) {
      ...
      EXPECT_CL_SUCCESS(clGetDeviceInfo(
          device, CL_DEVICE_NAME, sizeof(bufferString), bufferString, nullptr));
      ...
      std::cout << ", select this device with --oclPlatformIndex="
                << platformIndex << " --oclDeviceIndex=" << deviceIndex;
      std::cout << std::endl;
    }
  }
