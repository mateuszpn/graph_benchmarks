...
struct MultipleImmediateCmdListsWithDependenciesArguments
...
struct MultipleImmediateCmdListsWithDependencies
    : TestCase<MultipleImmediateCmdListsWithDependenciesArguments> {
  ...
  std::string getHelp() const override {
    return "Creates N immediate command lists. Submits kernels in order to "
           "each of those"
           "Each kernel has a dependency on previous one"
           "Submissions are small to allows concurrent execution"
           "Meassures time from scheduling start, till all command lists are "
           "completed";
  }
};
