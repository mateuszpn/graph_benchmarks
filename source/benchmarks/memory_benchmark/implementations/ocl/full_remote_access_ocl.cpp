...
static TestResult run(const FullRemoteAccessMemoryArguments &arguments,
  ...
  case StreamMemoryType::Read:
    kernelName =
        blockAccess ? "full_remote_block_read" : "full_remote_scatter_read";
  ...
  case StreamMemoryType::Write:
    kernelName =
        blockAccess ? "full_remote_block_write" : "full_remote_scatter_write";
    buffers[buffersCount++] = clCreateBuffer(opencl.context, CL_MEM_READ_WRITE,
                                             bufferSize, nullptr, &retVal);
    break;
  ...
  const size_t localWorkSize =
      workItems > maxWorkgroupSize ? maxWorkgroupSize : workItems;
...
static RegisterTestCaseImplementation<FullRemoteAccessMemory>
