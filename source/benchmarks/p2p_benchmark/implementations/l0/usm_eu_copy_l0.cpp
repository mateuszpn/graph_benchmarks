...
static TestResult run(const UsmP2PCopyArguments &arguments,
  ...
  ASSERT_ZE_RESULT_SUCCESS(
      UsmHelper::allocate(UsmRuntimeMemoryPlacement::Device, levelzero,
                          srcDevice, arguments.size, &source));
  ASSERT_ZE_RESULT_SUCCESS(
      UsmHelper::allocate(UsmRuntimeMemoryPlacement::Device, levelzero,
                          dstDevice, arguments.size, &destination));
  ...
  if (arguments.useEvents) {
    ...
    ASSERT_ZE_RESULT_SUCCESS(zeEventPoolCreate(
        levelzero.context, &eventPoolDesc, 1, &srcDevice, &eventPool));
  ...
  ASSERT_ZE_RESULT_SUCCESS(zeCommandListCreate(levelzero.context, srcDevice,
                                               &cmdListDesc, &cmdList));
