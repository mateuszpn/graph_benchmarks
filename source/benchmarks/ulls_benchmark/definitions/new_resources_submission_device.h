...
struct NewResourcesSubmissionDevice
    : TestCase<NewResourcesSubmissionDeviceArguments> {
  ...
  std::string getHelp() const override {
    return "enqueues kernel that uses a buffer placed in device memory to "
           "measure resource "
           "preparation time. The resource is destroyed and recreated for each "
           "iteration "
