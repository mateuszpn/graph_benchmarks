...
std::string getCompilerOptions(DataType dataType, MathOperation operation,
  ...
  options.addDefinitionKeyValue("DATATYPE",
                                DataTypeHelper::toOpenclC(dataType));
...
static TestResult run(const DoMathOperationArguments &arguments,
  ...
  // Check support
  if (!MathOperationHelper::isSupportedAsNormal(arguments.operation,
                                                arguments.dataType)) {
  ...
  const size_t operationsPerLoop =
      128u; // non-tweakable constant, this is hardcoded in the kernel code
  const size_t enqueueCount =
      arguments.iterations + 1; // non-tweakable constant
  const auto data = MathOperationHelper::generateTestData(
      arguments.dataType, arguments.operation, loopIterations,
      operationsPerLoop, enqueueCount);

  // Create and initialize the buffer with test data
  cl_mem buffer = clCreateBuffer(opencl.context, CL_MEM_READ_WRITE,
                                 gws * data.sizeOfDataType, nullptr, &retVal);
  ASSERT_CL_SUCCESS(retVal);
  ASSERT_CL_SUCCESS(clEnqueueWriteBuffer(
      opencl.commandQueue, buffer, CL_FALSE, 0, data.sizeOfDataType,
      data.initialValue, 0, nullptr, nullptr));
  ...
  const std::string compilerOptions =
      getCompilerOptions(arguments.dataType, arguments.operation, false);
  ...
  if (arguments.mixGrfModes) {
    const std::string largeGrfCompilerOptions =
        getCompilerOptions(arguments.dataType, arguments.operation, true);
    if (auto result = ProgramHelperOcl::buildProgramFromSourceFile(
            opencl.context, opencl.device, programName,
            largeGrfCompilerOptions.c_str(), largeGrfProgram);
        result != TestResult::Success) {
      return result;
    }
    ...
    ASSERT_CL_SUCCESS(
        clSetKernelArg(largeGrfKernel, 0, sizeof(buffer), &buffer));
    ASSERT_CL_SUCCESS(clSetKernelArg(largeGrfKernel, 1, data.sizeOfDataType,
                                     data.otherArgument));
    ASSERT_CL_SUCCESS(clSetKernelArg(
        largeGrfKernel, 2, sizeof(data.loopIterations), &data.loopIterations));
    ASSERT_CL_SUCCESS(
        clEnqueueNDRangeKernel(opencl.commandQueue, largeGrfKernel, 1, nullptr,
                               &gws, &lws, 0, nullptr, eventForEnqueue));
  ...
  ASSERT_CL_SUCCESS(
      clSetKernelArg(kernel, 1, data.sizeOfDataType, data.otherArgument));
  ASSERT_CL_SUCCESS(clSetKernelArg(kernel, 2, sizeof(data.loopIterations),
                                   &data.loopIterations));
  ...
  for (auto i = 0u; i < arguments.iterations; i++) {
    ...
    ASSERT_CL_SUCCESS(clEnqueueNDRangeKernel(
        opencl.commandQueue, kernelForExecution, 1, nullptr, &gws, &lws, 0,
        nullptr, eventForEnqueue));
    ASSERT_CL_SUCCESS(clFinish(opencl.commandQueue));
    timer.measureEnd();
    if (eventForEnqueue) {
      cl_ulong timeNs{};
...
static RegisterTestCaseImplementation<DoMathOperation>
