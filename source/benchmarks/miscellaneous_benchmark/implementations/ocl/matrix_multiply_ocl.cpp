...
static TestResult run(const MatrixMultiplyArguments &arguments,
  ...
  // Prepare data
  const size_t sizeInElements = arguments.numberOfElementsX *
                                arguments.numberOfElementsY *
                                arguments.numberOfElementsZ;
  ...
  const size_t gws[] = {arguments.numberOfElementsX,
                        arguments.numberOfElementsY,
                        arguments.numberOfElementsZ};
  ...
  cl_mem bufferX =
      clCreateBuffer(opencl.context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,
                     sizeInBytes, dataX.get(), &retVal);
  ASSERT_CL_SUCCESS(retVal);

  cl_mem bufferY =
      clCreateBuffer(opencl.context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,
                     sizeInBytes, dataY.get(), &retVal);
  ASSERT_CL_SUCCESS(retVal);

  cl_mem bufferResults = clCreateBuffer(opencl.context, CL_MEM_READ_WRITE,
                                        sizeInBytes, nullptr, &retVal);
  ASSERT_CL_SUCCESS(retVal);
  ...
  // Warmup kernel
  ASSERT_CL_SUCCESS(
      clSetKernelArg(kernel, 0, sizeof(bufferResults), &bufferResults));
  ...
  ASSERT_CL_SUCCESS(clEnqueueNDRangeKernel(opencl.commandQueue, kernel, 3,
  ...
  ASSERT_CL_SUCCESS(clEnqueueReadBuffer(
      opencl.commandQueue, bufferResults, true, 0u, sizeInBytes,
      resultsFromRun.get(), 0u, nullptr, nullptr));
  ...
  for (auto i = 0u; i < arguments.iterations; i++) {
    ...
    ASSERT_CL_SUCCESS(
        ProfilingHelper::getEventDurationInNanoseconds(profilingEvent, timeNs));
    ASSERT_CL_SUCCESS(clReleaseEvent(profilingEvent));
    statistics.pushValue(std::chrono::nanoseconds{timeNs}, sizeInBytes * 3,
                         typeSelector.getUnit(), typeSelector.getType(), "bw");
  }
...
static RegisterTestCaseImplementation<MatrixMultiply>
