...
static TestResult run(const QueueInOrderMemcpyArguments &arguments,
  ...
  QueueProperties queueProperties = QueueProperties::create()
                                        .setForceBlitter(arguments.isCopyOnly)
                                        .allowCreationFail();
  ...
  ASSERT_ZE_RESULT_SUCCESS(BufferContentsHelperL0::fillBuffer(
      levelzero, source, arguments.size, BufferContents::Random, true));
  ASSERT_ZE_RESULT_SUCCESS(BufferContentsHelperL0::fillBuffer(
      levelzero, destination, arguments.size, BufferContents::Random, true));
  ...
  ASSERT_ZE_RESULT_SUCCESS(
      zeCommandListCreateImmediate(levelzero.context, levelzero.device,
                                   &levelzero.commandQueueDesc, &cmdList));
  // Warmup
  ze_event_handle_t signalEvent = events[0];
  ASSERT_ZE_RESULT_SUCCESS(zeCommandListAppendMemoryCopy(
      cmdList, destination, source, arguments.size, signalEvent, 0, nullptr));
  ze_event_handle_t waitEvent = signalEvent;
  for (auto j = 1u; j < arguments.count; ++j) {
    signalEvent = events[j];
    ASSERT_ZE_RESULT_SUCCESS(zeCommandListAppendMemoryCopy(
        cmdList, destination, source, arguments.size, signalEvent, 1,
        &waitEvent));
    waitEvent = signalEvent;
  }
  ASSERT_ZE_RESULT_SUCCESS(
      zeEventHostSynchronize(waitEvent, std::numeric_limits<uint64_t>::max()));
  ...
  for (auto i = 0u; i < arguments.iterations; i++) {
    ...
    ASSERT_ZE_RESULT_SUCCESS(zeEventHostSynchronize(
        waitEvent, std::numeric_limits<uint64_t>::max()));
    timer.measureEnd();
...
static RegisterTestCaseImplementation<QueueInOrderMemcpy>
