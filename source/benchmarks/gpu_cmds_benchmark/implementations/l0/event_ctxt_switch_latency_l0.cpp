...
static TestResult run(const EventCtxtSwitchLatencyArguments &arguments,
  ...
  QueueProperties queuePropertiesFirst =
      QueueProperties::create().setForceEngine(arguments.firstEngine);
  ...
  QueueProperties queuePropertiesSecond =
      QueueProperties::create().setForceEngine(arguments.secondEngine);
  ...
  ASSERT_ZE_RESULT_SUCCESS(zeCommandListCreate(
      levelzero.context, levelzero.device, &cmdListDescFirst, &cmdListFirst));
  ...
  ASSERT_ZE_RESULT_SUCCESS(zeCommandListCreate(
      levelzero.context, levelzero.device, &cmdListDescSecond, &cmdListSecond));
  ...
  ASSERT_ZE_RESULT_SUCCESS(zeCommandListCreate(
      levelzero.context, levelzero.device, &cmdListDescReset, &cmdListReset));

  ASSERT_ZE_RESULT_SUCCESS(zeCommandListAppendWriteGlobalTimestamp(
      cmdListFirst, beginTimestamp, nullptr, 0, nullptr));
  for (auto commandIndex = 0u; commandIndex < arguments.measuredCommands;
       commandIndex++) {
    if (commandIndex % 2 == 0) {
      ASSERT_ZE_RESULT_SUCCESS(
          zeCommandListAppendSignalEvent(cmdListFirst, events[commandIndex]));
      ASSERT_ZE_RESULT_SUCCESS(zeCommandListAppendWaitOnEvents(
          cmdListFirst, 1, &events[commandIndex + 1]));
    } else {
      ASSERT_ZE_RESULT_SUCCESS(zeCommandListAppendWaitOnEvents(
          cmdListSecond, 1, &events[commandIndex - 1]));
      ASSERT_ZE_RESULT_SUCCESS(
          zeCommandListAppendSignalEvent(cmdListSecond, events[commandIndex]));
    }

    ASSERT_ZE_RESULT_SUCCESS(
        zeCommandListAppendEventReset(cmdListReset, events[commandIndex]));
  }
  ASSERT_ZE_RESULT_SUCCESS(zeCommandListAppendWriteGlobalTimestamp(
      cmdListFirst, endTimestamp, nullptr, 0, nullptr));
  ...
  ASSERT_ZE_RESULT_SUCCESS(zeCommandQueueExecuteCommandLists(
      cmdQueueFirst, 1, &cmdListFirst, nullptr));
  ASSERT_ZE_RESULT_SUCCESS(zeCommandQueueExecuteCommandLists(
      cmdQueueSecond, 1, &cmdListSecond, nullptr));
  ASSERT_ZE_RESULT_SUCCESS(zeCommandQueueSynchronize(
      cmdQueueFirst, std::numeric_limits<uint64_t>::max()));

  // reset all events before next round
  ASSERT_ZE_RESULT_SUCCESS(zeCommandQueueExecuteCommandLists(
      cmdQueueFirst, 1, &cmdListReset, nullptr));
  ASSERT_ZE_RESULT_SUCCESS(zeCommandQueueSynchronize(
      cmdQueueFirst, std::numeric_limits<uint64_t>::max()));
...
static RegisterTestCaseImplementation<EventCtxtSwitchLatency>
