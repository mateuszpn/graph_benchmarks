...
std::unique_ptr<QueueFamiliesHelper::QueueDesc>
QueueFamiliesHelper::getPropertiesForSelectingEngine(ze_device_handle_t device,
                                                     Engine engine) {
...
std::vector<QueueFamiliesHelper::QueueFamilyDesc>
QueueFamiliesHelper::queryQueueFamilies(ze_device_handle_t device) {
  // Get queue ordinals
  uint32_t familiesCount = 0;
  EXPECT_ZE_RESULT_SUCCESS(
      zeDeviceGetCommandQueueGroupProperties(device, &familiesCount, nullptr));
  ...
  EXPECT_ZE_RESULT_SUCCESS(zeDeviceGetCommandQueueGroupProperties(
      device, &familiesCount, families.data()));
  ...
  for (uint32_t familyIndex = 0; familyIndex < familiesCount; familyIndex++) {
    const ze_command_queue_group_properties_t properties =
...
EngineGroup QueueFamiliesHelper::getEngineGroup(
    const ze_command_queue_group_properties_t family, bool &copyEngineFound) {
  const bool isCompute =
      family.flags & ZE_COMMAND_QUEUE_GROUP_PROPERTY_FLAG_COMPUTE;
  ...
  if (isCompute) {
    if (isCopy) {
      return EngineGroup::Compute; // this could also be RCS, no way to know
                                   // this
    } else {
      return EngineGroup::Unknown;
    }
  ...
  if (isCopy) {
    if (copyEngineFound) {
      ...
      return EngineGroup::Copy; // Assuming main copy engines are reported
                                // before link copy
    }
  }
...
size_t QueueFamiliesHelper::getCopyEnginesCount(
    const std::vector<QueueFamiliesHelper::QueueFamilyDesc> &families) {
