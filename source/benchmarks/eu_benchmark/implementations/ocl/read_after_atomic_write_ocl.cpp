...
static TestResult run(const ReadAfterAtomicWriteArguments &arguments,
  ...
  cl_mem buffer = clCreateBuffer(opencl.context, CL_MEM_READ_WRITE,
                                 gws * sizeof(cl_int), nullptr, &retVal);
  ASSERT_CL_SUCCESS(retVal);
  ASSERT_CL_SUCCESS(clEnqueueFillBuffer(
      opencl.commandQueue, buffer, &initialValue, sizeof(initialValue), 0,
      gws * sizeof(cl_int), 0, nullptr, nullptr));
  ...
  ASSERT_CL_SUCCESS(
      clSetKernelArg(kernel, 2, sizeof(shuffleRead), &shuffleRead));
  ASSERT_CL_SUCCESS(
      clSetKernelArg(kernel, 3, sizeof(loopIterations), &loopIterations));
...
static RegisterTestCaseImplementation<ReadAfterAtomicWrite>
