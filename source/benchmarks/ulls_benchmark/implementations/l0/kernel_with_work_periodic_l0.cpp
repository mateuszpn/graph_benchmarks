...
#include <thread>
static TestResult run(const KernelWithWorkPeriodicArguments &arguments,
  ...
  // Create kernel
  auto spirvModule =
      FileHelper::loadBinaryFile("ulls_benchmark_write_one_global_ids.spv");
  ...
  // Benchmark
  const auto submissionDelay =
      std::chrono::microseconds(arguments.timeBetweenSubmissions);
  for (auto i = 0u; i < arguments.iterations; ++i) {
    ...
    for (auto numKernel = 0u; numKernel < arguments.numSubmissions;
         ++numKernel) {
    ...
    statistics.pushValue(executionTime, typeSelector.getUnit(),
                         typeSelector.getType());
    if (powerMeter.isEnabled()) {
      statistics.pushValue(powerMeter.getTime(), typeSelector.getUnit(),
                           typeSelector.getType(), "timeWithSleeps");
      statistics.pushEnergy(powerMeter.getEnergy(),
                            MeasurementUnit::MicroJoules, MeasurementType::Gpu,
                            "energy");
      statistics.pushEnergy(powerMeter.getPower(), MeasurementUnit::Watts,
                            MeasurementType::Gpu, "avgPower");
    }
  }
...
static RegisterTestCaseImplementation<KernelWithWorkPeriodic>
