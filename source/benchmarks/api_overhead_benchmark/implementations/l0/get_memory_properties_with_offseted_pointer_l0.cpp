...
static TestResult
run(const GetMemoryPropertiesWithOffsetedPointerArguments &arguments,
  ...
  for (int64_t i = 0; i < arguments.AllocationsCount; i++) {
    ...
    ASSERT_ZE_RESULT_SUCCESS(
        zeMemGetAllocProperties(levelzero.context, ptr, &properties, nullptr));
  }
  ...
  for (auto i = 0u; i < arguments.iterations; i++) {
    ...
    for (int64_t j = 0; j < arguments.AllocationsCount; ++j) {
      ...
      ASSERT_ZE_RESULT_SUCCESS(zeMemGetAllocProperties(levelzero.context, ptr,
                                                       &properties, nullptr));
    }
    timer.measureEnd();
...
static RegisterTestCaseImplementation<GetMemoryPropertiesWithOffsetedPointer>
