...
static TestResult run(const Int64DivArguments &arguments,
  ...
  ASSERT_CL_SUCCESS(
      clSetKernelArg(kernel, 2, sizeof(loopIterations), &loopIterations));
  ...
  ASSERT_CL_SUCCESS(clEnqueueReadBuffer(opencl.commandQueue, buffer,
                                        CL_BLOCKING, 0, sizeof(cl_long), result,
                                        0, nullptr, nullptr));
...
static RegisterTestCaseImplementation<Int64Div> registerTestCase(run,
                                                                 Api::OpenCL);
