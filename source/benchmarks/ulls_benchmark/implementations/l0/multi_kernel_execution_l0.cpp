...
static TestResult run(const MultiKernelExecutionArguments &arguments,
  ...
  // Create kernel
  auto spirvModule =
      FileHelper::loadBinaryFile("ulls_benchmark_multi_kernel_execution.spv");
  ...
  kernelDesc.pKernelName =
      arguments.inOrderOverOOO ? "emptyWithSynchro" : "empty";
  ...
  if (!arguments.inOrderOverOOO) {
    ...
    for (auto iteration = 0u; iteration < arguments.iterations; iteration++) {
      ...
      if (arguments.profiling) {
        ...
        statistics.pushValue(totalTime, typeSelector.getUnit(),
                             MeasurementType::Gpu);
      } else {
    ...
    for (auto i = 0u; i < arguments.kernelCount; i++) {
      ze_event_desc_t eventDesc = {ZE_STRUCTURE_TYPE_EVENT_DESC, nullptr, i, 0,
                                   0};
      ASSERT_ZE_RESULT_SUCCESS(
          zeEventCreate(hEventPool, &eventDesc, &events[i]));
    }
    ...
    for (auto i = 0u; i < arguments.kernelCount; i++) {
      ASSERT_ZE_RESULT_SUCCESS(zeKernelSetArgumentValue(
          kernel, 0, sizeof(eventAddress), &eventAddress));
      ASSERT_ZE_RESULT_SUCCESS(zeKernelSetArgumentValue(
          kernel, 1, sizeof(completionValue), &completionValue));
      ASSERT_ZE_RESULT_SUCCESS(
          zeKernelSetArgumentValue(kernel, 2, sizeof(delayValue), &delayValue));
      ASSERT_ZE_RESULT_SUCCESS(zeCommandListAppendLaunchKernel(
          cmdList, kernel, &groupCount, events[i], 0, nullptr));
      ASSERT_ZE_RESULT_SUCCESS(
          eventGetAddressFunc(events[i], &completionValue, &eventAddress));
    }
...
static RegisterTestCaseImplementation<MultiKernelExecution>
