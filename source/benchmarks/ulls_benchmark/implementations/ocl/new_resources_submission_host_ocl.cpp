...
static TestResult run(const NewResourcesSubmissionHostArguments &arguments,
  ...
  void *hostMemory =
      clHostMemAllocINTEL(opencl.context, nullptr, 64, 0, &retVal);
  ...
  for (auto i = 0u; i < arguments.iterations; i++) {
    timer.measureStart();
    hostMemory =
        clHostMemAllocINTEL(opencl.context, nullptr, sizeInBytes, 0, &retVal);
    ASSERT_CL_SUCCESS(clSetKernelArgSVMPointer(kernel, 0, hostMemory));
    ASSERT_CL_SUCCESS(clEnqueueNDRangeKernel(opencl.commandQueue, kernel, 1,
                                             nullptr, &gws, &lws, 0, nullptr,
                                             nullptr));
...
static RegisterTestCaseImplementation<NewResourcesSubmissionHost>
