...
static TestResult run(const KernelSwitchLatencyArguments &arguments,
  ...
  ASSERT_CL_SUCCESS(retVal);
  const int kernelOperationsCount =
      std::max(0, static_cast<int>(arguments.kernelExecutionTime * 1.75));

  // Warmup, kernel
  ASSERT_CL_SUCCESS(
      clSetKernelArg(kernel, 0, sizeof(int), &kernelOperationsCount));
  ...
  for (auto i = 0u; i < arguments.iterations; i++) {
    ...
    for (auto j = 1u; j < arguments.kernelCount; j++) {
      ASSERT_CL_SUCCESS(clEnqueueNDRangeKernel(
          opencl.commandQueue, kernel, 1, nullptr, &gws, &lws, 1,
          &profilingEvents[j - 1], &profilingEvents[j]));
    }
    ...
    for (auto j = 1u; j < arguments.kernelCount; j++) {
      ASSERT_CL_SUCCESS(clGetEventProfilingInfo(
          profilingEvents[j - 1], CL_PROFILING_COMMAND_END, sizeof(cl_ulong),
          &end, nullptr));
      ASSERT_CL_SUCCESS(clGetEventProfilingInfo(
          profilingEvents[j], CL_PROFILING_COMMAND_START, sizeof(cl_ulong),
          &start, nullptr));
