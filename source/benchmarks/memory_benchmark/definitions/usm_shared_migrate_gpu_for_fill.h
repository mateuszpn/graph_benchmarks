...
struct UsmSharedMigrateGpuForFillArguments : TestCaseArgumentContainer {
  ...
  UsmSharedMigrateGpuForFillArguments()
      : bufferSize(*this, "size", "Size of the buffer"),
        prefetchMemory(*this, "prefetch",
                       "Explicitly migrate shared allocation to device "
                       "associated with command queue"),
        preferredLocation(*this, "preferredLocation",
                          "Apply memadvise with preferred device location "
                          "(system, device, none)"),
        forceBlitter(*this, "forceBlitter", CommonHelpMessage::forceBlitter()) {
  }
};

struct UsmSharedMigrateGpuForFill
    : TestCase<UsmSharedMigrateGpuForFillArguments> {
  ...
  std::string getHelp() const override {
    return "allocates a unified shared memory buffer and measures bandwidth "
           "for memory fill operation that must migrate resource from CPU to "
           "GPU";
  }
};
