...
static TestResult run(const DriverGetPropertiesArguments &arguments,
  ...
  ASSERT_ZE_RESULT_SUCCESS(
      zeDriverGetProperties(drivers[0], &driverPropertiesWarmUp));
  ...
  for (auto j = 0u; j < arguments.iterations; j++) {
    ...
    timer.measureStart();
    ASSERT_ZE_RESULT_SUCCESS(
        zeDriverGetProperties(drivers[0], &driverProperties));
    timer.measureEnd();
...
static RegisterTestCaseImplementation<DriverGetProperties>
