...
static TestResult run(const StreamAfterTransferArguments &arguments,
  ...
  cl_mem cleanerBuffer = clCreateBuffer(opencl.context, CL_MEM_READ_ONLY,
                                        cleanerSize, nullptr, &retVal);
  ASSERT_CL_SUCCESS(retVal);
  ASSERT_CL_SUCCESS(
      clSetKernelArg(cacheCleaner, 0u, sizeof(cleanerBuffer), &cleanerBuffer));
  ASSERT_CL_SUCCESS(
      clSetKernelArg(cacheCleaner, 1u, elementSize, &scalarValue));
  ...
  for (auto i = 0u; i < arguments.iterations; i++) {
    ...
    ASSERT_CL_SUCCESS(clEnqueueNDRangeKernel(
        opencl.commandQueue, cacheCleaner, 1, nullptr, &cleanCacheWorkSize,
    ...
    for (auto bufferId = 0u; bufferId < buffersCount; bufferId++) {
      ASSERT_CL_SUCCESS(clEnqueueWriteBuffer(
          opencl.commandQueue, buffers[bufferId], true, 0u,
          bufferSizes[bufferId], data.get(), 0u, nullptr, nullptr));
    }
...
static RegisterTestCaseImplementation<StreamAfterTransfer>
