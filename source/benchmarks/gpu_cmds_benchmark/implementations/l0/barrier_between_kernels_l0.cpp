...
static TestResult run(const BarrierBetweenKernelsArguments &arguments,
  ...
  if (arguments.remoteAccess &&
      arguments.flushedMemory == UsmMemoryPlacement::Host) {
    return TestResult::DeviceNotCapable;
  }
  ...
  ASSERT_ZE_RESULT_SUCCESS(
      zeMemAllocHost(levelzero.context, &hostAllocationDesc,
                     timestampBufferSize, 0, &timestampBuffer));
  ASSERT_ZE_RESULT_SUCCESS(
      zeContextMakeMemoryResident(levelzero.context, levelzero.device,
                                  timestampBuffer, timestampBufferSize))
  ...
  if (arguments.flushedMemory == UsmMemoryPlacement::Device) {
    if (arguments.remoteAccess) {
      ASSERT_ZE_RESULT_SUCCESS(zeMemAllocDevice(
          levelzero.context, &deviceAllocationDesc, outputBufferSize, 0,
          levelzero.getDevice(DeviceSelection::Tile1), &outputBuffer));
    } else {
      ASSERT_ZE_RESULT_SUCCESS(zeMemAllocDevice(
          levelzero.context, &deviceAllocationDesc, outputBufferSize, 0,
          levelzero.getDevice(DeviceSelection::Tile0), &outputBuffer));
    }
  } else {
    ASSERT_ZE_RESULT_SUCCESS(
        zeMemAllocHost(levelzero.context, &hostAllocationDesc, outputBufferSize,
                       0, &outputBuffer));
  }

  ASSERT_ZE_RESULT_SUCCESS(zeContextMakeMemoryResident(
      levelzero.context, levelzero.device, outputBuffer, outputBufferSize));
  ...
  if (arguments.onlyReads == 0) {
    ...
    ASSERT_ZE_RESULT_SUCCESS(zeCommandListCreate(
        levelzero.context, levelzero.device, &cmdListDesc, &cmdListForFill));
    uint32_t pattern = 1u;
    ASSERT_ZE_RESULT_SUCCESS(zeCommandListAppendMemoryFill(
        cmdListForFill, outputBuffer, &pattern, sizeof(uint32_t),
        outputBufferSize, nullptr, 0, nullptr));
    ASSERT_ZE_RESULT_SUCCESS(zeCommandListClose(cmdListForFill));

    // Warmup
    ASSERT_ZE_RESULT_SUCCESS(zeCommandQueueExecuteCommandLists(
        levelzero.commandQueue, 1, &cmdListForFill, nullptr));
    ASSERT_ZE_RESULT_SUCCESS(zeCommandQueueSynchronize(
        levelzero.commandQueue, std::numeric_limits<uint64_t>::max()));
  }

  // Create kernel
  auto spirvModule = FileHelper::loadBinaryFile(
      "gpu_cmds_benchmark_write_one_global_ids_with_check.spv");
  ...
  ASSERT_ZE_RESULT_SUCCESS(zeKernelSetGroupSize(
      kernel, static_cast<uint32_t>(workgroupSize), 1u, 1u));
  ASSERT_ZE_RESULT_SUCCESS(
      zeKernelSetArgumentValue(kernel, 0, sizeof(outputBuffer), &outputBuffer));
  ...
  const ze_group_count_t groupCount{static_cast<uint32_t>(workgroupCount), 1u,
                                    1u};
  ...
  ASSERT_ZE_RESULT_SUCCESS(zeCommandListAppendWriteGlobalTimestamp(
      cmdList, beginTimestamp, nullptr, 0, nullptr));
  ASSERT_ZE_RESULT_SUCCESS(
      zeCommandListAppendBarrier(cmdList, event, 0u, nullptr));
  ASSERT_ZE_RESULT_SUCCESS(zeCommandListAppendWriteGlobalTimestamp(
      cmdList, endTimestamp, nullptr, 0, nullptr));
  ASSERT_ZE_RESULT_SUCCESS(zeCommandListAppendWriteGlobalTimestamp(
      cmdList, meassurmentCostStart, nullptr, 0, nullptr));
  ASSERT_ZE_RESULT_SUCCESS(zeCommandListAppendWriteGlobalTimestamp(
      cmdList, meassurmentCostEnd, nullptr, 0, nullptr));
  ...
  for (auto i = 0u; i < arguments.iterations; i++) {
    ...
    auto commandTime =
        std::chrono::nanoseconds(*endTimestamp - *beginTimestamp);
    auto meassureTime =
        std::chrono::nanoseconds(*meassurmentCostEnd - *meassurmentCostStart);
    if (commandTime >= meassureTime) {
      commandTime -= meassureTime;
    }

    commandTime *= timerResolution;
    statistics.pushValue(commandTime, typeSelector.getUnit(),
  ...
  ASSERT_ZE_RESULT_SUCCESS(zeContextEvictMemory(
      levelzero.context, levelzero.device, outputBuffer, outputBufferSize));
  ASSERT_ZE_RESULT_SUCCESS(zeMemFree(levelzero.context, outputBuffer));
  ASSERT_ZE_RESULT_SUCCESS(
      zeContextEvictMemory(levelzero.context, levelzero.device, timestampBuffer,
                           timestampBufferSize));
  ASSERT_ZE_RESULT_SUCCESS(zeMemFree(levelzero.context, timestampBuffer));
  return TestResult::Success;
}

static RegisterTestCaseImplementation<BarrierBetweenKernels>
