...
static TestResult run(const KernelAndCopyArguments &arguments,
  ...
  // Create queues
  const auto queueForCopyProperties =
      QueueProperties::create()
          .setForceBlitter(arguments.useCopyQueue)
          .allowCreationFail();
  ...
  if (arguments.twoQueues) {
    ...
    if (arguments.runKernel) {
      ...
      if (arguments.runCopy) {
        FATAL_ERROR_IF(arguments.useCopyQueue,
                       "Configuration (runKernel && useCopyQueue && "
                       "!twoQueues) is invalid");
  ...
  if (arguments.runKernel) {
    bufferForKernel = clCreateBuffer(opencl.context, CL_MEM_READ_WRITE,
                                     bufferForKernelSize, nullptr, &retVal);
  }
  if (arguments.runCopy) {
    bufferForCopy1 = clCreateBuffer(opencl.context, CL_MEM_READ_WRITE,
                                    bufferForCopySize, nullptr, &retVal);
    bufferForCopy2 = clCreateBuffer(opencl.context, CL_MEM_READ_WRITE,
                                    bufferForCopySize, nullptr, &retVal);
  }

  // Create kernel
  const char *source =
      "__kernel void fill_with_ones(__global char *buffer, int size) { "
  ...
  if (arguments.runKernel) {
    ASSERT_CL_SUCCESS(
        clSetKernelArg(kernel, 0, sizeof(bufferForKernel), &bufferForKernel));
    ASSERT_CL_SUCCESS(clSetKernelArg(kernel, 1, sizeof(bufferForKernelSize),
                                     &bufferForKernelSize));
    ASSERT_CL_SUCCESS(clEnqueueNDRangeKernel(queueForKernel, kernel, 1, nullptr,
                                             &gws, &lws, 0, nullptr, nullptr));
    ASSERT_CL_SUCCESS(clFinish(queueForKernel));
  }
  if (arguments.runCopy) {
    ASSERT_CL_SUCCESS(
        clEnqueueCopyBuffer(queueForCopy, bufferForCopy1, bufferForCopy2, 0, 0,
                            bufferForCopySize, 0, nullptr, nullptr));
  ...
  for (auto i = 0u; i < arguments.iterations; i++) {
    ...
    if (arguments.runKernel) {
      ASSERT_CL_SUCCESS(clEnqueueNDRangeKernel(
          queueForKernel, kernel, 1, nullptr, &gws, &lws, 0, nullptr, nullptr));
    }
    if (arguments.runCopy) {
      ASSERT_CL_SUCCESS(
          clEnqueueCopyBuffer(queueForCopy, bufferForCopy1, bufferForCopy2, 0,
                              0, bufferForCopySize, 0, nullptr, nullptr));
    }
...
static RegisterTestCaseImplementation<KernelAndCopy>
