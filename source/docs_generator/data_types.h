...
struct BenchmarkInstance {
  ...
  BenchmarkInstance(const std::string &location, const std::string &name,
                    const std::string &path)
      : location(location), baseName(deduceBaseName(name)), path(path),
        api(deduceApi(name)) {}

  static std::string deduceBaseName(const std::string &name) {
    const auto position = name.find_last_of("_");
    FATAL_ERROR_IF(position == std::string::npos,
                   "Could not deduce base name from ", name);
    return name.substr(0, position);
  }

  static Api deduceApi(const std::string &name) {
    const auto position = name.find_last_of("_");
    FATAL_ERROR_IF(position == std::string::npos, "Could not deduce API from ",
                   name);
    const std::string apiString = name.substr(position + 1);
    const Api api = parseApi(apiString);
    FATAL_ERROR_IF(api == Api::Unknown || api == Api::All,
                   "Could not parse API from ", name);
    return api;
  }
};
...
using TestCaseArguments =
    std::map<std::string, TestCaseArgument, LexicographicalLess>;
