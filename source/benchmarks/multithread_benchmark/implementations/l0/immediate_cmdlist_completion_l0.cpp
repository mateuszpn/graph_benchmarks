...
static void issueToImmediateCmdList(ThreadSpecificData *threadData,
  ...
  EXPECT_ZE_RESULT_SUCCESS(zeCommandListAppendMemoryCopy(
      threadData->cmdList, threadData->deviceDstMemory,
  ...
  EXPECT_ZE_RESULT_SUCCESS(zeEventHostSynchronize(
      threadData->event, std::numeric_limits<uint64_t>::max()));
  threadData->timer.measureEnd();
}

static TestResult
getEngineInfo(std::vector<EngineInfo> &supportedEngineInfo,
              LevelZero &levelzero, std::string_view engineGroup,
              const ImmediateCommandListCompletionArguments &arguments) {
  ...
  ASSERT_ZE_RESULT_SUCCESS(zeDeviceGetCommandQueueGroupProperties(
      levelzero.device, &queueGroupPropertiesCount, nullptr));
  FATAL_ERROR_IF(queueGroupPropertiesCount == 0,
                 "No queue group properties found!");
  std::vector<ze_command_queue_group_properties_t> queueGroupProperties(
      queueGroupPropertiesCount);
  ASSERT_ZE_RESULT_SUCCESS(zeDeviceGetCommandQueueGroupProperties(
      levelzero.device, &queueGroupPropertiesCount,
      queueGroupProperties.data()));

  auto insertToEngineInfoList = [&supportedEngineInfo](auto &numQueues,
                                                       auto &ordinal) {
  ...
  return supportedEngineInfo.size() > 0 ? TestResult::Success
                                        : TestResult::Error;
}

static TestResult run(const ImmediateCommandListCompletionArguments &arguments,
                      Statistics &statistics) {
  // Setup
  LevelZero levelzero;

  std::vector<EngineInfo> supportedEngineInfo{};
  std::string engineGroup =
      static_cast<const std::string &>(arguments.engineGroup);
  const auto status =
      getEngineInfo(supportedEngineInfo, levelzero, engineGroup, arguments);
  if (status != TestResult::Success) {
    return status;
  }

  const auto numberOfSupportedEngines = supportedEngineInfo.size();
  if (supportedEngineInfo.size() * arguments.threadsPerEngine >
  ...
  for (auto i = 0u; i < arguments.numberOfThreads; i++) {
    commandQueueDesc.ordinal =
        supportedEngineInfo[i % numberOfSupportedEngines].ordinal;
    commandQueueDesc.index =
        supportedEngineInfo[i % numberOfSupportedEngines].engineIndex;
    ASSERT_ZE_RESULT_SUCCESS(zeCommandListCreateImmediate(
        levelzero.context, levelzero.device, &commandQueueDesc,
        &threadData[i].cmdList));
    eventDesc.index = i;
    ASSERT_ZE_RESULT_SUCCESS(
        zeEventCreate(eventPool, &eventDesc, &threadData[i].event));
    ASSERT_ZE_RESULT_SUCCESS(UsmHelper::allocate(UsmMemoryPlacement::Host,
                                                 levelzero, arguments.copySize,
                                                 &threadData[i].hostSrcMemory));
    ASSERT_ZE_RESULT_SUCCESS(UsmHelper::allocate(
        UsmMemoryPlacement::Device, levelzero, arguments.copySize,
        &threadData[i].deviceDstMemory));
    threadData[i].maxMemoryAllocSize =
        static_cast<uint32_t>(arguments.copySize);
  }
  ...
  for (auto i = 0u; i < arguments.iterations; i++) {
    ...
    auto aggregatedThreadDuration =
        static_cast<std::chrono::high_resolution_clock::duration>(0);
    for (auto j = 0u; j < arguments.numberOfThreads; j++) {
      zeEventHostReset(threadData[j].event);
      aggregatedThreadDuration += threadData[j].timer.get();
    }
    const auto averageThreadDuration =
        aggregatedThreadDuration / arguments.numberOfThreads;
    statistics.pushValue(averageThreadDuration, MeasurementUnit::Microseconds,
                         MeasurementType::Cpu, "Average Thread Duration");
  }

  // Cleanup
  for (auto i = 0u; i < arguments.numberOfThreads; i++) {
    ASSERT_ZE_RESULT_SUCCESS(UsmHelper::deallocate(
        UsmMemoryPlacement::Host, levelzero, threadData[i].hostSrcMemory));
    ASSERT_ZE_RESULT_SUCCESS(UsmHelper::deallocate(
        UsmMemoryPlacement::Device, levelzero, threadData[i].deviceDstMemory));
...
static RegisterTestCaseImplementation<ImmediateCommandListCompletion>
