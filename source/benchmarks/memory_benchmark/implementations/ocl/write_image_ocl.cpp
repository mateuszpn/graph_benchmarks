...
static TestResult run(const WriteImageArguments &arguments,
  ...
  ASSERT_CL_SUCCESS(clEnqueueWriteImage(opencl.commandQueue, image, true,
  ...
  for (auto i = 0u; i < arguments.iterations; i++) {
    ...
    timer.measureStart();
    ASSERT_CL_SUCCESS(
        clEnqueueWriteImage(opencl.commandQueue, image, true, origin, region, 0,
                            0, hostptrAlloc.ptr, 0, nullptr, eventForEnqueue));
    timer.measureEnd();
...
static RegisterTestCaseImplementation<WriteImage> registerTestCase(run,
                                                                   Api::OpenCL);
