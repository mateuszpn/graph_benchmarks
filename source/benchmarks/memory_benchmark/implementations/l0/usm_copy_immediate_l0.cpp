...
static TestResult run(const UsmCopyImmediateArguments &arguments,
  ...
  if (isUsmMemoryType(arguments.sourcePlacement)) {
    ASSERT_ZE_RESULT_SUCCESS(BufferContentsHelperL0::fillBuffer(
        levelzero, source, arguments.size, arguments.contents, true));
  }
  if (isUsmMemoryType(arguments.destinationPlacement)) {
    ASSERT_ZE_RESULT_SUCCESS(BufferContentsHelperL0::fillBuffer(
        levelzero, destination, arguments.size, arguments.contents, true));
  }

  // Warmup
  ASSERT_ZE_RESULT_SUCCESS(zeCommandListAppendMemoryCopy(
      cmdList, destination, source, arguments.size, event, 0, nullptr));
