...
static TestResult run(const IoqKernelSwitchLatencyArguments &arguments,
  ...
  const std::vector<uint8_t> kernelSource =
      FileHelper::loadTextFile("benchmark_write_multiple.cl");
  ...
  for (auto i = 0u; i < arguments.iterations; ++i) {
    ...
    for (auto j = 1u; j < arguments.kernelCount; ++j) {
      ...
      } // disregard invalid values (where kernel start is before previous
        // kernel end)
    }
    ...
    statistics.pushValue(switchTime / validSwitchTimeValueCount,
...
static RegisterTestCaseImplementation<IoqKernelSwitchLatency>
