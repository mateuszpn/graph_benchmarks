...
static TestResult run(const FlushTimeArguments &arguments,
  ...
  QueueProperties queueProperties =
      QueueProperties::create().setOoq(arguments.useOoq);
  ...
  ASSERT_CL_SUCCESS(clEnqueueNDRangeKernel(opencl.commandQueue, kernel, 1,
                                           nullptr, &gws, lwsForNdr, 0, nullptr,
                                           eventForNdr));
...
static RegisterTestCaseImplementation<FlushTime> registerTestCase(run,
                                                                  Api::OpenCL);
