...
struct UsmP2PCopyArguments : TestCaseArgumentContainer {
  ...
  UsmP2PCopyArguments()
      : srcDeviceId(*this, "srcDeviceId", "Source device"),
        ...
        reuseCommandList(*this, "reuseCmdList",
                         "Command list is reused between iterations") {}
};

struct UsmEUCopy : TestCase<UsmP2PCopyArguments> {
  using TestCase<UsmP2PCopyArguments>::TestCase;

  std::string getTestCaseName() const override { return "UsmEUCopy"; }

  std::string getHelp() const override {
    return "allocates two unified device memory buffers on separate devices, "
           "performs a copy "
           "between them using a compute engine, and reports bandwidth. Test "
           "first checks for "
           "P2P capabilities in the target platform before submitting the "
           "copy.";
  }
};
