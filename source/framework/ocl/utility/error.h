...
#define ASSERT_CL_SUCCESS(retVal)                                              \
  {                                                                            \
    ...
    if (tempVarForDefine != CL_SUCCESS) {                                      \
      NON_FATAL_ERROR("ASSERT_CL_SUCCESS", #retVal,                            \
                      std::to_string(tempVarForDefine).c_str(),                \
                      oclErrorToString(tempVarForDefine));                     \
...
#define ASSERT_CL_COMPILATION_SUCCESS(retVal)                                  \
  {                                                                            \
    ...
    if (tempVarForDefine != CL_SUCCESS) {                                      \
      size_t numBytes = 0;                                                     \
      clGetProgramBuildInfo(program, opencl.device, CL_PROGRAM_BUILD_LOG, 0,   \
                            NULL, &numBytes);                                  \
...
#define EXPECT_CL_SUCCESS(retVal)                                              \
  {                                                                            \
    ...
    if (tempVarForDefine != CL_SUCCESS) {                                      \
      NON_FATAL_ERROR("EXPECT_CL_SUCCESS", #retVal,                            \
                      std::to_string(tempVarForDefine).c_str(),                \
                      oclErrorToString(tempVarForDefine));                     \
    }                                                                          \
  }
...
#define CL_SUCCESS_OR_ERROR(retVal, message)                                   \
  {                                                                            \
    const auto tempVarForDefine = (retVal);                                    \
    if (tempVarForDefine != CL_SUCCESS) {                                      \
      const auto _message = std::string(message) +                             \
                            " (Fatal OpenCL error occurred, retVal=" +         \
                            std::to_string(tempVarForDefine) + ")";            \
