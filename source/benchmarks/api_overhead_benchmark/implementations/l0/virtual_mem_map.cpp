...
static TestResult prepareVirtualMemoryMaps(
    LevelZero &levelzero, const size_t reserveSize,
  ...
  for (uint32_t i = 0; i < iterationCount; i++) {
    ...
    ASSERT_ZE_RESULT_SUCCESS(zeVirtualMemReserve(levelzero.context, nullptr,
                                                 reserveSize, &virtualMem));
    ...
    ASSERT_ZE_RESULT_SUCCESS(zePhysicalMemCreate(
        levelzero.context, levelzero.device, &physDesc, &physicalMemoryHandle));
...
static TestResult cleanupVirtualMemoryMaps(
    LevelZero &levelzero, std::vector<void *> &reservedMemlist,
    std::vector<ze_physical_mem_handle_t> &physicalMemoryHandleList,
    const size_t reserveSize) {

  for (auto &virtualMem : reservedMemlist) {
    ASSERT_ZE_RESULT_SUCCESS(
        zeVirtualMemUnmap(levelzero.context, virtualMem, reserveSize));
  }

  for (auto &virtualMem : reservedMemlist) {
    ASSERT_ZE_RESULT_SUCCESS(
        zeVirtualMemFree(levelzero.context, virtualMem, reserveSize));
  }

  for (auto &physicalMemoryHandle : physicalMemoryHandleList) {
    ASSERT_ZE_RESULT_SUCCESS(
        zePhysicalMemDestroy(levelzero.context, physicalMemoryHandle));
  }
...
static TestResult run(const VirtualMemMapArguments &arguments,
  ...
  ASSERT_ZE_RESULT_SUCCESS(zeVirtualMemQueryPageSize(
      levelzero.context, levelzero.device, arguments.reserveSize, &pageSize));
  ...
  // Handle access type
  ze_memory_access_attribute_t accessType =
      ZE_MEMORY_ACCESS_ATTRIBUTE_READWRITE;
  std::string accessTypeString =
      static_cast<const std::string &>(arguments.accessType);
  ...
  auto status =
      prepareVirtualMemoryMaps(levelzero, reserveSize, reservedMemlist,
                               physicalMemoryHandleList, warmupIterationCount);
  if (status != TestResult::Success) {
    return status;
  }
  for (auto i = 0u; i < warmupIterationCount; i++) {
    ASSERT_ZE_RESULT_SUCCESS(zeVirtualMemMap(
        levelzero.context, reservedMemlist[i], reserveSize,
        physicalMemoryHandleList[i], alignedOffset, accessType));
  }

  status = cleanupVirtualMemoryMaps(levelzero, reservedMemlist,
                                    physicalMemoryHandleList, reserveSize);
  if (status != TestResult::Success) {
    return status;
  }
  ...
  // Benchmark
  Timer timer;
  status = prepareVirtualMemoryMaps(
      levelzero, reserveSize, reservedMemlist, physicalMemoryHandleList,
      static_cast<uint32_t>(arguments.iterations));
  if (status != TestResult::Success) {
    return status;
  }
...
static RegisterTestCaseImplementation<VirtualMemMap> registerTestCase(run,
                                                                      Api::L0);
