...
struct ImmediateCmdListWalkerSubmission
    : Workload<ImmediateCmdListWalkerSubmissionArguments> {};

TestResult run(const ImmediateCmdListWalkerSubmissionArguments &arguments,
  ...
  ZE_RESULT_SUCCESS_OR_RETURN_ERROR(UsmHelper::allocate(
      UsmMemoryPlacement::Host, levelzero, bufferSize, &hostMemory));
  ZE_RESULT_SUCCESS_OR_RETURN_ERROR(zeContextMakeMemoryResident(
      levelzero.context, levelzero.device, hostMemory, bufferSize));
  ...
  ZE_RESULT_SUCCESS_OR_RETURN_ERROR(
      zeKernelSetArgumentValue(kernel, 0, sizeof(hostMemory), &hostMemory));
  eventDesc.index = 0;
  ZE_RESULT_SUCCESS_OR_RETURN_ERROR(
  ...
  for (auto i = 0u; i < 5; i++) {
    ZE_RESULT_SUCCESS_OR_RETURN_ERROR(zeCommandListAppendLaunchKernel(
        cmdList, kernel, &groupCount, event, 0, nullptr));
    ZE_RESULT_SUCCESS_OR_RETURN_ERROR(
