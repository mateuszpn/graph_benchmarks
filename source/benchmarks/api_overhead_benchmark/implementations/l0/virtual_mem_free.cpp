...
static TestResult doVirtualMemFree(LevelZero &levelzero,
                                   const VirtualMemFreeArguments &arguments,
                                   Timer &timer) {

  size_t pageSize = 0;
  auto reserveSize = arguments.freeSize;
  ASSERT_ZE_RESULT_SUCCESS(zeVirtualMemQueryPageSize(
      levelzero.context, levelzero.device, reserveSize, &pageSize));
...
static TestResult run(const VirtualMemFreeArguments &arguments,
...
static RegisterTestCaseImplementation<VirtualMemFree> registerTestCase(run,
                                                                       Api::L0);
