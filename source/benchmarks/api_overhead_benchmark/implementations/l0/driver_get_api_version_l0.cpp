...
static TestResult run(const DriverGetApiVersionArguments &arguments,
  ...
  ASSERT_ZE_RESULT_SUCCESS(
      zeDriverGetApiVersion(drivers[0], &driverApiVersionWarmUp));
  ...
  for (auto j = 0u; j < arguments.iterations; j++) {
    ...
    timer.measureStart();
    ASSERT_ZE_RESULT_SUCCESS(
        zeDriverGetApiVersion(drivers[0], &driverApiVersion));
    timer.measureEnd();
...
static RegisterTestCaseImplementation<DriverGetApiVersion>
