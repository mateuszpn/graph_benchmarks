...
static TestResult run(const ExecuteCommandListWithIndirectArguments &arguments,
  ...
  for (uint32_t i = 0; i < arguments.IndirectAllocationsAmount; i++) {
    ...
    ASSERT_ZE_RESULT_SUCCESS(
        L0::UsmHelper::allocate(arguments.placement, levelzero, 4096u, &ptr));
  ...
  for (uint32_t i = 0; i < arguments.IndirectAllocationsAmount; i++) {
    ...
    ASSERT_ZE_RESULT_SUCCESS(
        zeMemFree(levelzero.context, indirectAllocations[i]));
  }
...
static RegisterTestCaseImplementation<ExecuteCommandListWithInidrect>
