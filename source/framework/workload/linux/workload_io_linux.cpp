...
public:
  WorkloadIoLinux(int synchronizationPipeIn, int synchronizationPipeOut,
                  int measurementPipe)
  ...
  void writeToMeasurements(const std::string &measurements) override {
    if (measurementPipe) {
      ssize_t numberOfBytesWritten =
          write(measurementPipe, measurements.c_str(), measurements.size());
      FATAL_ERROR_IF_SYS_CALL_FAILED(
          numberOfBytesWritten,
          "Writing measurements in a child process failed");
      FATAL_ERROR_IF(numberOfBytesWritten == 0,
                     "No character was written when writing measurements in a "
                     "child process");
    } else {
  ...
  void writeSynchronizationChar(char c) override {
    if (synchronizationPipeOut) {
      ...
      FATAL_ERROR_IF_SYS_CALL_FAILED(
          numberOfBytesWritten,
          "Writing synchronization char in a child process failed");
      FATAL_ERROR_IF(numberOfBytesWritten == 0,
                     "No character was written when writing synchronization "
                     "char in a child process");
    } else {
      std::cout << c;
    }
  }
  ...
  char readSynchronizationChar() override {
    if (synchronizationPipeIn) {
      ...
      FATAL_ERROR_IF_SYS_CALL_FAILED(
          numberOfBytesRead,
          "Reading synchronization char in a child process failed");
      FATAL_ERROR_IF(numberOfBytesRead == 0,
                     "No character was read when reading synchronization char "
                     "in a child process");
      FATAL_ERROR_IF(buffer !=
                         ProcessSynchronizationHelper::synchronizationChar,
                     "Invalid synchronization received in a child process from "
                     "the parent: '",
                     buffer, "'");
      return buffer;
    } else {
...
std::unique_ptr<WorkloadIo>
WorkloadIo::create(const WorkloadArgumentContainer &arguments) {
  return std::unique_ptr<WorkloadIo>(new WorkloadIoLinux(
      arguments.synchronizationPipeIn, arguments.synchronizationPipeOut,
      arguments.measurementPipe));
}
