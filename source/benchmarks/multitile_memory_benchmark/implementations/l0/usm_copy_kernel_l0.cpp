...
static TestResult run(const UsmCopyKernelArguments &arguments,
  ...
  const uint64_t timerResolution =
      levelzero.getTimerResolution(arguments.queuePlacement);

  // Create buffers
  void *src{}, *dst{};
  ASSERT_ZE_RESULT_SUCCESS(UsmHelper::allocate(
      arguments.srcPlacement, levelzero, arguments.size, &src));
  ASSERT_ZE_RESULT_SUCCESS(UsmHelper::allocate(
      arguments.dstPlacement, levelzero, arguments.size, &dst));

  // Create kernel
  const auto kernelBinary =
      FileHelper::loadBinaryFile("multitile_memory_benchmark_copy_buffer.spv");
  ...
  ASSERT_ZE_RESULT_SUCCESS(zeModuleCreate(levelzero.context,
                                          levelzero.commandQueueDevice,
  ...
  const uint32_t elementsCount =
      static_cast<uint32_t>(arguments.size) / sizeof(int);
  ASSERT_ZE_RESULT_SUCCESS(zeKernelSuggestGroupSize(kernel, elementsCount, 1u,
                                                    1u, &wgsX, &wgsY, &wgsZ));
  ...
  ASSERT_ZE_RESULT_SUCCESS(
      zeKernelSetArgumentValue(kernel, 0, sizeof(src), &src));
  ASSERT_ZE_RESULT_SUCCESS(
      zeKernelSetArgumentValue(kernel, 1, sizeof(dst), &dst));
  ...
  for (DeviceSelection device :
       DeviceSelectionHelper::split(DeviceSelectionHelper::withoutHost(
           arguments.queuePlacement | arguments.srcPlacement))) {
    ASSERT_ZE_RESULT_SUCCESS(zeContextMakeMemoryResident(
        levelzero.context, levelzero.getDevice(device), src, arguments.size));
  }
  for (DeviceSelection device :
       DeviceSelectionHelper::split(DeviceSelectionHelper::withoutHost(
           arguments.queuePlacement | arguments.dstPlacement))) {
    ASSERT_ZE_RESULT_SUCCESS(zeContextMakeMemoryResident(
        levelzero.context, levelzero.getDevice(device), dst, arguments.size));
  }
  ...
  ASSERT_ZE_RESULT_SUCCESS(zeCommandListCreate(
      levelzero.context, levelzero.commandQueueDevice, &cmdListDesc, &cmdList));
  ...
  for (auto i = 0u; i < arguments.iterations; i++) {
    ...
    if (arguments.useEvents) {
      ...
      statistics.pushValue(commandTime, arguments.size, typeSelector.getUnit(),
...
static RegisterTestCaseImplementation<UsmCopyKernel> registerTestCase(run,
                                                                      Api::L0);
