...
UrState::UrState() {
  ...
  EXPECT_UR_RESULT_SUCCESS(
      urPlatformGet(&adapter, adapter_count, 0, nullptr, &platform_count));

  std::vector<ur_platform_handle_t> platforms(platform_count);
  EXPECT_UR_RESULT_SUCCESS(urPlatformGet(
      &adapter, adapter_count, platform_count, platforms.data(), nullptr));

  if (Configuration::get().urPlatformIndex >= platform_count) {
    FATAL_ERROR("Invalid UR platform index. platformIndex=",
                Configuration::get().urPlatformIndex,
  ...
  EXPECT_UR_RESULT_SUCCESS(urDeviceGet(platform, UR_DEVICE_TYPE_ALL,
  ...
  if (Configuration::get().urDeviceIndex >= device_count) {
    FATAL_ERROR("Invalid UR device index. deviceIndex=",
                Configuration::get().urDeviceIndex,
