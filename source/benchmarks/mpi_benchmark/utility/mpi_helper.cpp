...
std::vector<std::chrono::nanoseconds> runLauncher(const std::string &launcher,
                                                  const int nRanks,
  ...
  const std::string mpiCommand = launcher + " -n " + std::to_string(nRanks) +
                                 ' ' + workingDir + binary + ' ' + args;
    ...
    for (auto &line : output) {
      ...
      if (pos == 0) {
        measurements.emplace_back(
            std::stoull(line.substr(line.find_last_of("**Measurement:") + 1)));
