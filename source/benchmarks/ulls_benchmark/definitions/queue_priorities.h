...
struct QueuePrioritiesArguments : TestCaseArgumentContainer {
  ...
  QueuePrioritiesArguments()
      : lowPriorityKernelTime(
            *this, "lowTime",
            "How long each work item is in low priority kernel"),
        usePriorities(*this, "priorities",
                      "Low priority command queue property is used"),
        workgroupCount(*this, "wgc", "Workgroup count of high priority kernel"),
        highPriorityKernelTime(
            *this, "highTime",
            "How long each work item is in high priority kernel"),
        sleepTime(*this, "sleep",
                  "sleep time in us after low priority kernel flushed") {}
};

struct QueuePriorities : TestCase<QueuePrioritiesArguments> {
  using TestCase<QueuePrioritiesArguments>::TestCase;

  std::string getTestCaseName() const override { return "QueuePriorities"; }

  std::string getHelp() const override {
    return "Uses queues with different priorities to meassure submission and "
