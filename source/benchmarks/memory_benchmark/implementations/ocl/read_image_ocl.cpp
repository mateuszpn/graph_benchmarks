...
static TestResult run(const ReadImageArguments &arguments,
  ...
  imageDescription.image_type =
      ImageHelperOcl::getOclImageTypeFromDimensions(arguments.size);
  ...
  cl_mem image = clCreateImage(opencl.context, 0, &imageFormat,
                               &imageDescription, nullptr, &retVal);
  ASSERT_CL_SUCCESS(retVal);
  const auto imageSizeInBytes = ImageHelperOcl::getImageSizeInBytes(
      channelOrder, channelFormat, arguments.size);
  // Create hostptr
  HostptrReuseHelper::Alloc hostptrAlloc{};
  ASSERT_CL_SUCCESS(HostptrReuseHelper::allocateBufferHostptr(
      opencl, arguments.hostPtrPlacement, imageSizeInBytes, hostptrAlloc));

  // Warmup
  const size_t origin[] = {0, 0, 0};
  const size_t *region = arguments.size;
  ASSERT_CL_SUCCESS(clEnqueueReadImage(opencl.commandQueue, image, true, origin,
                                       region, 0, 0, hostptrAlloc.ptr, 0,
                                       nullptr, nullptr));
  ...
  for (auto i = 0u; i < arguments.iterations; i++) {
    ...
    timer.measureStart();
    ASSERT_CL_SUCCESS(clEnqueueReadImage(opencl.commandQueue, image, true,
                                         origin, region, 0, 0, hostptrAlloc.ptr,
...
static RegisterTestCaseImplementation<ReadImage> registerTestCase(run,
                                                                  Api::OpenCL);
