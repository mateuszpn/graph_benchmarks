...
static TestResult run(const ReductionArguments &arguments,
  ...
  ASSERT_CL_SUCCESS(clEnqueueReadBuffer(opencl.commandQueue, buffer, true, 0u,
                                        4u, &actualSum, 0u, nullptr, nullptr));
  ...
  for (auto i = 0u; i < arguments.iterations; i++) {
    int zero = 0u;
    ASSERT_CL_SUCCESS(clEnqueueWriteBuffer(opencl.commandQueue, buffer, true,
                                           0u, 4u, &zero, 0u, nullptr,
                                           nullptr));
    ASSERT_CL_SUCCESS(clEnqueueNDRangeKernel(opencl.commandQueue, kernel, 1,
                                             nullptr, &gws, nullptr, 0, nullptr,
                                             &profilingEvent));
...
static RegisterTestCaseImplementation<Reduction> registerTestCase(run,
                                                                  Api::OpenCL);
