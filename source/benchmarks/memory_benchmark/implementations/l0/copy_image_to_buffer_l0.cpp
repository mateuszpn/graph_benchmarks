...
static TestResult run(const CopyImageToBufferArguments &arguments,
  ...
  if (!ImageHelperL0::validateImageDimensions(levelzero.device,
                                              arguments.region)) {
    return TestResult::DeviceNotCapable;
  }
  Timer timer;
  ...
  imageDesc.type =
      ImageHelperL0::getL0ImageTypeFromDimensions(arguments.region);
  ...
  ASSERT_ZE_RESULT_SUCCESS(zeImageCreate(levelzero.context, levelzero.device,
                                         &imageDesc, &srcImage));
  void *destination{};
  ASSERT_ZE_RESULT_SUCCESS(UsmHelper::allocate(
      arguments.destinationPlacement, levelzero, arguments.size, &destination));
  const ze_image_region_t reg = {0u,
                                 0u,
                                 0u,
                                 (uint32_t)arguments.region[0],
                                 (uint32_t)arguments.region[1],
                                 (uint32_t)arguments.region[2]};
  const size_t imageSizeInBytes = ImageHelperL0::getImageSizeInBytes(
  ...
  ASSERT_ZE_RESULT_SUCCESS(zeCommandListAppendImageCopyToMemory(
      cmdList, destination, srcImage, &reg, event, 0, nullptr));
