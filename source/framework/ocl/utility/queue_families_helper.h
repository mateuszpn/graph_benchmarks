...
public:
  ...
  static inline std::unique_ptr<PropertiesForSelectingQueue>
  getPropertiesForSelectingEngine(cl_device_id device, Engine engine) {
    ...
    const EngineGroup engineGroup =
        EngineHelper::getEngineGroup(engine, copyEnginesCount);
    const size_t engineIndex =
        EngineHelper::getEngineIndexWithinGroup(engine, copyEnginesCount);
  ...
  static size_t getQueueCountForEngineGroup(cl_device_id clDevice,
  ...
  static bool
  validateCapability(cl_command_queue queue,
                     cl_command_queue_capabilities_intel capability) {
  ...
  static bool validateCapabilities(cl_command_queue queue, Args &&...args) {
    return validateCapabilities(getQueueCapabilities(queue),
...
private:
  ...
  static inline std::vector<QueueFamilyDesc>
  queryQueueFamilies(cl_device_id device) {
    // Get families count
    size_t familyPropertiesSize{};
    cl_int retVal =
        clGetDeviceInfo(device, CL_DEVICE_QUEUE_FAMILY_PROPERTIES_INTEL, 0,
                        nullptr, &familyPropertiesSize);
    if (retVal != CL_SUCCESS) {
      return {};
    }
    size_t familiesCount =
        familyPropertiesSize / sizeof(cl_queue_family_properties_intel);
    if (familiesCount == 0 ||
        familiesCount * sizeof(cl_queue_family_properties_intel) !=
            familyPropertiesSize) {
      return {};
    }

    // Get families
    auto families =
        std::make_unique<cl_queue_family_properties_intel[]>(familiesCount);
    retVal = clGetDeviceInfo(device, CL_DEVICE_QUEUE_FAMILY_PROPERTIES_INTEL,
                             familyPropertiesSize, families.get(), nullptr);
    if (retVal != CL_SUCCESS) {
      return {};
    }
  ...
  getCopyEnginesCount(const std::vector<QueueFamilyDesc> &families) {
  ...
  static cl_command_queue_capabilities_intel
  getQueueCapabilities(cl_command_queue queue) {
    cl_uint familyIndex = {};
    cl_int retVal =
        clGetCommandQueueInfo(queue, CL_QUEUE_FAMILY_INTEL, sizeof(familyIndex),
                              &familyIndex, nullptr);
    if (retVal != CL_SUCCESS) {
      return CL_QUEUE_DEFAULT_CAPABILITIES_INTEL;
    }
    cl_device_id device = {};
    EXPECT_CL_SUCCESS(clGetCommandQueueInfo(queue, CL_QUEUE_DEVICE,
  ...
  static bool
  validateCapability(cl_command_queue_capabilities_intel queueCapabilities,
                     cl_command_queue_capabilities_intel capability) {
    return queueCapabilities == CL_QUEUE_DEFAULT_CAPABILITIES_INTEL ||
           ((queueCapabilities & capability) == capability);
  }

  template <typename... Args>
  static bool
  validateCapabilities(cl_command_queue_capabilities_intel queueCapabilities,
                       cl_command_queue_capabilities_intel capability,
                       Args &&...args) {
    ...
    if constexpr (sizeof...(Args) > 0) {
      return validateCapabilities(queueCapabilities,
                                  std::forward<Args>(args)...);
    } else {
      return true;
    }
