...
[[maybe_unused]] static const inline RegisterTestCase<KernelAndCopy>
    registerTestCase{};

class KernelAndCopyTest
...
INSTANTIATE_TEST_SUITE_P(
    KernelAndCopyTest, KernelAndCopyTest,
    ::testing::Values(
        // Without copy queue
        ...
        std::make_tuple(true, true, true,
                        false), // kernel+copy on different queues

        // With copy queue
        std::make_tuple(false, false, true, true), // only copy
        std::make_tuple(true, true, true,
                        true) // kernel+copy on different queues
        ));
