...
public:
  PowerMeter() {
    if (Configuration::get().measurePower) {
      ...
      if (!this->isEnabled()) {
        fprintf(stderr, "Requested to measure power but could not obtain power "
                        "handle from sysman API. Power will not be measured.");
      }
    }
  }
  ...
  uint64_t getEnergy() { return energyEnd.energy - energyStart.energy; }

  std::chrono::microseconds getTime() {
    return std::chrono::microseconds(getRawTime());
  }

  double getPower() { return (this->getEnergy() * 1.0) / this->getRawTime(); }
...
protected:
  ...
  zes_pwr_handle_t getPowerHandle() {
    ...
    pCount = 1;
    if (ZE_RESULT_SUCCESS != zesDriverGet(&pCount, &sysmanDriverHandle) ||
        !sysmanDriverHandle) {
      return nullptr;
    }
    ...
    if (ZE_RESULT_SUCCESS !=
            zesDeviceGet(sysmanDriverHandle, &pCount, &sysmanDeviceHandle) ||
        !sysmanDeviceHandle) {
      return nullptr;
    }

    if (ZE_RESULT_SUCCESS !=
            zesDeviceEnumPowerDomains(sysmanDeviceHandle, &pCount, nullptr) ||
        !pCount) {
      return nullptr;
    }
    ...
    if (ZE_RESULT_SUCCESS != zesDeviceEnumPowerDomains(sysmanDeviceHandle,
                                                       &pCount,
                                                       &powerHandleReturned)) {
      return nullptr;
    }
