...
struct SlmTrafficArguments : TestCaseArgumentContainer {
  ...
  SlmTrafficArguments()
      : size(*this, "size", "SLM Size"),
        occupancyDivider(*this, "occupancyDiv",
                         "H/W load divider by 8, 4, 2, full occupancy"),
...
struct SlmTraffic : TestCase<SlmTrafficArguments> {
  ...
  std::string getHelp() const override {
    return "generates SLM local memory transactions inside thread group to "
           "measure latency between reads (uses Intel only private "
