...
static TestResult run(const CopyWithEventArguments &arguments,
  ...
  ASSERT_ZE_RESULT_SUCCESS(zeMemAllocShared(levelzero.context, &deviceDesc,
                                            &hostDesc, allocSize, 1,
                                            levelzero.device, &srcBuffer));

  void *dstBuffer = nullptr;
  ASSERT_ZE_RESULT_SUCCESS(zeMemAllocShared(levelzero.context, &deviceDesc,
                                            &hostDesc, allocSize, 1,
                                            levelzero.device, &dstBuffer));
  ...
  for (auto commandIndex = 0u; commandIndex < arguments.measuredCommands;
    ...
    ASSERT_ZE_RESULT_SUCCESS(zeCommandListAppendMemoryCopy(
        cmdList, dstBuffer, srcBuffer, allocSize, event, 0, nullptr));
  }
...
static RegisterTestCaseImplementation<CopyWithEvent> registerTestCase(run,
                                                                      Api::L0);
