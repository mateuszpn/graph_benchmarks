...
static TestResult
run(const ExecuteCommandListImmediateMultiKernelArguments &arguments,
  ...
  auto spirvModule1 =
      FileHelper::loadBinaryFile("api_overhead_benchmark_write_one.spv");
  ...
  moduleDescs[0].pInputModule =
      reinterpret_cast<const uint8_t *>(spirvModule0.data());
  moduleDescs[0].inputSize = spirvModule0.size();
  ASSERT_ZE_RESULT_SUCCESS(zeModuleCreate(levelzero.context, levelzero.device,
                                          &moduleDescs[0], &modules[0],
                                          nullptr));
  kernelDescs[0].pKernelName = "eat_time";
  ASSERT_ZE_RESULT_SUCCESS(
      zeKernelCreate(modules[0], &kernelDescs[0], &kernels[0]));

  moduleDescs[1].format = ZE_MODULE_FORMAT_IL_SPIRV;
  moduleDescs[1].pInputModule =
      reinterpret_cast<const uint8_t *>(spirvModule1.data());
  moduleDescs[1].inputSize = spirvModule1.size();
  ASSERT_ZE_RESULT_SUCCESS(zeModuleCreate(levelzero.context, levelzero.device,
                                          &moduleDescs[1], &modules[1],
                                          nullptr));
  kernelDescs[1].pKernelName = "write_one";
  ASSERT_ZE_RESULT_SUCCESS(
      zeKernelCreate(modules[1], &kernelDescs[1], &kernels[1]));

  // Create event
  uint32_t numEventsMultiplier =
      arguments.addBarrier
          ? static_cast<uint32_t>(arguments.numKernelsAfterBarrier + 1)
          : 2u;
  uint32_t eventsCount =
      static_cast<uint32_t>(arguments.amountOfCalls) * numEventsMultiplier;
  ...
  ZE_RESULT_SUCCESS_OR_ERROR(zeEventPoolCreate(
      levelzero.context, &eventPoolDesc, 1, &levelzero.device, &eventPool));
  ...
  ASSERT_ZE_RESULT_SUCCESS(
      zeMemAllocDevice(levelzero.context, &deviceAllocationDesc,
                       outputBufferSize, 0, levelzero.device, &outputBuffer));

  // Configure kernel
  ASSERT_ZE_RESULT_SUCCESS(zeKernelSetGroupSize(kernels[0], 1u, 1u, 1u));
  int kernelOperationsCount =
      static_cast<int>(arguments.kernelExecutionTime * 4);
  ASSERT_ZE_RESULT_SUCCESS(zeKernelSetArgumentValue(kernels[0], 0, sizeof(int),
                                                    &kernelOperationsCount));
  ...
  ASSERT_ZE_RESULT_SUCCESS(zeKernelSetArgumentValue(
      kernels[1], 0, sizeof(outputBuffer), &outputBuffer));
  ...
  ASSERT_ZE_RESULT_SUCCESS(zeCommandListAppendLaunchKernel(
      cmdList, kernels[0], &groupCount0, events[0], 0, nullptr));
  ASSERT_ZE_RESULT_SUCCESS(zeCommandListAppendLaunchKernel(
      cmdList, kernels[1], &groupCount0, events[1], 0, nullptr));
  ASSERT_ZE_RESULT_SUCCESS(
      zeEventHostSynchronize(events[0], std::numeric_limits<uint64_t>::max()));
  ASSERT_ZE_RESULT_SUCCESS(
      zeEventHostSynchronize(events[1], std::numeric_limits<uint64_t>::max()));
  ...
  for (auto i = 0u; i < arguments.iterations; i++) {
    ...
    for (uint32_t callId = 0u; callId < arguments.amountOfCalls; callId++) {
      if (arguments.addBarrier) {
        uint32_t numKernelsToAdd =
            (((arguments.numKernelsBeforeBarrier) % 2) == 0)
                ? static_cast<uint32_t>(arguments.numKernelsBeforeBarrier)
                : static_cast<uint32_t>(arguments.numKernelsBeforeBarrier) + 1;
        for (uint32_t kernelId = 0; kernelId < numKernelsToAdd; kernelId++) {
          ASSERT_ZE_RESULT_SUCCESS(zeCommandListAppendLaunchKernel(
              cmdList, kernels[kernelId % 2], &groupCount0, nullptr, 0,
              nullptr));
        }
        ...
        numKernelsToAdd =
            (((arguments.numKernelsAfterBarrier) % 2) == 0)
                ? static_cast<uint32_t>(arguments.numKernelsAfterBarrier)
                : static_cast<uint32_t>(arguments.numKernelsAfterBarrier + 1);
        for (uint32_t kernelId = 0; kernelId < numKernelsToAdd; kernelId++) {
          ASSERT_ZE_RESULT_SUCCESS(zeCommandListAppendLaunchKernel(
              cmdList, kernels[kernelId % 2], &groupCount0, events[eventId++],
              0, nullptr));
        }
      } else {
        ASSERT_ZE_RESULT_SUCCESS(zeCommandListAppendLaunchKernel(
            cmdList, kernels[0], &groupCount0,
            arguments.addBarrier ? nullptr : events[eventId++], 0, nullptr));
        ASSERT_ZE_RESULT_SUCCESS(zeCommandListAppendLaunchKernel(
            cmdList, kernels[1], &groupCount0,
            arguments.addBarrier ? nullptr : events[eventId++], 0, nullptr));
      }
    }
    timer.measureEnd();
    ...
    for (auto eventUsed = 0u; eventUsed < eventId; eventUsed++) {
      ASSERT_ZE_RESULT_SUCCESS(zeEventHostSynchronize(
          events[eventUsed], std::numeric_limits<uint64_t>::max()));
...
static RegisterTestCaseImplementation<ExecuteCommandListImmediateMultiKernel>
