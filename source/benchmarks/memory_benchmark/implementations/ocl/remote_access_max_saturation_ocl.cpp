...
static TestResult run(const RemoteAccessMaxSaturationArguments &arguments,
  ...
  const uint32_t scalarValue =
      static_cast<uint32_t>(arguments.workItemPackSize);
  ...
  ASSERT_CL_SUCCESS(clEnqueueFillBuffer(opencl.commandQueue, buffer, &fillValue,
                                        ...
                                        nullptr, nullptr));
  ASSERT_CL_SUCCESS(
      clSetKernelArg(kernel, static_cast<cl_uint>(0), sizeof(buffer), &buffer))
  ASSERT_CL_SUCCESS(clSetKernelArg(kernel, static_cast<cl_uint>(1),
                                   sizeof(cl_uint), &scalarValue));
  ASSERT_CL_SUCCESS(
      clSetKernelArg(kernel, static_cast<cl_uint>(2), elementSize, &n_th));
  ASSERT_CL_SUCCESS(clSetKernelArg(kernel, static_cast<cl_uint>(3),
                                   sizeof(cl_uint), &writesPerWorkgroup));
...
static RegisterTestCaseImplementation<RemoteAccessMaxSaturation>
