...
[[maybe_unused]] static const inline RegisterTestCase<WriteBuffer>
    registerTestCase{};

class WriteBufferTest : public ::testing::TestWithParam<
                            std::tuple<DeviceSelection, DeviceSelection,
                                       DeviceSelection, size_t, bool, bool>> {};
...
TEST_P(WriteBufferTest, Test) {
  ...
  if (!args.validateArgumentsExtra()) {
    GTEST_SKIP(); // If above arguments make no sense (e.g. queue created
                  // outside of the context), skip the case
  }
...
INSTANTIATE_TEST_SUITE_P(
    WriteBufferTest, WriteBufferTest,
    ::testing::Combine(::CommonGtestArgs::contextDeviceSelections(),
