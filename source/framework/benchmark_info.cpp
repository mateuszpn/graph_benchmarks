...
BenchmarkInfo &BenchmarkInfo::get() {
  FATAL_ERROR_IF(
      instance == nullptr,
      "BenchmarkInfo::get() called before BenchmarkInfo::initialize()");
  return *instance;
}

void BenchmarkInfo::initialize(const std::string &name,
                               const std::string &description,
                               int testCaseColumnWidth) {
  FATAL_ERROR_IF(instance != nullptr,
                 "BenchmarkInfo::initialize() called multiple times");
...
std::string BenchmarkInfo::getBenchmarkName() const { return name; }
