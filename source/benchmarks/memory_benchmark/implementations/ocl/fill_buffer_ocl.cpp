...
static TestResult run(const FillBufferArguments &arguments,
  ...
  if (!QueueFamiliesHelper::validateCapability(
          opencl.commandQueue, CL_QUEUE_CAPABILITY_FILL_BUFFER_INTEL)) {
    return TestResult::DeviceNotCapable;
  }
  Timer timer;
  cl_int retVal;
  ...
  // Warmup
  ASSERT_CL_SUCCESS(clEnqueueFillBuffer(opencl.commandQueue, buffer,
                                        pattern.get(), arguments.patternSize, 0,
  ...
  for (auto i = 0u; i < arguments.iterations; i++) {
    ...
    ASSERT_CL_SUCCESS(clEnqueueFillBuffer(
        opencl.commandQueue, buffer, pattern.get(), arguments.patternSize, 0,
...
static RegisterTestCaseImplementation<FillBuffer> registerTestCase(run,
                                                                   Api::OpenCL);
