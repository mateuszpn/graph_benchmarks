...
static TestResult run(const ResetCommandListArguments &arguments,
  ...
  QueueProperties queueProperties = QueueProperties::create()
                                        .setForceBlitter(arguments.copyOnly)
                                        .allowCreationFail();
  ...
  if (isUsmMemoryType(arguments.sourcePlacement)) {
    ASSERT_ZE_RESULT_SUCCESS(BufferContentsHelperL0::fillBuffer(
        levelzero, source, arguments.size, BufferContents::Zeros, false));
  }
  ASSERT_ZE_RESULT_SUCCESS(UsmHelper::allocate(
      UsmMemoryPlacement::Device, levelzero, arguments.size, &destination));
  ...
  ASSERT_ZE_RESULT_SUCCESS(zeCommandListCreate(
      levelzero.context, levelzero.device, &commandListDesc, &commandList));

  // Warmup
  ASSERT_ZE_RESULT_SUCCESS(zeCommandListAppendMemoryCopy(
      commandList, destination, source, arguments.size, nullptr, 0, nullptr));
  ASSERT_ZE_RESULT_SUCCESS(zeCommandListClose(commandList));
  ASSERT_ZE_RESULT_SUCCESS(zeCommandQueueExecuteCommandLists(
      levelzero.commandQueue, 1, &commandList, nullptr));
...
static RegisterTestCaseImplementation<ResetCommandList>
