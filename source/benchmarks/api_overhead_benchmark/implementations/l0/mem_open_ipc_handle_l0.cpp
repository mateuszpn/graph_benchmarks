...
static TestResult run(const MemOpenIpcHandleArguments &arguments,
    ...
    for (int64_t index = 0; index < arguments.AllocationsCount; index++) {
      ...
      if (index & 1) {
        ASSERT_ZE_RESULT_SUCCESS(UsmHelper::deallocate(
            arguments.sourcePlacement, levelzero, allocations[index]));
        auto bufferSize = sizeof(uint64_t);
        void *ptr = nullptr;
        ASSERT_ZE_RESULT_SUCCESS(UsmHelper::allocate(
            arguments.sourcePlacement, levelzero, bufferSize, &ptr));
  ...
  for (int64_t i = 0; i < arguments.AllocationsCount; i++) {
    std::fill_n(ipcHandles[i].data, ZE_MAX_IPC_HANDLE_SIZE,
                static_cast<char>(0));
    ASSERT_ZE_RESULT_SUCCESS(
        zeMemGetIpcHandle(levelzero.context, allocations[i], &ipcHandles[i]));
    ASSERT_ZE_RESULT_SUCCESS(zeMemOpenIpcHandle(levelzero.context,
                                                levelzero.device, ipcHandles[i],
                                                0, &ipcPointers[i]));
    EXPECT_NE(ipcPointers[i], nullptr);
  }
  for (int64_t i = 0; i < arguments.AllocationsCount; i++) {
    ASSERT_ZE_RESULT_SUCCESS(
        zeMemPutIpcHandle(levelzero.context, ipcHandles[i]));
  }

  // Benchmark
  for (auto i = 0u; i < arguments.iterations; i++) {
    for (int64_t j = 0; j < arguments.AllocationsCount; ++j) {
      std::fill_n(ipcHandles[j].data, ZE_MAX_IPC_HANDLE_SIZE,
                  static_cast<char>(0));
      ASSERT_ZE_RESULT_SUCCESS(
          zeMemGetIpcHandle(levelzero.context, allocations[j], &ipcHandles[j]));
    }

    timer.measureStart();
    for (int64_t j = 0; j < arguments.AllocationsCount; j++) {
      ASSERT_ZE_RESULT_SUCCESS(
          zeMemOpenIpcHandle(levelzero.context, levelzero.device, ipcHandles[j],
                             0, &ipcPointers[j]));
    }
    timer.measureEnd();
    ...
    for (int64_t j = 0; j < arguments.AllocationsCount; j++) {
      ASSERT_ZE_RESULT_SUCCESS(
          zeMemPutIpcHandle(levelzero.context, ipcHandles[j]));
  ...
  // Cleanup
  for (int64_t i = 0; i < arguments.AllocationsCount; i++) {
    ASSERT_ZE_RESULT_SUCCESS(UsmHelper::deallocate(arguments.sourcePlacement,
                                                   levelzero, allocations[i]));
  }

  return TestResult::Success;
}

static RegisterTestCaseImplementation<MemOpenIpcHandle>
