...
static TestResult run(const MultiArgumentKernelTimeArguments &arguments,
  ...
  const std::vector<uint8_t> kernelSource =
      FileHelper::loadTextFile("api_overhead_benchmark_multi_arg_kernel.cl");
  ...
  std::string kernelName =
      "kernelWith" + std::to_string(arguments.argumentCount);
  ...
  for (auto argumentId = 0u; argumentId < arguments.argumentCount;
       argumentId++) {
  ...
  ASSERT_CL_SUCCESS(clEnqueueNDRangeKernel(
      opencl.commandQueue, kernel, 3, nullptr, gws, lws, 0, nullptr, nullptr));
  ...
  for (auto i = 0u; i < arguments.iterations; i++) {
    ...
    for (auto argumentId = 0u; argumentId < arguments.argumentCount;
         argumentId++) {
      ASSERT_CL_SUCCESS(clSetKernelArgSVMPointer(kernel, argumentId,
                                                 allocations[argumentId].ptr));
    }
...
static RegisterTestCaseImplementation<MultiArgumentKernelTime>
