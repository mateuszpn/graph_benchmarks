...
struct BestWalkerSubmissionImmediate
    : TestCase<BestWalkerSubmissionImmediateArguments> {
  ...
  std::string getHelp() const override {
    return "enqueues kernel, which updates system memory location and then "
           "busy-loops on CPU "
           "until the update becomes visible. Kernel is enqueued using "
