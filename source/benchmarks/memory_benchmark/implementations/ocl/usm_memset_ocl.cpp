...
static TestResult run(const UsmMemsetArguments &arguments,
  ...
  Timer timer;
  auto clEnqueueMemsetINTEL =
      (pfn_clEnqueueMemsetINTEL)clGetExtensionFunctionAddressForPlatform(
          opencl.platform, "clEnqueueMemsetINTEL");
  ...
  ASSERT_CL_SUCCESS(UsmHelperOcl::allocate(opencl, arguments.usmMemoryPlacement,
                                           arguments.bufferSize, dstAlloc));

  // Warmup
  const uint8_t memsetValue = 0x1;
  ASSERT_CL_SUCCESS(clEnqueueMemsetINTEL(opencl.commandQueue, dstAlloc.ptr,
                                         memsetValue, arguments.bufferSize, 0,
                                         nullptr, nullptr));
  ...
  for (auto i = 0u; i < arguments.iterations; i++) {
    ASSERT_CL_SUCCESS(BufferContentsHelperOcl::fillUsmBufferOrHostPtr(
        opencl.commandQueue, dstAlloc.ptr, arguments.bufferSize,
        dstAlloc.placement, arguments.contents));
    ...
    if (eventForEnqueue) {
      ...
      statistics.pushValue(std::chrono::nanoseconds(timeNs),
                           arguments.bufferSize, typeSelector.getUnit(),
...
static RegisterTestCaseImplementation<UsmMemset>
