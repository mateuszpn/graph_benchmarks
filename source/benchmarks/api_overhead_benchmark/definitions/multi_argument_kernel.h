...
struct MultiArgumentKernelTimeArguments : TestCaseArgumentContainer {
  ...
  MultiArgumentKernelTimeArguments()
      : argumentCount(
            *this, "argumentCount",
            "argument count in a kernel, supported values 1,4,8,16,32,64"),
        measureSetKernelArg(
            *this, "measureSetArg",
            "control whether setKernelArgSvmPointer is measured or not") {}
};
...
struct MultiArgumentKernelTime : TestCase<MultiArgumentKernelTimeArguments> {
  ...
  std::string getHelp() const override {
    return "measures time spent in clEnqueueNDRangeKernel on CPU for kernels "
           "with mulitple arguments.";
  }
};
