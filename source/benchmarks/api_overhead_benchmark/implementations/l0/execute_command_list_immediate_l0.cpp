...
static TestResult run(const ExecuteCommandListImmediateArguments &arguments,
  ...
  ASSERT_ZE_RESULT_SUCCESS(zeCommandListAppendLaunchKernel(
      cmdList, kernel, &groupCount,
      arguments.useEventForHostSync ? event : nullptr, 0, nullptr));
  ...
  for (auto i = 0u; i < arguments.iterations; i++) {
    timer.measureStart();
    auto limit = arguments.useBarrierSynchronization
                     ? arguments.amountOfCalls
                     : arguments.amountOfCalls - 1;
    ...
    if (!arguments.useBarrierSynchronization) {
      ...
      ASSERT_ZE_RESULT_SUCCESS(zeCommandListAppendBarrier(
          cmdList, arguments.useEventForHostSync ? event : nullptr, 0u,
          nullptr));
    }
...
static RegisterTestCaseImplementation<ExecuteCommandListImmediate>
