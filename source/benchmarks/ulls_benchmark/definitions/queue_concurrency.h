  ...
  QueueConcurrencyArguments()
      : kernelTime(*this, "kernelTime", "How long each work item is in kernel"),
        workgroupCount(*this, "workgroupCount",
                       "Workgroup Count of each kernel"),
...
struct QueueConcurrency : TestCase<QueueConcurrencyArguments> {
  ...
  std::string getTestCaseName() const override { return "QueueConcurrency"; }

  std::string getHelp() const override {
    return "Submits multiple kernels to out of order queue returning events. "
           "There is a sequence of long kernel, short kernel, wait for short "
           "kernel "
