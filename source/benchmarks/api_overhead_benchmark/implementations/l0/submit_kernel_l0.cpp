...
static TestResult run(const SubmitKernelArguments &arguments,
  ...
  eventPoolDesc.flags |=
      arguments.useProfiling ? ZE_EVENT_POOL_FLAG_KERNEL_TIMESTAMP : 0;
  eventPoolDesc.count = static_cast<uint32_t>(
      arguments.numKernels); // ensures one unique event per kernel
  ...
  for (auto iteration = 0u; iteration < arguments.numKernels; iteration++) {
    // Note: this test calls zeKernelSetArgumentValue and zeKernelSetGroupSize
    // each time to be closer to the SYCL behavior!
    ASSERT_ZE_RESULT_SUCCESS(zeKernelSetArgumentValue(kernel, 0, sizeof(int),
    ...
    if (!arguments.discardEvents) {
      ...
      ASSERT_ZE_RESULT_SUCCESS(
          zeEventCreate(eventPool, &eventDesc, &events[iteration]));
      signalEvent = events[iteration];
    }
    ASSERT_ZE_RESULT_SUCCESS(zeCommandListAppendLaunchKernel(
        cmdList, kernel, &groupCount, signalEvent, 0, nullptr));
  }
  ...
  for (auto i = 0u; i < arguments.iterations; i++) {
    ...
    for (auto iteration = 0u; iteration < arguments.numKernels; iteration++) {
      ...
      ASSERT_ZE_RESULT_SUCCESS(zeKernelSetArgumentValue(
          kernel, 0, sizeof(int), &kernelOperationsCount));
      ASSERT_ZE_RESULT_SUCCESS(zeKernelSetGroupSize(kernel, 1u, 1u, 1u));
      // This isn't exactly what SYCL does, but it is a reasonable
      // approximation.
      ze_event_handle_t signalEvent = nullptr;
      if (!arguments.discardEvents) {
        eventDesc.index = iteration;
