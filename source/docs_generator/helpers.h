...
void assignAndValidate(std::string &destination, const std::string &source,
                       const char *message) {
  if (destination.empty()) {
    destination = source;
  } else {
    FATAL_ERROR_IF(destination != source, "String mismatch detected during ",
                   message);
  }
}
...
std::pair<std::string, std::string> splitToTwoTokens(const std::string &line) {
  ...
  FATAL_ERROR_IF(token1.find(';') != std::string::npos,
                 "Too many ';' delimeters in line");
