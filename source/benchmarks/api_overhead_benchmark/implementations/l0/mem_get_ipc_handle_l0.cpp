...
static TestResult run(const MemGetIpcHandleArguments &arguments,
  ...
  for (auto i = 0u; i < arguments.iterations; i++) {
    ...
    for (int64_t j = 0; j < arguments.AllocationsCount; ++j) {
      ...
      ASSERT_ZE_RESULT_SUCCESS(
          zeMemGetIpcHandle(levelzero.context, allocations[j], &pIpcHandle));
    }
    timer.measureEnd();
...
static RegisterTestCaseImplementation<MemGetIpcHandle>
