...
int BenchmarkMain::printVersion(bool enableWarning, const char *prefix) {
  ...
  if (enableWarning) {
    std::cerr << "Unknown version. Run CMake with \"-D INCLUDE_VERSION=ON\" to "
...
int BenchmarkMain::generateDocs() {
  ...
  const std::string benchmarkDescription =
      BenchmarkInfo::get().getBenchmarkDescription();
  ...
  for (const auto &testEntry : TestMap::get()) {
    ...
    const std::unique_ptr<ArgumentContainer> arguments =
    ...
    for (const Argument *argument : arguments->getArguments()) {
      ...
      if (containsIllegalCharacters(argumentKey, illegalCharacters)) {
        std::cerr << "ERROR: argument \"" << argumentKey << "\" in test case\""
                  << testCaseName << "\" contains illegal characters.";
        return 1;
      }
      const std::string argumentHelp = argument->getExtraHelp();
      if (containsIllegalCharacters(argumentKey, ";")) {
        std::cerr << "ERROR: help message of argument \"" << argumentKey
                  << "\" in test case\"" << testCaseName
                  << "\" contains illegal characters.";
        return 1;
      }
...
BenchmarkMain::BenchmarkMain(int argc, char **argv,
                             const std::string benchmarkVersion)
    : argc(argc), argv(argv), benchmarkVersion(benchmarkVersion) {}
...
void BenchmarkMain::printHelp() {
  ...
  for (const auto &entry : TestMap::get()) {
    ...
    std::cout << '\t' << testCase.getTestCaseName() << " - "
              << testCase.getHelp();
    std::cout << " Supported compute APIs: "
              << joinStrings(", ", apis, getUserFriendlyApiName) << ".";
...
int BenchmarkMain::setupEnvironment() {
  // Kernels will be loaded from the CWD, so we need to ensure we're in the
  // right directory.
  ...
  // Based on parsed command line arguments we can initialize Configuration,
  // which contains global toggles useful for various debug activities.
