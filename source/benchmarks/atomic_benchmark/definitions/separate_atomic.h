...
struct SeparateAtomicsArguments : TestCaseArgumentContainer {
  ...
  SeparateAtomicsArguments()
      : dataType(*this, "type", CommonHelpMessage::atomicDataType()),
        ...
        atomicsPerCacheline(
            *this, "atomicsPerCacheline",
            "Number of used addresses occupying a single cacheline (this "
            "causes operations to be serialized)"),
  ...
  bool validateArgumentsExtra() const override {
    ...
    if (atomicsPerCacheline >
        MemoryConstants::cachelineSize / DataTypeHelper::getSize(dataType)) {
      return false;
    }
    return true;
  }
...
struct SeparateAtomics : TestCase<SeparateAtomicsArguments> {
  ...
  std::string getTestCaseName() const override { return "SeparateAtomics"; }

  std::string getHelp() const override {
    return "enqueues kernel performing an atomic operation on different "
           "addresses";
  }
};
