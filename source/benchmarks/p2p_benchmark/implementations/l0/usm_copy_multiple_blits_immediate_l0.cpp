...
static TestResult
run(const UsmImmediateP2PCopyMultipleBlitsArguments &arguments,
  ...
  for (size_t blitterIndex : arguments.blitters.getEnabledBits()) {
    ...
    ASSERT_ZE_RESULT_SUCCESS(zeCommandListCreateImmediate(
        levelzero.context, srcDevice, &cmdQueueDesc, &list));
  ...
  for (auto i = 0u; i < lists.size(); i++) {
    const auto [offset, size] =
        blitSizeAssigner.getSpaceForBlit(lists[i].isMainCopyEngine);
  ...
  for (PerListData &list : lists) {
    ASSERT_ZE_RESULT_SUCCESS(
        zeCommandListAppendMemoryCopy(list.list, list.copyDst, list.copySrc,
                                      list.copySize, list.event, 0, nullptr));
    ASSERT_ZE_RESULT_SUCCESS(zeEventHostSynchronize(
        list.event, std::numeric_limits<uint64_t>::max()));
  ...
  for (auto i = 0u; i < arguments.iterations; i++) {
    ...
    for (PerListData &list : lists) {
      ...
      ASSERT_ZE_RESULT_SUCCESS(
          zeEventQueryKernelTimestamp(list.event, &timestampResult));
      ...
      statistics.pushValue(commandTime, list.copySize, typeSelector.getUnit(),
                           typeSelector.getType(), list.name);
    }
...
static RegisterTestCaseImplementation<UsmImmediateCopyMultipleBlits>
