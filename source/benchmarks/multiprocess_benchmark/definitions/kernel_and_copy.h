...
struct KernelAndCopyArguments : TestCaseArgumentContainer {
  ...
  KernelAndCopyArguments()
      : twoQueues(*this, "twoQueues",
                  "Enables using separate queues for both operations. Must be "
                  "used with runCopy and runKernel"),
        runKernel(*this, "runKernel", "Enqueue kernel during each iteration"),
        runCopy(*this, "runCopy",
                "Enqueue buffer to buffer copy during each iteration"),
        useCopyQueue(*this, "useCopyQueue",
                     "Use a specialized copy queue for the copy operation. "
                     "Must be used with runCopy") {}
...
struct KernelAndCopy : TestCase<KernelAndCopyArguments> {
  ...
  std::string getTestCaseName() const override { return "KernelAndCopy"; }

  std::string getHelp() const override {
    return "enqueues kernel and copy operation with the ability to perform "
           "both tasks on "
