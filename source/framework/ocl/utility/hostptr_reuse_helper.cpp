...
cl_int HostptrReuseHelper::allocateBufferHostptr(Opencl &opencl,
  ...
  case HostptrReuseMode::Usm: {
    ...
    outAlloc.ptr = usmFunctions.clHostMemAllocINTEL(outAlloc.context, nullptr,
                                                    size, 0llu, &retVal);
    outAlloc.clMemFreeINTEL = usmFunctions.clMemFreeINTEL;
    break;
  }
  case HostptrReuseMode::Map:
    outAlloc.memObject = clCreateBuffer(outAlloc.context, CL_MEM_READ_WRITE,
                                        size, nullptr, &retVal);
    CL_SUCCESS_OR_RETURN(retVal);
    outAlloc.ptr = clEnqueueMapBuffer(outAlloc.queue, outAlloc.memObject,
                                      CL_BLOCKING, CL_MAP_READ | CL_MAP_WRITE,
                                      0, size, 0u, nullptr, nullptr, &retVal);
    CL_SUCCESS_OR_RETURN(retVal);
    break;
  default:
...
cl_int HostptrReuseHelper::deallocateBufferHostptr(Alloc alloc) {
  ...
  case HostptrReuseMode::Map:
    CL_SUCCESS_OR_RETURN(clEnqueueUnmapMemObject(
        alloc.queue, alloc.memObject, alloc.ptr, 0, nullptr, nullptr));
