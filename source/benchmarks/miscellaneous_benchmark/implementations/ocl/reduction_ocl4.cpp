...
TestResult run(const ReductionArguments4 &arguments, Statistics &statistics) {
  ...
  size_t maxAllocSize = {};
  clGetDeviceInfo(opencl.device, CL_DEVICE_MAX_MEM_ALLOC_SIZE,
                  sizeof(maxAllocSize), &maxAllocSize, nullptr);
  ...
  cl_mem partialSums =
      clCreateBuffer(opencl.context, CL_MEM_READ_WRITE,
                     groupCount * sizeof(int32_t), nullptr, &retVal);
  ...
  ASSERT_CL_SUCCESS(
      clSetKernelArg(finalKernel, 1, sizeof(buffer), &partialSums));
  ASSERT_CL_SUCCESS(
      clSetKernelArg(finalKernel, 2, sizeof(cl_uint), &groupCount));

  ASSERT_CL_SUCCESS(clEnqueueNDRangeKernel(opencl.commandQueue, kernel, 1,
                                           nullptr, &gws, &lws, 0, nullptr,
                                           &profilingEvent));
  ASSERT_CL_SUCCESS(clEnqueueNDRangeKernel(opencl.commandQueue, finalKernel, 1,
                                           nullptr, &dispatchOne, &dispatchOne,
                                           0, nullptr, &profilingEvent2));
  ASSERT_CL_SUCCESS(clFinish(opencl.commandQueue));

  ASSERT_CL_SUCCESS(clEnqueueReadBuffer(opencl.commandQueue, partialSums, true,
                                        0u, 4u, &actualSum, 0u, nullptr,
                                        nullptr));
  ...
  ASSERT_CL_SUCCESS(
      ProfilingHelper::getEventDurationInNanoseconds(profilingEvent, timeNs));
  ASSERT_CL_SUCCESS(clReleaseEvent(profilingEvent));
  ASSERT_CL_SUCCESS(
      ProfilingHelper::getEventDurationInNanoseconds(profilingEvent2, timeNs2));
  ASSERT_CL_SUCCESS(clReleaseEvent(profilingEvent2));
  statistics.pushValue(std::chrono::nanoseconds{timeNs}, typeSelector.getUnit(),
                       typeSelector.getType(), "time");
  statistics.pushValue(std::chrono::nanoseconds{timeNs}, sizeInBytes,
                       MeasurementUnit::GigabytesPerSecond,
                       typeSelector.getType(), "bw");
  ...
  for (auto i = 0u; i < arguments.iterations; i++) {
    ...
    ASSERT_CL_SUCCESS(clEnqueueNDRangeKernel(
        opencl.commandQueue, finalKernel, 1, nullptr, &dispatchOne,
        &dispatchOne, 0, nullptr, &profilingEvent2));
    ASSERT_CL_SUCCESS(clFinish(opencl.commandQueue));

    ASSERT_CL_SUCCESS(
        ProfilingHelper::getEventDurationInNanoseconds(profilingEvent, timeNs));
    ASSERT_CL_SUCCESS(ProfilingHelper::getEventDurationInNanoseconds(
        profilingEvent2, timeNs2));
    ASSERT_CL_SUCCESS(clReleaseEvent(profilingEvent));
    ASSERT_CL_SUCCESS(clReleaseEvent(profilingEvent2));
...
static RegisterTestCaseImplementation<Reduction4> registerTestCase(run,
                                                                   Api::OpenCL);
