...
void DeviceInfo::registerFunctions(
    Api api, PrintDeviceInfoFunction printDeviceInfo,
    PrintAvailableDevicesFunction printAvailableDevices) {
  FATAL_ERROR_IF(printDeviceInfo == nullptr, "Cannot register null function");
  FATAL_ERROR_IF(printAvailableDevices == nullptr,
                 "Cannot register null function");

  auto &slot = functions[static_cast<int>(api)];
  FATAL_ERROR_IF(slot.printDeviceInfo != nullptr,
                 "printDeviceInfo function registered multiple times");
  FATAL_ERROR_IF(slot.printAvailableDevices != nullptr,
                 "printAvailableDevices function registered multiple times");
...
void DeviceInfo::printAvailableDevices() {
  for (int apiIndex = static_cast<int>(Api::FIRST);
    ...
    auto &printAvailableDevices =
        functions[static_cast<int>(api)].printAvailableDevices;
