...
struct RemoteAccessMaxSaturationArguments : TestCaseArgumentContainer {
  ...
  RemoteAccessMaxSaturationArguments()
      : size(*this, "size",
        ...
        remoteFraction(*this, "remoteFraction",
                       "Fraction of remote memory access. 1 / n"),
        workItemPackSize(
            *this, "workItemSize",
            "Number of work items group together for remote check"),
        writesPerWorkgroup(
            *this, "writesPerWorkgroup",
            "Number of work items per workgroup that access memory") {}
};

struct RemoteAccessMaxSaturation
    : TestCase<RemoteAccessMaxSaturationArguments> {
  ...
  std::string getHelp() const override {
    return "Uses stream memory write to measure max data bus saturation with "
           "different percentages of remote memory access";
  }
};
