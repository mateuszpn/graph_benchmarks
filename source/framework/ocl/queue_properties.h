...
struct QueueProperties {
  ...
  static QueueProperties create() {
    return QueueProperties().setDeviceSelection(
        Configuration::get().subDeviceSelection);
  }
  ...
  QueueProperties &setDeviceSelection(DeviceSelection newValue) {
    FATAL_ERROR_UNLESS(DeviceSelectionHelper::hasSingleDevice(newValue),
                       "Queue can be created only on a single device");
    this->deviceSelection = newValue;
    return *this;
  }

  bool fillQueueProperties(cl_device_id device,
    ...
    if (this->selectedEngine != Engine::Unknown) {
      const auto propertiesForBlitter =
          QueueFamiliesHelper::getPropertiesForSelectingEngine(
              device, this->selectedEngine);
