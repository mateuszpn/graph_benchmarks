...
static TestResult run(const MultiQueueSubmissionArguments &arguments,
  ...
  if (arguments.workgroupSize >
      levelzero.getDeviceComputeProperties().maxTotalGroupSize) {
  ...
  for (size_t i = 0; i < arguments.queueCount; i++) {
    ...
    ASSERT_ZE_RESULT_SUCCESS(zeMemAllocDevice(levelzero.context,
                                              &deviceAllocDesc, size, 0,
                                              levelzero.device, &pBuffer));
    buffers[i] = pBuffer;

    ASSERT_ZE_RESULT_SUCCESS(
        zeKernelSetArgumentValue(kernel, 0, sizeof(void *), &pBuffer));
  ...
  for (size_t i = 0; i < arguments.queueCount; i++) {
    ASSERT_ZE_RESULT_SUCCESS(
        zeCommandQueueExecuteCommandLists(queues[i], 1, &cmdLists[i], nullptr));
  }
  for (size_t i = 0; i < arguments.queueCount; i++) {
    ASSERT_ZE_RESULT_SUCCESS(zeCommandQueueSynchronize(
        queues[i], std::numeric_limits<uint64_t>::max()));
  }
  ...
  for (auto i = 0u; i < arguments.iterations; i++) {
    ...
    for (size_t j = 0; j < arguments.queueCount; j++) {
      ASSERT_ZE_RESULT_SUCCESS(zeCommandQueueExecuteCommandLists(
          queues[j], 1, &cmdLists[j], nullptr));
    }
    for (size_t j = 0; j < arguments.queueCount; j++) {
      ASSERT_ZE_RESULT_SUCCESS(zeCommandQueueSynchronize(
          queues[j], std::numeric_limits<uint64_t>::max()));
    }
    timer.measureEnd();
...
static RegisterTestCaseImplementation<MultiQueueSubmission>
