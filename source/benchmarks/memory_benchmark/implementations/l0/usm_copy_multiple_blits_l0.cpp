...
static TestResult run(const UsmCopyMultipleBlitsArguments &arguments,
  ...
  for (size_t blitterIndex : arguments.blitters.getEnabledBits()) {
    ...
    ASSERT_ZE_RESULT_SUCCESS(zeCommandQueueCreate(
        levelzero.context, levelzero.device, &cmdQueueDesc, &queue));

    cmdListDesc.commandQueueGroupOrdinal = cmdQueueDesc.ordinal;
    ze_command_list_handle_t list;
    ASSERT_ZE_RESULT_SUCCESS(zeCommandListCreate(
        levelzero.context, levelzero.device, &cmdListDesc, &list));
  ...
  ASSERT_ZE_RESULT_SUCCESS(UsmHelper::allocate(
      arguments.sourcePlacement, levelzero, arguments.size, &srcBuffer));
  ASSERT_ZE_RESULT_SUCCESS(UsmHelper::allocate(
      arguments.destinationPlacement, levelzero, arguments.size, &dstBuffer));
