...
TestResult runSingleIteration(
    const MultipleImmediateCmdListsWithDependenciesArguments &arguments,
    const std::vector<ze_command_list_handle_t> &cmdLists,
    const std::vector<ze_kernel_handle_t> &kernels,
    std::vector<ze_event_handle_t> &events, uint32_t submissionsPerQueue) {
  ...
  for (uint32_t i = 0u; i < cmdlistCount; i++) {
    ...
    ASSERT_ZE_RESULT_SUCCESS(
        zeCommandListAppendLaunchKernel(cmdLists[i], kernels[i], &groupCount,
                                        events[eventIdentifier], 0, nullptr));
  }
...
static TestResult
run(const MultipleImmediateCmdListsWithDependenciesArguments &arguments,
  ...
  const uint32_t totalEventCount =
      submissionsPerQueue * static_cast<uint32_t>(arguments.cmdlistCount);
  ...
  const auto kernelBinary =
      FileHelper::loadBinaryFile("ulls_benchmark_eat_time.spv");
  ...
  for (auto i = 0u; i < arguments.cmdlistCount; i++) {
    ...
    int32_t time = 100;
    ASSERT_ZE_RESULT_SUCCESS(
        zeKernelSetArgumentValue(kernels[i], 0, sizeof(time), &time));
  }
  ...
  for (auto iteration = 0u; iteration < arguments.iterations; iteration++) {
    timer.measureStart();
    result = runSingleIteration(arguments, cmdLists, kernels, events,
                                submissionsPerQueue);
    if (result != TestResult::Success) {
      return result;
    }
...
static RegisterTestCaseImplementation<MultipleImmediateCmdListsWithDependencies>
