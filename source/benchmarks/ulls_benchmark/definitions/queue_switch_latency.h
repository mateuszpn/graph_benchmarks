  ...
  QueueSwitchArguments()
      : kernelTime(*this, "kernelTime", "Time for each kernel execution"),
        switchCount(*this, "switchCount",
                    "How many switches form each iteration") {}
};

struct QueueSwitch : TestCase<QueueSwitchArguments> {
  using TestCase<QueueSwitchArguments>::TestCase;

  std::string getTestCaseName() const override { return "QueueSwitch"; }

  std::string getHelp() const override {
    return "creates multiple queues, creates dependencies with events between "
           "those and measures switch time";
  }
};