...
static TestResult run(const SetKernelArgSvmPointerArguments &arguments,
  ...
  const std::vector<uint8_t> kernelSource =
      FileHelper::loadTextFile("api_overhead_benchmark_fill_with_ones.cl");
  ...
  for (auto i = 0u; i < arguments.allocationsCount; i++) {
    ...
    ASSERT_CL_SUCCESS(UsmHelperOcl::allocate(opencl, UsmMemoryPlacement::Shared,
                                             arguments.allocationSize, alloc));
    allocations.push_back(alloc);
  }

  // Warmup
  for (auto i = 0u; i < arguments.allocationsCount; i++) {
    ASSERT_CL_SUCCESS(clSetKernelArgSVMPointer(
        kernels[i], 0, static_cast<cl_int *>(allocations[i].ptr)));
  }
  ...
  if (arguments.reallocate) {
    for (auto i = 0u; i < arguments.allocationsCount; i++) {
      ...
      ASSERT_CL_SUCCESS(UsmHelperOcl::allocate(
          opencl, UsmMemoryPlacement::Shared, arguments.allocationSize, alloc));
      allocations[i] = alloc;
    }
  }
  ...
  for (auto i = 0u; i < arguments.iterations; i++) {
    ...
    for (auto j = 0u; j < arguments.allocationsCount; j++) {
      ASSERT_CL_SUCCESS(clSetKernelArgSVMPointer(
          kernels[j], 0, static_cast<cl_int *>(allocations[j].ptr)));
    }
    timer.measureEnd();
