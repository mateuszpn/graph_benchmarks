...
static TestResult run(const WriteLatencyArguments &arguments,
  ...
  const ze_event_desc_t eventDesc2 = {ZE_STRUCTURE_TYPE_EVENT_DESC, nullptr, 1,
                                      ZE_EVENT_SCOPE_FLAG_HOST, 0u};
  ...
  ASSERT_ZE_RESULT_SUCCESS(
      zeCommandListAppendWaitOnEvents(cmdList, 1u, &hEvent2));
  ASSERT_ZE_RESULT_SUCCESS(zeCommandListAppendWriteGlobalTimestamp(
      cmdList, static_cast<uint64_t *>(buffer), nullptr, 0, nullptr));
...
#if ADD_ENTER_SUPPORT
    printf("\n Welcome to minimized test reproducer, state is prepared at this "
           "point buffer2(on which cpu will wait) is at %p , press enter to "
           "continue",
           volatileBuffer);
    std::cin.ignore();
#endif
...
static RegisterTestCaseImplementation<WriteLatency> registerTestCase(run,
                                                                     Api::L0);
