...
struct WalkerCompletionLatencyArguments : TestCaseArgumentContainer {
  ...
  WalkerCompletionLatencyArguments()
      : useFence(*this, "useFence",
                 "Use fence during submission and for further completion."),
        inOrderQueue(
            *this, "inOrderQueue",
            "If set use IOQ, otherwise OOQ. Applicable only for OCL.") {}
};
...
struct WalkerCompletionLatency : TestCase<WalkerCompletionLatencyArguments> {
  ...
  std::string getHelp() const override {
    return "enqueues a kernel writing to system memory and measures time "
           "between the moment when "
           "update is visible on CPU and the moment when synchronizing call "
           "returns";
  }
};
