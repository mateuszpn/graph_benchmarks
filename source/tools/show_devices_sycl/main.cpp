...
std::vector<sycl::device> createSubDevices(const sycl::device &rootDevice) {
  auto domain =
      rootDevice.get_info<sycl::info::device::partition_type_affinity_domain>();

  if (domain != sycl::info::partition_affinity_domain::numa) {
    return {};
  }
  std::vector<sycl::device> result = rootDevice.create_sub_devices<
      sycl::info::partition_property::partition_by_affinity_domain>(domain);
  return result;
}
...
void showPlatform(size_t indentLevel, const sycl::platform &platform,
                  size_t platformIndex) {
  const std::string indent0(indentLevel + 0, '\t');

  std::cout << indent0 << "Platform " << platformIndex << ": "
            << platform.get_info<sycl::info::platform::name>() << '\n';
}

void showDevice(size_t indentLevel, const sycl::device &device,
