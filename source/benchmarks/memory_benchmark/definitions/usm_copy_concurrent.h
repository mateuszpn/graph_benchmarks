...
struct UsmConcurrentCopy : TestCase<UsmConcurrentCopyArguments> {
  ...
  std::string getTestCaseName() const override { return "UsmConcurrentCopy"; }

  std::string getHelp() const override {
    return "allocates four unified shared memory buffers, 2 in device memory "
           "and 2 in host memory. Measures concurrent copy bandwidth between "
           "them.";
  }
};
