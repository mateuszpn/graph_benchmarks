  ...
  UnmapBufferArguments()
      : size(*this, "size", "Size of the buffer"),
        ...
        compressed(*this, "compressed",
                   CommonHelpMessage::compression("buffer")),
        mapFlags(*this, "mapFlags",
                 "OpenCL map flags passed during memory mapping"),
...
struct UnmapBuffer : TestCase<UnmapBufferArguments> {
  ...
  std::string getTestCaseName() const override { return "UnmapBuffer"; }

  std::string getHelp() const override {
    return "allocates an OpenCL buffer and measures unmap bandwidth. Unmapping "
           "operation means"
           "memory transfer from CPU to GPU or a no-op, depending on map "
           "flags.";
  }
};
