...
static TestResult run(const UsmFillMultipleBlitsArguments &arguments,
  ...
  ASSERT_ZE_RESULT_SUCCESS(UsmHelper::allocate(
      arguments.memoryPlacement, levelzero, arguments.size, &dstBuffer));
  ...
  BufferContentsHelperL0::fill(pattern.get(), arguments.patternSize,
                               arguments.patternContents);

  // Append commands
  for (PerQueueData &queue : queues) {
    ASSERT_ZE_RESULT_SUCCESS(zeCommandListAppendMemoryFill(
        queue.list, queue.fillDst, pattern.get(), arguments.patternSize,
        queue.fillSize, queue.event, 0, nullptr));
  ...
  for (auto i = 0u; i < arguments.iterations; i++) {
    ...
    for (PerQueueData &queue : queues) {
      ...
      statistics.pushValue(commandTime, queue.fillSize, typeSelector.getUnit(),
