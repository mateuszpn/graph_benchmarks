...
int printAvailableEngines(ze_device_handle_t device, uint32_t numberOfTabs) {
  ...
  EXPECT_ZE_RESULT_SUCCESS(zeDeviceGetCommandQueueGroupProperties(
      device, &numQueueGroups, queueProperties.data()));

  std::string tabDelimiter = "";
  tabDelimiter.append(numberOfTabs, '\t');
  std::cout << tabDelimiter << "\t" << numQueueGroups
            << " queue groups found\n";

  for (uint32_t i = 0; i < numQueueGroups; i++) {
    if (queueProperties[i].flags &
        ZE_COMMAND_QUEUE_GROUP_PROPERTY_FLAG_COMPUTE) {
      std::cout << tabDelimiter << "\t Group " << i
...
int printDeviceProperties(ze_device_handle_t device, uint32_t numberOfTabs) {
  ...
  std::cout << tabDelimiter << "\tname:        " << deviceProperties.name
            ...
            << "\n"
            << tabDelimiter << "\tsubslices per slice: "
            << deviceProperties.numSubslicesPerSlice << "\n"
            << tabDelimiter
            << "\tEUs per slice: " << deviceProperties.numEUsPerSubslice << "\n"
            << tabDelimiter
            << "\tthreads per EU: " << deviceProperties.numThreadsPerEU << "\n"
            << tabDelimiter << "\tdeviceId:    0x" << std::setw(4) << std::hex
            << std::setfill('0') << deviceProperties.deviceId << "\n"
  ...
  std::cout << tabDelimiter << "\thost: "
            << parseCaps(memoryAccessCapabilities.hostAllocCapabilities)
            << "\n";
  std::cout << tabDelimiter << "\tdevice: "
            << parseCaps(memoryAccessCapabilities.deviceAllocCapabilities)
            << "\n";
  std::cout << tabDelimiter << "\tshared single device: "
            << parseCaps(
                   memoryAccessCapabilities.sharedSingleDeviceAllocCapabilities)
            << "\n";
  std::cout << tabDelimiter << "\tshared cross device: "
            << parseCaps(
                   memoryAccessCapabilities.sharedCrossDeviceAllocCapabilities)
            << "\n";
  std::cout << tabDelimiter << "\tshared system caps: "
            << parseCaps(memoryAccessCapabilities.sharedSystemAllocCapabilities)
            << "\n";
...
int printPropertiesForAllSubDevices(ze_device_handle_t device,
...
int printDevicesWithTheSameBdfAddress(
  ...
  for (const auto &device : devices) {
    properties.at(dataIdentifier).properties.stype =
    ...
    ze_result_t res = zeDevicePciGetPropertiesExt(
        device, &properties.at(dataIdentifier++).properties);
  ...
  for (const auto &currentPciProp : properties) {
    ...
    pciBdf << std::hex << std::setfill('0') << std::setw(4) << domainId << ":"
           << std::setw(2) << busId << ":" << std::setw(2) << deviceId << ":"
           << functionId;
