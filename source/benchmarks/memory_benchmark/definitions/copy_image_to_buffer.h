...
struct CopyImageToBuffer : TestCase<CopyImageToBufferArguments> {
  ...
  std::string getTestCaseName() const override { return "CopyImageToBuffer"; }

  std::string getHelp() const override {
    return "allocates image and buffer and measures copy bandwidth between "
           "them using immediate command list for Level Zero and command queue "
           "for OpenCL.";
  }
};
