...
static TestResult run(const UsmCopyRegionArguments &arguments,
  ...
  ExtensionProperties extensionProperties =
      ExtensionProperties::create().setImportHostPointerFunctions(
  ...
  const ze_copy_region_t reg = {
      (uint32_t)arguments.origin[0], (uint32_t)arguments.origin[1],
      (uint32_t)arguments.origin[2], (uint32_t)arguments.region[0],
      (uint32_t)arguments.region[1], (uint32_t)arguments.region[2]};
  auto copySize =
      arguments.region[0] * arguments.region[1] * arguments.region[2];
  ASSERT_ZE_RESULT_SUCCESS(UsmHelper::allocate(
      arguments.sourcePlacement, levelzero, arguments.size, &source));
  ASSERT_ZE_RESULT_SUCCESS(UsmHelper::allocate(
      arguments.destinationPlacement, levelzero, arguments.size, &destination));
  ...
  auto commandQueueDesc = QueueFamiliesHelper::getPropertiesForSelectingEngine(
      levelzero.device, queueProperties.selectedEngine);
  ASSERT_ZE_RESULT_SUCCESS(zeCommandListCreateImmediate(
      levelzero.context, levelzero.device, &commandQueueDesc->desc, &cmdList));

  // Warmup
  ASSERT_ZE_RESULT_SUCCESS(zeCommandListAppendMemoryCopyRegion(
      cmdList, destination, &reg, static_cast<uint32_t>(arguments.region[0]),
      static_cast<uint32_t>(arguments.region[0] * arguments.region[1]), source,
      &reg, static_cast<uint32_t>(arguments.region[0]),
      static_cast<uint32_t>(arguments.region[0] * arguments.region[1]), event,
      0, nullptr));
  ...
  for (auto i = 0u; i < arguments.iterations; i++) {
    ...
    ASSERT_ZE_RESULT_SUCCESS(zeCommandListAppendMemoryCopyRegion(
        cmdList, destination, &reg, static_cast<uint32_t>(arguments.region[0]),
        static_cast<uint32_t>(arguments.region[0] * arguments.region[1]),
        source, &reg, static_cast<uint32_t>(arguments.region[0]),
        static_cast<uint32_t>(arguments.region[0] * arguments.region[1]), event,
        0, nullptr));
    ASSERT_ZE_RESULT_SUCCESS(
        zeEventHostSynchronize(event, std::numeric_limits<uint64_t>::max()));
    ...
    if (arguments.useEvents) {
      ...
      statistics.pushValue(commandTime, copySize, typeSelector.getUnit(),
                           typeSelector.getType());
    } else {
      statistics.pushValue(timer.get(), copySize, typeSelector.getUnit(),
                           typeSelector.getType());
    }
  ...
  ASSERT_ZE_RESULT_SUCCESS(
      UsmHelper::deallocate(arguments.sourcePlacement, levelzero, source));
  ASSERT_ZE_RESULT_SUCCESS(UsmHelper::deallocate(arguments.destinationPlacement,
                                                 levelzero, destination));

  return TestResult::Success;
}

static RegisterTestCaseImplementation<UsmCopyRegion> registerTestCase(run,
                                                                      Api::L0);
