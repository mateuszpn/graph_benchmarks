...
static TestResult run(const QueueSwitchArguments &arguments,
  ...
  ASSERT_ZE_RESULT_SUCCESS(zeCommandListCreate(
      levelzero.context, levelzero.device, &cmdListDesc, &cmdList1));
  ze_command_list_handle_t cmdList2{};
  ASSERT_ZE_RESULT_SUCCESS(zeCommandListCreate(
      levelzero.context, levelzero.device, &cmdListDesc, &cmdList2));
  ...
  const ze_event_pool_desc_t eventPoolDesc{
      ZE_STRUCTURE_TYPE_EVENT_POOL_DESC, &counterBasedDesc, flags,
      static_cast<uint32_t>(arguments.switchCount + 1)};
  ...
  for (auto eventId = 0u; eventId < arguments.switchCount + 1; eventId++) {
    ze_event_desc_t eventDesc = {ZE_STRUCTURE_TYPE_EVENT_DESC, nullptr, eventId,
                                 0, 0};
    ASSERT_ZE_RESULT_SUCCESS(
        zeEventCreate(hEventPool, &eventDesc, &profilingEvents[eventId]));
  }

  ASSERT_ZE_RESULT_SUCCESS(zeCommandListAppendLaunchKernel(
      cmdList1, kernel, &groupCount, profilingEvents[0], 0, nullptr));
  for (auto switchIdentifier = 0u; switchIdentifier < arguments.switchCount;
       switchIdentifier++) {
    ASSERT_ZE_RESULT_SUCCESS(zeCommandListAppendLaunchKernel(
        (switchIdentifier % 2) == 1 ? cmdList2 : cmdList1, kernel, &groupCount,
        profilingEvents[switchIdentifier + 1], 1,
        &profilingEvents[switchIdentifier]));
  }
  ...
  ASSERT_ZE_RESULT_SUCCESS(zeCommandQueueCreate(
      levelzero.context, levelzero.device, &commandQueueDesc, &queue1));
  ASSERT_ZE_RESULT_SUCCESS(zeCommandQueueCreate(
      levelzero.context, levelzero.device, &commandQueueDesc, &queue2));

  ASSERT_ZE_RESULT_SUCCESS(
      zeCommandQueueExecuteCommandLists(queue1, 1, &cmdList1, nullptr));
  ASSERT_ZE_RESULT_SUCCESS(
      zeCommandQueueExecuteCommandLists(queue2, 1, &cmdList2, nullptr));

  ASSERT_ZE_RESULT_SUCCESS(
      zeCommandQueueSynchronize(queue1, std::numeric_limits<uint64_t>::max()));
  ASSERT_ZE_RESULT_SUCCESS(
      zeCommandQueueSynchronize(queue2, std::numeric_limits<uint64_t>::max()));
  ...
  for (auto iteration = 0u; iteration < arguments.iterations; iteration++) {
    ...
    ASSERT_ZE_RESULT_SUCCESS(zeCommandQueueSynchronize(
        queue1, std::numeric_limits<uint64_t>::max()));
    ASSERT_ZE_RESULT_SUCCESS(zeCommandQueueSynchronize(
        queue2, std::numeric_limits<uint64_t>::max()));
    ...
    for (auto switchId = 0u; switchId < arguments.switchCount; switchId++) {
      ...
      ASSERT_ZE_RESULT_SUCCESS(zeEventQueryKernelTimestamp(
          profilingEvents[switchId], &earlierKernelTimestamp));
      ze_kernel_timestamp_result_t laterKernelTimestamp;
      ASSERT_ZE_RESULT_SUCCESS(zeEventQueryKernelTimestamp(
          profilingEvents[switchId + 1], &laterKernelTimestamp));
    ...
    statistics.pushValue(switchTime / (arguments.switchCount),
...
static RegisterTestCaseImplementation<QueueSwitch> registerTestCase(run,
                                                                    Api::L0);
