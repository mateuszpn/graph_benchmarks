...
static TestResult run(const UsmSharedFirstCpuAccessArguments &arguments,
  ...
  auto buffer = clSharedMemAllocINTEL(opencl.context, opencl.device, properties,
  ...
  for (auto i = 0u; i < arguments.iterations; i++) {
    buffer = clSharedMemAllocINTEL(opencl.context, opencl.device, properties,
                                   arguments.bufferSize, 0u, &retVal);
    ASSERT_CL_SUCCESS(retVal);
...
static RegisterTestCaseImplementation<UsmSharedFirstCpuAccess>
