...
struct HostptrBufferReuseModeArgument
    : EnumArgument<HostptrBufferReuseModeArgument, HostptrReuseMode> {
  ...
  const static inline EnumType enumValues[3] = {EnumType::None, EnumType::Usm,
                                                EnumType::Map};
  const static inline std::string enumValuesNames[3] = {"None", "Usm", "Map"};
};

struct HostptrImageReuseModeArgument
    : EnumArgument<HostptrImageReuseModeArgument, HostptrReuseMode> {
  using EnumArgument::EnumArgument;
  ThisType &operator=(EnumType newValue) {
    this->value = newValue;
    markAsParsed();
