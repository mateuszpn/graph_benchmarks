...
struct SlmSwitchLatencyArguments : TestCaseArgumentContainer {
  ...
  SlmSwitchLatencyArguments()
      : slmPerWkgKernel1(
            *this, "firstSlmSize",
            "Size of the shared local memory per thread group. First kernel."),
        slmPerWkgKernel2(
            *this, "secondSlmSize",
            "Size of the shared local memory per thread group. Second kernel."),
...
struct SlmSwitchLatency : TestCase<SlmSwitchLatencyArguments> {
  ...
  std::string getTestCaseName() const override { return "SlmSwitchLatency"; }

  std::string getHelp() const override {
    return "Enqueues 2 kernels with different SLM size. Measures switch time "
