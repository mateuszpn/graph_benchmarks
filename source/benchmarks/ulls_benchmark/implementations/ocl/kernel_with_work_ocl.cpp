...
static TestResult run(const KernelWithWorkArguments &arguments,
  ...
  const auto bufferSize =
      sizeof(cl_int) * arguments.workgroupCount * arguments.workgroupSize * 2;
  ...
  for (auto i = 0u; i < arguments.iterations; i++) {
    ...
    if (arguments.usedIds == WorkItemIdUsage::AtomicPerWorkgroup) {
      ...
      ASSERT_CL_SUCCESS(clEnqueueReadBuffer(opencl.commandQueue, buffer, true,
                                            0u, 8u, &returnedValue, 0u, nullptr,
                                            nullptr));
...
static RegisterTestCaseImplementation<KernelWithWork>
