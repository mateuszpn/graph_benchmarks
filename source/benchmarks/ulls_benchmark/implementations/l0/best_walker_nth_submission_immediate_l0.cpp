...
static TestResult
run(const BestWalkerNthSubmissionImmediateArguments &arguments,
  ...
  ze_host_mem_alloc_desc_t allocationDesc{
  ...
  for (uint32_t i = 0; i < arguments.kernelCount; i++) {
    ASSERT_ZE_RESULT_SUCCESS(zeCommandListAppendLaunchKernel(
        cmdList, kernels[i].kernel, &groupCount, kernels[i].event, 0, nullptr));
  }
  for (uint32_t i = 0; i < arguments.kernelCount; i++) {
    ASSERT_ZE_RESULT_SUCCESS(zeEventHostSynchronize(
        kernels[i].event, std::numeric_limits<uint64_t>::max()));
  ...
  for (auto i = 0u; i < arguments.iterations; i++) {
    *volatileBuffer = 0;
    ...
    for (uint32_t j = 0; j < arguments.kernelCount; ++j) {
      ASSERT_ZE_RESULT_SUCCESS(zeCommandListAppendLaunchKernel(
          cmdList, kernels[j].kernel, &groupCount, kernels[j].event, 0,
          nullptr));
    }
    ...
    for (uint32_t j = 0; j < arguments.kernelCount; ++j) {
      ASSERT_ZE_RESULT_SUCCESS(zeEventHostSynchronize(
          kernels[j].event, std::numeric_limits<uint64_t>::max()));
...
static RegisterTestCaseImplementation<BestWalkerNthSubmissionImmediate>
