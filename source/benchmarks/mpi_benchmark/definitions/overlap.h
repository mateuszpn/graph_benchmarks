  ...
  MpiOverlapArguments()
      : mpiLauncher(*this, "mpiLauncher",
        ...
        gpuCompute(*this, "gpuCompute",
                   "Whether we do computation on the GPU or not"),
        numMpiTestCalls(*this, "numMpiTestCalls",
                        "Number of MPI_Test calls during computation") {
    mpiLauncher = "mpirun";
    messageSize = 1;
...
struct MpiOverlap : public TestCase<MpiOverlapArguments> {
  ...
  std::string getTestCaseName() const override { return "MpiOverlap"; }

  std::string getHelp() const override {
    return "Measures the computation-communication overlap efficency when "
           "using non-blocking message transfers in an MPI+X application.";
  }
};
