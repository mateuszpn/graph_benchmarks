...
MathOperationTestData KernelHelper::getDataForKernel(DataType dataType,
  ...
  const size_t loopIterations =
      100u; // Kernel performs some number of loop iterations
  const size_t operatorApplicationsPerIteration =
      128u; // Each iteration performs some number of atomic operations
  return MathOperationHelper::generateTestData(
      dataType, operation, loopIterations, operatorApplicationsPerIteration,
      totalThreadsCount);
}
...
std::string KernelHelper::getCompilerOptions(DataType dataType,
  ...
  if (MathOperationHelper::requiresIntelGlobalAtomicsExtension(operation,
                                                               dataType)) {
  ...
  options.addDefinitionKeyValue("ATOMIC_DATATYPE",
                                DataTypeHelper::toOpenclC(dataType));
  options.addDefinitionKeyValue("DATATYPE",
                                DataTypeHelper::toOpenclC(dataType));
  options.addDefinitionKeyValue("OTHER_ARGUMENT_BUFFER_SIZE",
                                std::to_string(otherArgumentBufferSize));
  return options.str();
}

std::string KernelHelper::getCompilerOptionsExplicit(
    DataType dataType, MathOperation operation, AtomicMemoryOrder order,
  ...
  options.addDefinitionKeyValue(
      "ATOMIC_DATATYPE", DataTypeHelper::toExplicitAtomicOpenclC(dataType));
  options.addDefinitionKeyValue("DATATYPE",
                                DataTypeHelper::toOpenclC(dataType));
  options.addDefinitionKeyValue("OTHER_ARGUMENT_BUFFER_SIZE",
                                std::to_string(otherArgumentBufferSize));
...
void KernelHelper::addAtomicOpMacro(CompilerOptionsBuilder &options,
                                    MathOperation operation) {
  const static char *functionNames[] = {
      "ERROR",          "atomic_add", "atomic_sub", "atomic_xchg",
      "atomic_cmpxchg", "atomic_inc", "atomic_dec", "atomic_min",
      "atomic_max",     "atomic_and", "atomic_or",  "atomic_xor"};
...
void KernelHelper::addExplicitAtomicOpMacro(CompilerOptionsBuilder &options,
                                            MathOperation operation,
  ...
  const static char *functionNames[] = {
      ...
      "atomic_fetch_add_explicit",
      "atomic_fetch_sub_explicit", // inc and dec emulated with add and sub
