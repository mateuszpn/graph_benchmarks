...
static TestResult run(const CopyImageRegionArguments &arguments,
  ...
  const size_t region[] = {arguments.size[0], arguments.size[1],
                           arguments.size[2]};

  ASSERT_CL_SUCCESS(clEnqueueCopyImage(opencl.commandQueue, srcImage, dstImage,
  ...
  for (auto i = 0u; i < arguments.iterations; i++) {
    ...
    timer.measureStart();
    ASSERT_CL_SUCCESS(clEnqueueCopyImage(opencl.commandQueue, srcImage,
                                         dstImage, origin, origin, region, 0,
...
static RegisterTestCaseImplementation<CopyImageRegion>
