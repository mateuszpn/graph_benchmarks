...
static TestResult
doVirtualMemReserve(LevelZero &levelzero,
                    const VirtualMemReserveArguments &arguments, Timer &timer) {
  ...
  if (arguments.useNull) {
    ...
    ASSERT_ZE_RESULT_SUCCESS(
        zeVirtualMemFree(levelzero.context, reservedPtr, reserveSize));
    timer.measureStart();
    ASSERT_ZE_RESULT_SUCCESS(zeVirtualMemReserve(levelzero.context, reservedPtr,
                                                 reserveSize, &reservedPtr));
    timer.measureEnd();
    EXPECT_NE(reservedPtr, static_cast<void *>(nullptr));
  }
...
static TestResult run(const VirtualMemReserveArguments &arguments,
...
static RegisterTestCaseImplementation<VirtualMemReserve>
