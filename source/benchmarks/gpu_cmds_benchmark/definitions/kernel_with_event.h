...
struct KernelWithEventArguments : TestCaseArgumentContainer {
  ...
  KernelWithEventArguments()
      : measuredCommands(*this, "measuredCmds",
        ...
        useHostSignalEvent(
            *this, "hostSignalEvent",
            "Use ZE_EVENT_POOL_HOST_VISIBLE for ze_event_pool_desc_t::flags,"
            " and use ZE_EVENT_SCOPE_FLAG_HOST for ze_event_desc_t::signal"),
        useDeviceWaitEvent(
            *this, "devWaitEvent",
            "Use ZE_EVENT_SCOPE_FLAG_DEVICE for ze_event_desc_t::wait"),
        useTimestampEvent(*this, "timestampEvent",
                          "Use ZE_EVENT_POOL_FLAG_KERNEL_TIMESTAMP for "
                          "ze_event_pool_desc_t::flags") {}
};
...
public:
  ...
  std::string getTestCaseName() const override { return "KernelWithEvent"; }

  std::string getHelp() const override {
    return "measures time required to run an empty kernel with various event "
           "configurations.";
  }
};
