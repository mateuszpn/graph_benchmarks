...
static TestResult
run(const BestWalkerSubmissionImmediateMultiCmdlistsArguments &arguments,
  ...
  for (auto i = 0u; i < arguments.cmdlistCount; i++) {
    ...
    ASSERT_ZE_RESULT_SUCCESS(zeKernelSetArgumentValue(
        kernels[i], 0, sizeof(buffers[i]), &buffers[i]));
  }
  ...
  for (auto i = 0u; i < arguments.cmdlistCount; i++) {
    ASSERT_ZE_RESULT_SUCCESS(zeCommandListAppendLaunchKernel(
        cmdLists[i], kernels[i], &groupCount, events[i], 0, nullptr));
  }
  for (auto i = 0u; i < arguments.cmdlistCount; i++) {
...
static RegisterTestCaseImplementation<
    BestWalkerSubmissionImmediateMultiCmdlists>
