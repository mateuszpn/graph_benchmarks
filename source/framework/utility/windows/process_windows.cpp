...
public:
  EnvironmentRestorer() {
    ...
    while (environment[environmentSize] != '\0' ||
           environment[environmentSize - 1] != '\0') {
      environmentSize++;
    }
    environmentSize++;
  ...
  ~EnvironmentRestorer() {
    if (SetEnvironmentStringsA(this->storedEnv.get()) == 0) {
      FATAL_ERROR_IN_DESTRUCTOR(
          std::string("restoring environemnt variables, ") +
          getErrorFromLastErrorCode());
    }
  }
...
void asyncReadThreadBody(ProcessDataWindows *processDataWindows) {
  ...
  while (true) {
    BOOL retVal = ReadFile(processDataWindows->processStdOut.read, buffer,
                           bufferSize, &numberOfBytesRead, NULL);
...
void Process::run() {
  ...
  // Create pipes for stdout and stdin of the child process. Pipes that are
  // passed to the process (stdOut.write, processStdIn.read) should be closed
  ...
  FATAL_ERROR_IF_SYS_CALL_FAILED(
      CreatePipe(&processDataWindows->processStdOut.read,
                 &processDataWindows->processStdOut.write,
                 &pipeSecutrityAttributes, 0),
      "creating pipe for stdout")
  FATAL_ERROR_IF_SYS_CALL_FAILED(
      CreatePipe(&processDataWindows->processStdIn.read,
                 &processDataWindows->processStdIn.write,
                 &pipeSecutrityAttributes, 0),
      "creating pipe for stdin")
  FATAL_ERROR_IF_SYS_CALL_FAILED(
      SetHandleInformation(processDataWindows->processStdOut.read,
  ...
  // Prepare env variables - create RAII restorer and update existing ones, so
  // the child process inherits them
  EnvironmentRestorer envRestorer = {};
  for (const auto &envVariable : envVariables) {
    FATAL_ERROR_IF_SYS_CALL_FAILED(
        SetEnvironmentVariableA(envVariable.first.c_str(),
                                envVariable.second.c_str()),
  ...
  PROCESS_INFORMATION processInfo{};
  FATAL_ERROR_IF_SYS_CALL_FAILED(
      CreateProcessA(exeNameWithExtension.c_str(), commandLine.str().data(),
                     NULL, NULL, TRUE, 0, NULL, NULL, &startupInfo,
                     &processDataWindows->processInfo),
      "creating process");

  // Create an asynchronous thread for reading stdout/stderr pipes. This is
  // needed for cases when process outputs a substantial amount of data
  // exceeding internal system buffer. This causes a deadlock, because
  // WaitForSingleProcess will block inifinitely.
  processDataWindows->asyncReadThread = std::make_unique<std::thread>(
      asyncReadThreadBody, processDataWindows.get());
...
void Process::freeOsSpecificData() {
  ProcessDataWindows *processDataWindows =
      static_cast<ProcessDataWindows *>(this->osSpecificData);
  ...
  FATAL_ERROR_IF_SYS_CALL_FAILED(
      CloseHandle(processDataWindows->processInfo.hProcess),
      "closing process handle");
  FATAL_ERROR_IF_SYS_CALL_FAILED(
      CloseHandle(processDataWindows->processInfo.hThread),
      "closing thread handle");
  FATAL_ERROR_IF_SYS_CALL_FAILED(
      CloseHandle(processDataWindows->processStdOut.read),
      "closing stdout.read handle");
  FATAL_ERROR_IF_SYS_CALL_FAILED(
      CloseHandle(processDataWindows->processStdIn.write),
      "closing stdin.write handle");
...
void Process::waitForFinish() {
  ...
  if (WaitForSingleObject(processDataWindows->processInfo.hProcess,
                          std::numeric_limits<DWORD>::max()) != WAIT_OBJECT_0) {
    FATAL_ERROR(std::string("waiting for process to end, ") +
                getErrorFromLastErrorCode());
  }

  FATAL_ERROR_IF_SYS_CALL_FAILED(
      CloseHandle(processDataWindows->processStdOut.write),
      "closing stdout.write handle");
  FATAL_ERROR_IF_SYS_CALL_FAILED(
      CloseHandle(processDataWindows->processStdIn.read),
      "closing stdin.read handle");
...
TestResult Process::getResult() {
  ...
  if (!processDataWindows->hasResult) {
    ...
    DWORD exitCode{};
    FATAL_ERROR_IF_SYS_CALL_FAILED(
        GetExitCodeProcess(processDataWindows->processInfo.hProcess, &exitCode),
        "retrieving process exit code");
...
const std::string &Process::getMeasurements() { return getStdout(); }
...
void Process::synchronizationSignal() {
  ...
  FATAL_ERROR_IF_SYS_CALL_FAILED(
      WriteFile(processDataWindows->processStdIn.write, &buffer, 1,
                &numberOfBytesWritten, nullptr),
      "writing to child process's stdin");
}
...
void Process::synchronizationWait() {
  ...
  DWORD numberOfBytesRead = {};
  FATAL_ERROR_IF_SYS_CALL_FAILED(
      ReadFile(processDataWindows->processStdOut.read, &buffer, 1,
               &numberOfBytesRead, NULL),
      "reading from child process's stdout");
  FATAL_ERROR_IF(
