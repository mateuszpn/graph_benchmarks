...
static TestResult
doVirtualMemSetAccessAttrib(LevelZero &levelzero,
                            const VirtualMemSetAccessAttribArguments &arguments,
                            Timer &timer) {
  ...
  ASSERT_ZE_RESULT_SUCCESS(
      zeVirtualMemReserve(levelzero.context, nullptr, size, &virtualMem));
  ...
  timer.measureStart();
  ASSERT_ZE_RESULT_SUCCESS(zeVirtualMemSetAccessAttribute(
      levelzero.context, virtualMem, size, accessType));
  timer.measureEnd();

  // Cleanup
  ASSERT_ZE_RESULT_SUCCESS(
      zeVirtualMemFree(levelzero.context, virtualMem, size));

  return TestResult::Success;
}

static TestResult run(const VirtualMemSetAccessAttribArguments &arguments,
...
static RegisterTestCaseImplementation<VirtualMemSetAccessAttrib>
