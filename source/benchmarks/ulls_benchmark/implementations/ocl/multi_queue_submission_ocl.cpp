...
static TestResult run(const MultiQueueSubmissionArguments &arguments,
  ...
  for (size_t i = 0; i < arguments.queueCount; i++) {
    ...
    buffers.push_back(clCreateBuffer(opencl.context, CL_MEM_READ_WRITE, size,
                                     nullptr, &retVal));
  }
  ...
  for (size_t i = 0; i < arguments.queueCount; i++) {
    ASSERT_CL_SUCCESS(
        clSetKernelArg(kernel, 0, sizeof(buffers[i]), &buffers[i]));
    ASSERT_CL_SUCCESS(clEnqueueNDRangeKernel(
        queues[i], kernel, 1, nullptr, &gws, nullptr, 0, nullptr, nullptr));
  }
  for (size_t i = 0; i < arguments.queueCount; i++) {
