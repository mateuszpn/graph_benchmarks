...
static TestResult run(const NewResourcesWithGpuAccessArguments &arguments,
  ...
  // Create kernel
  auto kernelBinary =
      FileHelper::loadBinaryFile("ulls_benchmark_fill_with_ones.spv");
  ...
  const uint32_t elements =
      static_cast<uint32_t>(arguments.size) / sizeof(uint32_t);
  uint32_t groupSizeX{}, groupSizeY{}, groupSizeZ{};
  ASSERT_ZE_RESULT_SUCCESS(
      zeKernelSuggestGroupSize(kernel, static_cast<uint32_t>(arguments.size), 1,
                               1, &groupSizeX, &groupSizeY, &groupSizeZ));
  uint32_t groupsCount =
      elements / groupSizeX; // may be rounded down, but that shouldn't matter
                             // for big buffers
  if (groupsCount == 0) {
    // very small buffer
    groupSizeX = elements;
  ...
  const ze_device_mem_alloc_desc_t allocationDesc{
      ZE_STRUCTURE_TYPE_HOST_MEM_ALLOC_DESC};
  void *buffer = nullptr;
  ASSERT_ZE_RESULT_SUCCESS(zeMemAllocDevice(levelzero.context, &allocationDesc,
                                            bufferSize, 0, levelzero.device,
                                            &buffer));
