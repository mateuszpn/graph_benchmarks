...
struct ReadAfterAtomicWriteArguments : TestCaseArgumentContainer {
  ...
  ReadAfterAtomicWriteArguments()
      : workgroupSize(*this, "wgs", "Workgroup size"),
        atomic(*this, "atomic",
               "If true, write to global memory will be atomic."),
        shuffleRead(*this, "shuffleRead",
                    "If true, each thread will write and read different memory "
                    "cell. Otherwise it will be the same one."),
...
struct ReadAfterAtomicWrite : TestCase<ReadAfterAtomicWriteArguments> {
  ...
  std::string getHelp() const override {
    return "enqueues kernel, which writes to global memory using atomic and "
           "then reads non atomically";
  }
};
