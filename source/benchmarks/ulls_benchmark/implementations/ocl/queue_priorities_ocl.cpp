...
static TestResult run(const QueuePrioritiesArguments &arguments,
  ...
  bool singleCcs = QueueFamiliesHelper::getQueueCountForEngineGroup(
                       opencl.device, EngineGroup::Compute) < 2;
  if (singleCcs && arguments.usePriorities == 0) {
    return TestResult::DeviceNotCapable;
  }

  const std::vector<uint8_t> kernelSource =
      FileHelper::loadTextFile("ulls_benchmark_eat_time.cl");
  ...
  // setup kernel time
  cl_uint kernelTime =
      static_cast<cl_uint>(arguments.lowPriorityKernelTime) * 7u;
  ASSERT_CL_SUCCESS(
      clSetKernelArg(lowPriorityKernel, 0, sizeof(kernelTime), &kernelTime));
  kernelTime = static_cast<cl_uint>(arguments.highPriorityKernelTime) * 7u;
  ASSERT_CL_SUCCESS(
      clSetKernelArg(highPriorityKernel, 0, sizeof(kernelTime), &kernelTime));

  cl_command_queue lowPriorityQueue;
  cl_command_queue highPriorityQueue;
  if (arguments.usePriorities) {
    cl_queue_properties lowPriorityProperties[] = {CL_QUEUE_FAMILY_INTEL,
                                                   0u,
    ...
    cl_queue_properties highPriorityProperties[] = {CL_QUEUE_FAMILY_INTEL,
                                                    0u,
                                                    CL_QUEUE_INDEX_INTEL,
                                                    singleCcs ? 0u : 1u,
                                                    CL_QUEUE_PRIORITY_KHR,
    ...
    lowPriorityQueue = clCreateCommandQueueWithProperties(
        opencl.context, opencl.device, lowPriorityProperties, &retVal);
    ASSERT_CL_SUCCESS(retVal);
    highPriorityQueue = clCreateCommandQueueWithProperties(
        opencl.context, opencl.device, highPriorityProperties, &retVal);
    ASSERT_CL_SUCCESS(retVal);
  } else {
    cl_queue_properties lowPriorityProperties[] = {CL_QUEUE_FAMILY_INTEL, 0u,
                                                   CL_QUEUE_INDEX_INTEL, 0, 0};

    cl_queue_properties highPriorityProperties[] = {CL_QUEUE_FAMILY_INTEL, 0u,
                                                    CL_QUEUE_INDEX_INTEL,
                                                    singleCcs ? 0u : 1u, 0};
  ...
  size_t lws = 64u;
  // Warmup, kernel
  ASSERT_CL_SUCCESS(clEnqueueNDRangeKernel(lowPriorityQueue, lowPriorityKernel,
                                           1, nullptr, &gws, &lws, 0, nullptr,
                                           nullptr));
  ASSERT_CL_SUCCESS(clFinish(lowPriorityQueue));
  ASSERT_CL_SUCCESS(retVal);
  ASSERT_CL_SUCCESS(clEnqueueNDRangeKernel(highPriorityQueue,
                                           highPriorityKernel, 1, nullptr, &gws,
  ...
  for (auto i = 0u; i < arguments.iterations; i++) {
    ASSERT_CL_SUCCESS(
        clEnqueueNDRangeKernel(lowPriorityQueue, lowPriorityKernel, 1, nullptr,
                               &gwsLowPriority, &lws, 0, nullptr, nullptr));
    ...
    ASSERT_CL_SUCCESS(clEnqueueNDRangeKernel(
        highPriorityQueue, highPriorityKernel, 1, nullptr, &gwsHighPriority,
        &lws, 0, nullptr, nullptr));
    ASSERT_CL_SUCCESS(clFinish(highPriorityQueue));
    timer.measureEnd();
    ASSERT_CL_SUCCESS(retVal);
...
static RegisterTestCaseImplementation<QueuePriorities>
