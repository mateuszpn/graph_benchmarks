...
bool MathOperationHelper::requiresIntelGlobalAtomicsExtension(
    MathOperation operation, DataType type) {
  const bool isFloat = type == DataType::Float;
  const bool isAdditionOrSubtraction =
      operation == MathOperation::Add || operation == MathOperation::Sub;
  return isFloat && isAdditionOrSubtraction;
}

bool MathOperationHelper::isSupportedAsAtomic(MathOperation operation,
                                              DataType type,
...
bool MathOperationHelper::isSupportedAsNormal(MathOperation operation,
                                              DataType type) {
  switch (operation) {
  case MathOperation::Unknown:
    FATAL_ERROR("Unknown math operation");
...
MathOperationTestData MathOperationHelper::generateTestData(
    DataType dataType, MathOperation operation, size_t loopIterations,
    size_t operationsPerLoop, size_t totalThreadsCount) {
  switch (dataType) {
  case DataType::Float:
    return MathOperationHelper::generateTestData<float>(
        operation, loopIterations, operationsPerLoop, totalThreadsCount);
  case DataType::Int32:
    return MathOperationHelper::generateTestData<int32_t>(
        operation, loopIterations, operationsPerLoop, totalThreadsCount);
  case DataType::Int64:
    return MathOperationHelper::generateTestData<int64_t>(
        operation, loopIterations, operationsPerLoop, totalThreadsCount);
  default:
