...
static TestResult run(const UsmFillArguments &arguments,
  ...
  if (arguments.patternContents == BufferContents::Random) {
    BufferContentsHelperOcl::fillWithRandomBytes(pattern.get(),
                                                 arguments.patternSize);
  }

  // Warmup
  ASSERT_CL_SUCCESS(clEnqueueMemFillINTEL(
      opencl.commandQueue, dstAlloc.ptr, pattern.get(), arguments.patternSize,
      arguments.bufferSize, 0, nullptr, nullptr));
  ...
  for (auto i = 0u; i < arguments.iterations; i++) {
    ...
    ASSERT_CL_SUCCESS(clEnqueueMemFillINTEL(
        opencl.commandQueue, dstAlloc.ptr, pattern.get(), arguments.patternSize,
        arguments.bufferSize, 0, nullptr, eventForEnqueue));
    ASSERT_CL_SUCCESS(clFinish(opencl.commandQueue));
    timer.measureEnd();
