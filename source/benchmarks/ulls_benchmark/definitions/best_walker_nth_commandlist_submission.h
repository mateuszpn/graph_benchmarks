...
struct BestWalkerNthCommandListSubmission
    : TestCase<BestWalkerNthCommandListSubmissionArguments> {
  ...
  std::string getHelp() const override {
    return "enqueues single kernel on n command lists, which updates system "
           "memory location and then busy-loops on CPU "
           "until the update of the kernel of nth command list becomes "
           "visible. This is L0 only test.";
  }
};
