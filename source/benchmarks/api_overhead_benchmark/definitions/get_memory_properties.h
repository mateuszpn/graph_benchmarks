...
struct GetMemoryProperties : TestCase<GetMemoryPropertiesArguments> {
  ...
  std::string getTestCaseName() const override { return "GetMemoryProperties"; }

  std::string getHelp() const override {
    return "measures time spent in zeMemGetAllocProperties on CPU when driver "
           "is queried for memory properties.";
  }
};
