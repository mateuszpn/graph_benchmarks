...
struct OneLocalAtomic : TestCase<OneLocalAtomicArguments> {
  ...
  std::string getTestCaseName() const override { return "OneLocalAtomic"; }

  std::string getHelp() const override {
    return "enqueues kernel performing an atomic operation on a single "
           "location placed in SLM";
  }
};
