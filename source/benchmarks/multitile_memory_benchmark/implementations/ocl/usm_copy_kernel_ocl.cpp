...
static TestResult run(const UsmCopyKernelArguments &arguments,
  ...
  // Create kernel
  const char *source =
      "kernel void copy_buffer(__global int *src, __global int *dst) { const "
      "uint gid = get_global_id(0);  dst[gid] = src[gid]; }";
...
static RegisterTestCaseImplementation<UsmCopyKernel>
