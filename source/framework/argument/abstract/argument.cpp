...
Argument::Argument(ArgumentContainer &parent, const std::string &key,
                   const std::string &extraHelp)
    : key(key), extraHelp(extraHelp) {
  parent.pushArgument(*this);
}

const std::string Argument::getKey() const { return key; }
...
std::string Argument::getExtraHelp() const { return extraHelp; }
...
void Argument::markAsParsed() { this->parsed = true; }
