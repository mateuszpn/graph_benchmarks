...
struct UsmSharedMigrateCpuArguments : TestCaseArgumentContainer {
  ...
  UsmSharedMigrateCpuArguments()
      : accessAllBytes(*this, "accessAllBytes",
...
struct UsmSharedMigrateCpu : TestCase<UsmSharedMigrateCpuArguments> {
  ...
  std::string getTestCaseName() const override { return "UsmSharedMigrateCpu"; }

  std::string getHelp() const override {
    return "allocates a unified shared memory buffer and measures bandwidth "
           "for kernel that must migrate resource from GPU to CPU";
  }
};
