...
static TestResult run(const SlmTrafficArguments &arguments,
                      Statistics &statistics) {
  MeasurementFields typeSelector(MeasurementUnit::Latency,
                                 MeasurementType::Gpu);
  ...
  ASSERT_CL_SUCCESS(clGetDeviceInfo(opencl.device,
                                    CL_DEVICE_MAX_CLOCK_FREQUENCY,
                                    sizeof(gpuFreq), &gpuFreq, nullptr));
  ...
  const cl_mem source = clCreateBuffer(
      opencl.context, memFlagsIn, inOutBuffSize, hostInBuffer.get(), &retVal);
  ASSERT_CL_SUCCESS(retVal);

  const cl_mem destination = clCreateBuffer(opencl.context, memFlagsOut,
                                            inOutBuffSize, nullptr, &retVal);
  ASSERT_CL_SUCCESS(retVal);

  const size_t sharedDataSize = 256UL;

  const size_t slmInitData =
      (size_t)(sharedDataSize * numDataGroups * sizeof(float));
  ...
  const cl_mem slmInitBuff = clCreateBuffer(
      opencl.context, memFlagsIn, slmInitData, hostBuffTable.get(), &retVal);
  ASSERT_CL_SUCCESS(retVal);
  ...
  if (!buildFromFile) {
    programSlmRead =
        ...
        "\n#endif"
        "\n"
        "\ninline uint4 loadSlmSharedData(__local uint* pSharedData, const "
        "uint4 x, const uint y) {"
        "\n    const uint  offset = y * LOCAL_SIZE;"
        "\n    const uint4 i = x + offset;"
        "\n    return (uint4)(pSharedData[i.x], pSharedData[i.y], "
        "pSharedData[i.z], pSharedData[i.w]);"
        "\n}"
        "\n"
        "\n__attribute__((intel_reqd_sub_group_size(16)))"
        "\n__kernel void slmRead_latencyTest(__global const uint4* restrict "
        "src, __global uint4* restrict dst,"
        "\n    __global const uint* restrict pSharedData, const uint "
        "innerMaxLoops)"
        "\n{"
        "\n    const size_t x = get_global_id(0);"
        "\n    __local uint  localBuffer[SLM_MAX_SIZE];"
        "\n    uint4 perfData = (uint4)(0, (uint)-1, 0, 0);  //avg, min, max, "
        "isOverflow"
        "\n    const uint lid = get_local_id(0);"
        "\n    if (lid < LOCAL_SIZE) {"
        "\n        const uint slmBuffTile = min((uint)get_local_size(0), "
        "(uint)LOCAL_SIZE);"
        ...
        "\n        }"
        "\n        perfData[0] = ((perfData[0] * j) + (uint)time)/(j+1);     "
        "//cumulative average"
        "\n        perfData[1] = min(perfData[1], (uint)time);               "
        "//min"
        "\n        perfData[2] = max(perfData[2], (uint)time);               "
        "//max"
        "\n    }"
        "\n    if(innerMaxLoops > 0xFF) {"
        "\n        dst[x] = data; }"
    ...
    const auto programSrc =
        FileHelper::loadBinaryFile("slmRead_CryptKernelBased.cl");
  ...
  if (TestResult result = ProgramHelperOcl::buildProgramFromSource(
          opencl.context, opencl.device, programSlmRead, programSize,
          buildOptions.str().c_str(), program);
  ...
  ASSERT_CL_SUCCESS(
      clSetKernelArg(kernel, 2, sizeof(slmInitBuff), &slmInitBuff));
  ...
  for (auto i = 0u; i < arguments.iterations; i++) {
    ...
    retVal |= clEnqueueReadBuffer(opencl.commandQueue, destination, true, 0,
                                  dataReadSize, slmClocksBuffer.get(), 0,
                                  nullptr, nullptr);
    ...
    for (size_t item = 0; item < gws; item++) {
      size_t avgInThread =
          static_cast<size_t>(slmClocksBuffer.get()[(item * vectorSize)]);
      size_t minInThread =
          static_cast<size_t>(slmClocksBuffer.get()[(item * vectorSize) + 1]);
      size_t maxInThread =
          static_cast<size_t>(slmClocksBuffer.get()[(item * vectorSize) + 2]);
    ...
    std::chrono::steady_clock::duration averageLatency =
        static_cast<std::chrono::steady_clock::duration>(
            std::accumulate(vecPerThreadAverages.begin(),
                            vecPerThreadAverages.end(), (size_t)0) /
            vecPerThreadAverages.size());

    if (printPerEnqueueStats) {
      size_t minInAllThreads =
          *std::min_element(vecPerThreadMins.begin(), vecPerThreadMins.end());
      size_t maxInAllThreads =
          *std::max_element(vecPerThreadMax.begin(), vecPerThreadMax.end());
      std::cout << "[" << i << "] Average Latency "
                << static_cast<size_t>(
                       std::accumulate(vecPerThreadAverages.begin(),
                                       vecPerThreadAverages.end(), (size_t)0) /
                       vecPerThreadAverages.size())
                << " [clk]  min: " << minInAllThreads
                << " [clk]  max: " << maxInAllThreads
                << " [clk] (SLM uint4 load.slm.d32)" << std::endl;
    }
    statistics.pushValue(averageLatency, typeSelector.getUnit(),
...
static RegisterTestCaseImplementation<SlmTraffic> registerTestCase(run,
                                                                   Api::OpenCL);
