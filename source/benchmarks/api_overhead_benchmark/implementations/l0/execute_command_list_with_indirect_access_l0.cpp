...
static TestResult
run(const ExecuteCommandListWithIndirectAccessArguments &arguments,
  ...
  auto spirvModule = FileHelper::loadBinaryFile(
      "api_overhead_benchmark_indirect_access_kernel.spv");
  ...
  ASSERT_ZE_RESULT_SUCCESS(zeKernelSetIndirectAccess(
      kernel, ZE_KERNEL_INDIRECT_ACCESS_FLAG_HOST |
  ...
  for (uint32_t i = 0; i < arguments.IndirectAllocationsAmount; i++) {
    ...
    ASSERT_ZE_RESULT_SUCCESS(zeMemAllocHost(
        levelzero.context, &hostAllocationDesc, sizeof(int32_t), 4u, &ptr));
  ...
  for (auto &allocation : indirectAllocations) {
    *allocation = 0;
    const ze_host_mem_alloc_desc_t hostAllocationDesc{
        ZE_STRUCTURE_TYPE_HOST_MEM_ALLOC_DESC};
    void *ptr = nullptr;
    ASSERT_ZE_RESULT_SUCCESS(zeMemAllocHost(levelzero.context,
                                            &hostAllocationDesc,
                                            sizeof(st_container), 4u, &ptr));
  ...
  ASSERT_ZE_RESULT_SUCCESS(zeKernelSetArgumentValue(
      kernel, 0, sizeof(st_container *), &wrappedIndirectAllocations.back()));
  ...
  // Validate warmup
  EXPECT_EQ(arguments.IndirectAllocationsAmount,
            *wrappedIndirectAllocations.at(0)->value);
  ...
  *wrappedIndirectAllocations.at(0)->value = 0;
  ...
  for (auto i = 0u; i < arguments.iterations; i++) {
    ...
    if (arguments.AllocateMemory) {
      const ze_device_mem_alloc_desc_t deviceAllocationDescriptor{
          ZE_STRUCTURE_TYPE_DEVICE_MEM_ALLOC_DESC};
      ASSERT_ZE_RESULT_SUCCESS(
          zeMemAllocDevice(levelzero.context, &deviceAllocationDescriptor, 4u,
                           4u, levelzero.device, &temporaryPtr));
      ASSERT_ZE_RESULT_SUCCESS(zeContextMakeMemoryResident(
          levelzero.context, levelzero.device, temporaryPtr, 4u));
    }
  ...
  for (uint32_t i = 0; i < arguments.IndirectAllocationsAmount; i++) {
    ASSERT_ZE_RESULT_SUCCESS(
        zeMemFree(levelzero.context, wrappedIndirectAllocations[i]->value));
    ASSERT_ZE_RESULT_SUCCESS(
        zeMemFree(levelzero.context, wrappedIndirectAllocations[i]));
  }
...
static RegisterTestCaseImplementation<ExecuteCommandListWithIndirectAccess>
