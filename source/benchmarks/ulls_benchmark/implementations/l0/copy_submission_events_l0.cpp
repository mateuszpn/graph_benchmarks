...
static TestResult run(const CopySubmissionEventsArguments &arguments,
  ...
  auto queueDesc = QueueFamiliesHelper::getPropertiesForSelectingEngine(
      levelzero.device, arguments.engine);
  if (nullptr == queueDesc) {
    return TestResult::DeviceNotCapable;
  }
  levelzero.commandQueue =
      levelzero.createQueue(levelzero.device, queueDesc->desc);
  ...
  const uint32_t timestampValidBits =
      levelzero.getTimestampValidBits(levelzero.device);
  const uint32_t kernelTimestampValidBits =
      levelzero.getKernelTimestampValidBits(levelzero.device);
  const uint32_t sharedTimestampValidBits =
      std::min(timestampValidBits, kernelTimestampValidBits);
  ...
  ASSERT_ZE_RESULT_SUCCESS(zeMemAllocHost(levelzero.context, &allocationDesc,
                                          bufferSize, 0, &hostMemory));

  const ze_device_mem_alloc_desc_t allocationDescDevice{
  ...
  ASSERT_ZE_RESULT_SUCCESS(zeMemAllocDevice(levelzero.context,
                                            &allocationDescDevice, bufferSize,
                                            0, levelzero.device, &destination));
  ...
  ASSERT_ZE_RESULT_SUCCESS(zeCommandListAppendMemoryCopy(
      cmdList, destination, hostMemory, bufferSize, hEvent, 0, nullptr));
  ASSERT_ZE_RESULT_SUCCESS(zeCommandListClose(cmdList));

  // Warmup
  uint64_t hostEnqueueTimestamp = 0;
  uint64_t deviceEnqueueTimestamp = 0;
  ASSERT_ZE_RESULT_SUCCESS(zeDeviceGetGlobalTimestamps(
      levelzero.device, &hostEnqueueTimestamp, &deviceEnqueueTimestamp));
  ...
  ASSERT_ZE_RESULT_SUCCESS(
      zeEventQueryKernelTimestamp(hEvent, &deviceStartTimestamp));

  uint64_t truncatedDeviceEnqueueTimestamp = BitHelper::isolateLowerNBits(
      deviceEnqueueTimestamp, sharedTimestampValidBits);
  uint64_t truncatedKernelStartTimestamp = BitHelper::isolateLowerNBits(
      deviceStartTimestamp.global.kernelStart, sharedTimestampValidBits);
  ...
  for (auto i = 0u; i < arguments.iterations; i++) {
    ...
    ASSERT_ZE_RESULT_SUCCESS(
        zeEventQueryKernelTimestamp(hEvent, &deviceStartTimestamp));
    truncatedDeviceEnqueueTimestamp = BitHelper::isolateLowerNBits(
        deviceEnqueueTimestamp, sharedTimestampValidBits);
    truncatedKernelStartTimestamp = BitHelper::isolateLowerNBits(
        deviceStartTimestamp.global.kernelStart, sharedTimestampValidBits);

    std::chrono::nanoseconds submissionTime =
        levelzero.getAbsoluteSubmissionTime(truncatedKernelStartTimestamp,
                                            truncatedDeviceEnqueueTimestamp,
                                            timerResolution);

    statistics.pushValue(submissionTime, typeSelector.getUnit(),
...
static RegisterTestCaseImplementation<CopySubmissionEvents>
