...
struct DeviceSelectionHelper {
  const static inline DeviceSelection devices[] = {
      DeviceSelection::Host,  DeviceSelection::Root,  DeviceSelection::Tile0,
      DeviceSelection::Tile1, DeviceSelection::Tile2, DeviceSelection::Tile3};
  const static inline DeviceSelection subDevices[] = {
      DeviceSelection::Tile0, DeviceSelection::Tile1, DeviceSelection::Tile2,
      DeviceSelection::Tile3};
  ...
  static size_t getMaxSubDeviceIndex(DeviceSelection deviceSelection) {
    ...
    FATAL_ERROR_IF(
        !foundSubDevice,
        "Cannot call getMaxSubDeviceIndex, when there aren't any subDevices");
    return maxIndex;
  }

  static bool hasDevice(DeviceSelection deviceSelection,
                        DeviceSelection device) {
    FATAL_ERROR_IF(device == DeviceSelection::Unknown,
                   "Cannot check for unknown device");
