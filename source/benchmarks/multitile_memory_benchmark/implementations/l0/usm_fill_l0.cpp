...
static TestResult run(const UsmFillArguments &arguments,
  ...
  if (levelzero.context == nullptr || levelzero.commandQueue == nullptr ||
      arguments.patternSize > levelzero.commandQueueMaxFillSize) {
  ...
  ASSERT_ZE_RESULT_SUCCESS(UsmHelper::allocate(
      arguments.bufferPlacement, levelzero, arguments.size, &buffer));
  ...
  for (DeviceSelection device :
       DeviceSelectionHelper::split(arguments.contextPlacement)) {
    ASSERT_ZE_RESULT_SUCCESS(zeContextMakeMemoryResident(
        levelzero.context, levelzero.getDevice(device), buffer,
        arguments.size));
  }
  ...
  ASSERT_ZE_RESULT_SUCCESS(zeCommandListAppendMemoryFill(
      cmdList, buffer, pattern.get(), arguments.patternSize, arguments.size,
      event, 0, nullptr));
  ...
  for (DeviceSelection device :
       DeviceSelectionHelper::split(arguments.contextPlacement)) {
    ASSERT_ZE_RESULT_SUCCESS(zeContextEvictMemory(levelzero.context,
                                                  levelzero.getDevice(device),
                                                  buffer, arguments.size));
  }
