...
struct UsmCopyRegion : TestCase<UsmCopyRegionArguments> {
  ...
  std::string getTestCaseName() const override { return "UsmCopyRegion"; }

  std::string getHelp() const override {
    return "allocates two unified shared memory buffers and measures region "
           "copy bandwidth between them using immediate command list.";
  }
};
