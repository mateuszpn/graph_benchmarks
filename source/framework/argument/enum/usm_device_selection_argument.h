...
struct DeviceSelectionArgumentBaseTraits {
  ...
  const static inline EnumType enumValues[6] = {
      EnumType::Host,  EnumType::Root,  EnumType::Tile0,
      EnumType::Tile1, EnumType::Tile2, EnumType::Tile3};
  const static inline std::string enumValuesNames[6] = {
      "Host", "Root", "Tile0", "Tile1", "Tile2", "Tile3"};
};

template <bool allowHost, bool allowDevice, bool allowShared>
struct UsmDeviceSelectionArgumentBase
    : BitfieldEnumArgument<DeviceSelectionArgumentBaseTraits, DeviceSelection> {
  ...
  bool validateExtra() const override {
    ...
    const auto gpuDevicesCount =
        DeviceSelectionHelper::getDevicesCount(gpuDevices);
    if (gpuDevicesCount > 1) {
      return false;
    }

    const bool hasHost =
        DeviceSelectionHelper::hasDevice(this->value, DeviceSelection::Host);
...
using UsmDeviceSelectionArgument =
    UsmDeviceSelectionArgumentBase<true, true, true>;
using UsmSharedDeviceSelectionArgument =
    UsmDeviceSelectionArgumentBase<false, false, true>;
