...
static TestResult run(const CopyImageToBufferArguments &arguments,
  ...
  cl_mem srcImage = clCreateImage(opencl.context, 0, &imageFormat,
                                  &imageDescription, nullptr, &retVal);
  ASSERT_CL_SUCCESS(retVal);
  cl_mem dstBuffer = clCreateBuffer(opencl.context, CL_MEM_READ_WRITE,
  ...
  ASSERT_CL_SUCCESS(clEnqueueCopyImageToBuffer(opencl.commandQueue, srcImage,
                                               dstBuffer, origin, region, 0, 0,
  ...
  for (auto i = 0u; i < arguments.iterations; i++) {
    timer.measureStart();
    ASSERT_CL_SUCCESS(clEnqueueCopyImageToBuffer(opencl.commandQueue, srcImage,
                                                 dstBuffer, origin, region, 0,
                                                 0, nullptr, eventForEnqueue));
    ASSERT_CL_SUCCESS(clFinish(opencl.commandQueue));
    timer.measureEnd();
...
static RegisterTestCaseImplementation<CopyImageToBuffer>
