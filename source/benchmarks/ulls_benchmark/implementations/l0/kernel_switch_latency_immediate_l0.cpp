...
static TestResult run(const KernelSwitchLatencyImmediateArguments &arguments,
  ...
  if (!arguments.useProfiling &&
      (!arguments.inOrder || !arguments.counterBasedEvents)) {
    return TestResult::ApiNotCapable;
  }
  ...
  ASSERT_ZE_RESULT_SUCCESS(
      zeKernelSetGroupSize(kernel, static_cast<uint32_t>(lws), 1u, 1u));
  int kernelOperationsCount =
      static_cast<int>(arguments.kernelExecutionTime * 8);
  ASSERT_ZE_RESULT_SUCCESS(
      zeKernelSetArgumentValue(kernel, 0, sizeof(int), &kernelOperationsCount));
  ...
  const ze_event_pool_counter_based_exp_desc_t counterBasedDesc{
      ZE_STRUCTURE_TYPE_COUNTER_BASED_EVENT_POOL_EXP_DESC, nullptr,
      ZE_EVENT_POOL_COUNTER_BASED_EXP_FLAG_IMMEDIATE};
  const ze_event_pool_desc_t eventPoolDesc{
      ZE_STRUCTURE_TYPE_EVENT_POOL_DESC,
      arguments.counterBasedEvents ? &counterBasedDesc : nullptr, flags,
      static_cast<uint32_t>(arguments.kernelCount)};

  uint32_t numDevices = 1;
  ze_event_pool_handle_t hEventPool;
  ASSERT_ZE_RESULT_SUCCESS(zeEventPoolCreate(levelzero.context, &eventPoolDesc,
                                             numDevices, &levelzero.device,
                                             &hEventPool));

  std::vector<ze_event_handle_t> profilingEvents(arguments.kernelCount);
  for (auto i = 0u; i < arguments.kernelCount; i++) {
    ze_event_desc_t eventDesc = {ZE_STRUCTURE_TYPE_EVENT_DESC, nullptr, i, 0,
                                 0};
    ASSERT_ZE_RESULT_SUCCESS(
        zeEventCreate(hEventPool, &eventDesc, &profilingEvents[i]));
  }
  Timer timer;
  ...
  if (!arguments.useProfiling) {
    ...
    const ze_event_pool_desc_t profilingEventPoolDesc{
        ZE_STRUCTURE_TYPE_EVENT_POOL_DESC,
        arguments.counterBasedEvents ? &counterBasedDesc : nullptr,
        profilingFlags, 1u};
    ASSERT_ZE_RESULT_SUCCESS(
        zeEventPoolCreate(levelzero.context, &profilingEventPoolDesc,
                          numDevices, &levelzero.device, &profilingEventPool));
    ze_event_desc_t eventDesc = {ZE_STRUCTURE_TYPE_EVENT_DESC, nullptr, 0u, 0,
                                 0};
    ze_event_handle_t eventHandle = {0u};
    ASSERT_ZE_RESULT_SUCCESS(
        zeEventCreate(profilingEventPool, &eventDesc, &eventHandle));

    for (auto j = 0u; j < arguments.kernelCount + 1; j++) {
      ASSERT_ZE_RESULT_SUCCESS(zeCommandListAppendLaunchKernel(
          cmdList, kernel, &groupCount, eventHandle, 0, nullptr));
      ASSERT_ZE_RESULT_SUCCESS(zeEventHostSynchronize(
          eventHandle, std::numeric_limits<uint64_t>::max()));
      ze_kernel_timestamp_result_t eventTimestamps;
      ASSERT_ZE_RESULT_SUCCESS(
          zeEventQueryKernelTimestamp(eventHandle, &eventTimestamps));
      // first run is a warmup
      if (j > 0) {
        kernelsTime +=
            std::chrono::nanoseconds((eventTimestamps.global.kernelEnd -
                                      eventTimestamps.global.kernelStart) *
                                     timerResolution);
      }
  ...
  for (auto iteration = 0u; iteration < arguments.iterations; ++iteration) {
    timer.measureStart();
    ASSERT_ZE_RESULT_SUCCESS(zeCommandListAppendLaunchKernel(
        cmdList, kernel, &groupCount, profilingEvents[0], 0, nullptr));
    for (auto j = 1u; j < arguments.kernelCount; j++) {
      if (arguments.barrier) {
        ASSERT_ZE_RESULT_SUCCESS(
            zeCommandListAppendBarrier(cmdList, nullptr, 0u, nullptr));
        ASSERT_ZE_RESULT_SUCCESS(zeCommandListAppendLaunchKernel(
            cmdList, kernel, &groupCount, profilingEvents[j], 0, nullptr));
      } else {
        ASSERT_ZE_RESULT_SUCCESS(zeCommandListAppendLaunchKernel(
            cmdList, kernel, &groupCount, profilingEvents[j], 1,
            &profilingEvents[j - 1]));
      }
    }

    ASSERT_ZE_RESULT_SUCCESS(
        zeEventHostSynchronize(profilingEvents[arguments.kernelCount - 1],
    ...
    if (arguments.useProfiling) {
      for (auto j = 1u; j < arguments.kernelCount; j++) {
        ...
        ASSERT_ZE_RESULT_SUCCESS(zeEventQueryKernelTimestamp(
            profilingEvents[j - 1], &earlierKernelTimestamp));
        ze_kernel_timestamp_result_t laterKernelTimestamp;
        ASSERT_ZE_RESULT_SUCCESS(zeEventQueryKernelTimestamp(
            profilingEvents[j], &laterKernelTimestamp));

        switchTime +=
            std::chrono::nanoseconds((laterKernelTimestamp.global.kernelStart -
                                      earlierKernelTimestamp.global.kernelEnd) *
                                     timerResolution);
      }
    } else {
    ...
    statistics.pushValue(switchTime / (arguments.kernelCount - 1),
...
static RegisterTestCaseImplementation<KernelSwitchLatencyImmediate>
