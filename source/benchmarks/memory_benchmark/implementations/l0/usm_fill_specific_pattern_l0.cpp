...
static TestResult run(const UsmFillSpecificPatternArguments &arguments,
  ...
  if (levelzero.commandQueue == nullptr ||
      pattern.size() > levelzero.commandQueueMaxFillSize) {
  ...
  ASSERT_ZE_RESULT_SUCCESS(UsmHelper::allocate(
      arguments.usmMemoryPlacement, levelzero, arguments.bufferSize, &buffer));
  ASSERT_ZE_RESULT_SUCCESS(zeContextMakeMemoryResident(
      levelzero.context, levelzero.device, buffer, arguments.bufferSize));
  ...
  if (requiresImport(arguments.usmMemoryPlacement)) {
    ...
    ASSERT_ZE_RESULT_SUCCESS(zeContextMakeMemoryResident(
        levelzero.context, levelzero.device, patternAddress, pattern.size()));
  }
  ...
  ASSERT_ZE_RESULT_SUCCESS(zeCommandListAppendMemoryFill(
      cmdList, buffer, pattern.data(), pattern.size(), arguments.bufferSize,
      event, 0, nullptr));
  ...
  for (auto i = 0u; i < arguments.iterations; i++) {
    ASSERT_ZE_RESULT_SUCCESS(BufferContentsHelperL0::fillBuffer(
        levelzero, buffer, arguments.bufferSize, arguments.contents, false))
  ...
  ASSERT_ZE_RESULT_SUCCESS(
      UsmHelper::deallocate(arguments.usmMemoryPlacement, levelzero, buffer));
  if (requiresImport(arguments.usmMemoryPlacement)) {
    ASSERT_ZE_RESULT_SUCCESS(zeContextEvictMemory(
        levelzero.context, levelzero.device, patternAddress, pattern.size()));
...
static RegisterTestCaseImplementation<UsmFillSpecificPattern>
