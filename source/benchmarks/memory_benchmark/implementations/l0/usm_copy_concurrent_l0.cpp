...
static TestResult run(const UsmConcurrentCopyArguments &arguments,
  ...
  auto h2dQueueDesc = QueueFamiliesHelper::getPropertiesForSelectingEngine(
      levelzero.device, arguments.h2dEngine);
  if (nullptr == h2dQueueDesc) {
    return TestResult::DeviceNotCapable;
  }

  auto d2hQueueDesc = QueueFamiliesHelper::getPropertiesForSelectingEngine(
      levelzero.device, arguments.d2hEngine);
  ...
  ASSERT_ZE_RESULT_SUCCESS(
      zeCommandListCreateImmediate(levelzero.context, levelzero.device,
                                   &h2dQueueDesc->desc, &h2dCommandList));
  ASSERT_ZE_RESULT_SUCCESS(
      zeCommandListCreateImmediate(levelzero.context, levelzero.device,
                                   &d2hQueueDesc->desc, &d2hCommandList));
  ...
  ASSERT_ZE_RESULT_SUCCESS(UsmHelper::allocate(
      UsmMemoryPlacement::Host, levelzero, arguments.size, &host1));
  ASSERT_ZE_RESULT_SUCCESS(UsmHelper::allocate(
      UsmMemoryPlacement::Device, levelzero, arguments.size, &device1));
  ASSERT_ZE_RESULT_SUCCESS(UsmHelper::allocate(
      UsmMemoryPlacement::Host, levelzero, arguments.size, &host2));
  ASSERT_ZE_RESULT_SUCCESS(UsmHelper::allocate(
      UsmMemoryPlacement::Device, levelzero, arguments.size, &device2));

  // Warmup
  ASSERT_ZE_RESULT_SUCCESS(zeCommandListAppendMemoryCopy(
      h2dCommandList, device1, host1, arguments.size, h2dEvent, 1, &waitEvent));
  ASSERT_ZE_RESULT_SUCCESS(zeCommandListAppendMemoryCopy(
      d2hCommandList, host2, device2, arguments.size, d2hEvent, 1, &waitEvent));

  ASSERT_ZE_RESULT_SUCCESS(zeEventHostSignal(waitEvent));
  ASSERT_ZE_RESULT_SUCCESS(
      zeEventHostSynchronize(h2dEvent, std::numeric_limits<uint64_t>::max()));
  ASSERT_ZE_RESULT_SUCCESS(
      zeEventHostSynchronize(d2hEvent, std::numeric_limits<uint64_t>::max()));
  ...
  for (auto i = 0u; i < arguments.iterations; i++) {
    ...
    ASSERT_ZE_RESULT_SUCCESS(
        zeCommandListAppendMemoryCopy(h2dCommandList, device1, host1,
                                      arguments.size, h2dEvent, 1, &waitEvent));
    ASSERT_ZE_RESULT_SUCCESS(
        zeCommandListAppendMemoryCopy(d2hCommandList, host2, device2,
                                      arguments.size, d2hEvent, 1, &waitEvent));
    timer.measureStart();
    ASSERT_ZE_RESULT_SUCCESS(zeEventHostSignal(waitEvent));
    ASSERT_ZE_RESULT_SUCCESS(
        zeEventHostSynchronize(h2dEvent, std::numeric_limits<uint64_t>::max()));
...
static RegisterTestCaseImplementation<UsmConcurrentCopy>
