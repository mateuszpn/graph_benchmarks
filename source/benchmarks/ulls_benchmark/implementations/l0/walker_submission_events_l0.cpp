...
static TestResult run(const WalkerSubmissionEventsArguments &arguments,
  ...
  // Create kernel
  auto spirvModule =
      FileHelper::loadBinaryFile("ulls_benchmark_empty_kernel.spv");
  ...
  ASSERT_ZE_RESULT_SUCCESS(zeCommandListAppendLaunchKernel(
      cmdList, kernel, &dispatchTraits, hEvent, 0, nullptr));
  ...
  ASSERT_ZE_RESULT_SUCCESS(
      zeEventQueryKernelTimestamp(hEvent, &kernelTimestamp));
  ...
  uint64_t truncatedKernelStartTimestamp = BitHelper::isolateLowerNBits(
      kernelTimestamp.global.kernelStart, sharedTimestampValidBits);
...
static RegisterTestCaseImplementation<WalkerSubmissionEvents>
