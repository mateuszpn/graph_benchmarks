...
struct StreamAfterTransfer : TestCase<StreamAfterTransferArguments> {
  ...
  std::string getTestCaseName() const override { return "StreamAfterTransfer"; }

  std::string getHelp() const override {
    return "Goal of this test is to measure how stream kernels perform right "
           "after host to device transfer populating the data. "
           "Test does clean caches, then emits transfers and then follows with "
           "stream kernel and measures GPU execution time of it.";
  }
};
