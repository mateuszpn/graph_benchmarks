...
struct AppendWaitOnEventsImmediateArguments : TestCaseArgumentContainer {
  ...
  AppendWaitOnEventsImmediateArguments()
      : eventSignaled(*this, "eventSignaled",
                      "Event is already signaled before "
                      "zeCommandListAppendWaitOnEvents call"),
        useIoq(*this, "ioq", "Use In order queue") {}
};

struct AppendWaitOnEventsImmediate
    : TestCase<AppendWaitOnEventsImmediateArguments> {
  ...
  std::string getHelp() const override {
    return "Measures time spent to zeCommandListAppendWaitOnEvents using "
