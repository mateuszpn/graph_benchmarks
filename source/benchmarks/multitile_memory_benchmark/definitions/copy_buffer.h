...
struct CopyBufferArguments : TestCaseArgumentContainer {
  ...
  CopyBufferArguments()
      : contextPlacement(*this, "context", "How context will be created"),
        queuePlacement(
            *this, "queue",
            "Which device within the context will perform the operation"),
        srcPlacement(*this, "src", "Placement of memory for the source buffer"),
        dstPlacement(*this, "dst",
                     "Placement of memory for the destination buffer"),
        size(*this, "size", "Size of the buffers"),
        srcCompressed(*this, "srcCompressed",
                      CommonHelpMessage::compression("source buffer")),
        dstCompressed(*this, "dstCompressed",
                      CommonHelpMessage::compression("destination buffer")),
...
struct CopyBuffer : TestCase<CopyBufferArguments> {
  ...
  std::string getTestCaseName() const override { return "CopyBuffer"; }

  std::string getHelp() const override {
    return "allocates two OpenCL buffers and measures copy bandwidth between "
           "them. Buffers "
