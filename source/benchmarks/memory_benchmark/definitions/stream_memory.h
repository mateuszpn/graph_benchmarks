...
struct StreamMemoryArguments : TestCaseArgumentContainer {
  ...
  StreamMemoryArguments()
      : type(*this, "type", "Memory streaming type"),
        ...
        memoryPlacement(*this, "memoryPlacement",
                        "Memory type used for stream"),
        partialMultiplier(*this, "multiplier",
                          "multiplies id used for accessing the resources to "
                          "simulate partials") {}
};
...
struct StreamMemory : TestCase<StreamMemoryArguments> {
  ...
  std::string getTestCaseName() const override { return "StreamMemory"; }

  std::string getHelp() const override {
    return "Streams memory inside of kernel in a fashion described by 'type'. "
           "Copy means one "
           "memory location is read from and the second one is written to. "
           "Triad means two "
