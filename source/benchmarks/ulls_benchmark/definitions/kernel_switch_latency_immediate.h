  ...
  KernelSwitchLatencyImmediateArguments()
      : kernelCount(*this, "count", "Count of kernels"),
        ...
        barrier(*this, "barrier",
                "synchronization with barrier instead of events"),
        hostVisible(*this, "cpuVisible", "events are with host visible flag"),
        inOrder(*this, "ioq", "use in order queue/command list"),
        counterBasedEvents(*this, "ctrEvents",
                           "use counter based events for in order"),
        useProfiling(*this, "profiling",
                     "use profiling to obtain switch time") {}
};

struct KernelSwitchLatencyImmediate
    : TestCase<KernelSwitchLatencyImmediateArguments> {
  using TestCase<KernelSwitchLatencyImmediateArguments>::TestCase;

  std::string getTestCaseName() const override { return "KernelSwitchImm"; }

  std::string getHelp() const override {
    return "measures time from end of one kernel till start of next kernel "
           "using immediate command lists";
  }
};
