...
#define ASSERT_ZE_RESULT_SUCCESS(retVal)                                       \
  {                                                                            \
    ...
    if (tempVarForDefine != ZE_RESULT_SUCCESS) {                               \
      NON_FATAL_ERROR("ASSERT_ZE_RESULT_SUCCESS", #retVal,                     \
                      std::to_string(tempVarForDefine).c_str(),                \
                      l0ErrorToString(tempVarForDefine));                      \
...
#define EXPECT_ZE_RESULT_SUCCESS(retVal)                                       \
  {                                                                            \
    const ze_result_t tempVarForDefine = (retVal);                             \
    if (tempVarForDefine != ZE_RESULT_SUCCESS) {                               \
      NON_FATAL_ERROR("EXPECT_ZE_RESULT_SUCCESS", #retVal,                     \
                      std::to_string(tempVarForDefine).c_str(),                \
                      l0ErrorToString(tempVarForDefine));                      \
    }                                                                          \
  }
...
#define ZE_RESULT_SUCCESS_OR_ERROR(retVal)                                     \
  {                                                                            \
    ...
    if (tempVarForDefine != ZE_RESULT_SUCCESS) {                               \
      const auto _message =                                                    \
          std::string("Fatal LevelZero error occurred, retVal=") +             \
