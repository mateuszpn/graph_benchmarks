...
static TestResult run(const UsmCopyArguments &arguments,
  ...
  if (!clCreateBufferWithPropertiesINTEL ||
      !opencl.getExtensions().isUsmSupported()) {
    return TestResult::DriverFunctionNotFound;
  }
  Timer timer;

  // Create buffer
  void *src = UsmHelperOcl::allocate(arguments.srcPlacement, opencl,
                                     arguments.size, &retVal);
  ASSERT_CL_SUCCESS(retVal);
  void *dst = UsmHelperOcl::allocate(arguments.dstPlacement, opencl,
                                     arguments.size, &retVal);
  ASSERT_CL_SUCCESS(retVal);

  // Warmup
  ASSERT_CL_SUCCESS(clEnqueueMemcpyINTEL(opencl.commandQueue, CL_FALSE, dst,
                                         src, arguments.size, 0, nullptr,
                                         nullptr));
