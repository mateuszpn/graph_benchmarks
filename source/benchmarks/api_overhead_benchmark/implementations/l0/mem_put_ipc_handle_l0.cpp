...
static TestResult run(const MemPutIpcHandleArguments &arguments,
  ...
  for (auto i = 0u; i < arguments.iterations; i++) {
    ...
    for (int64_t k = 0; k < arguments.AllocationsCount; k++) {
      ASSERT_ZE_RESULT_SUCCESS(
          zeMemPutIpcHandle(levelzero.context, ipcHandles[k]));
    }
    timer.measureEnd();
...
static RegisterTestCaseImplementation<MemPutIpcHandle>
