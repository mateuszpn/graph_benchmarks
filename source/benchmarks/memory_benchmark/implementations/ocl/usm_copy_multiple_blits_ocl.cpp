...
static TestResult run(const UsmCopyMultipleBlitsArguments &arguments,
  ...
  for (size_t blitterIndex : arguments.blitters.getEnabledBits()) {
    ...
    if (!QueueFamiliesHelper::validateCapability(
            queue, CL_QUEUE_CAPABILITY_TRANSFER_BUFFER_INTEL)) {
      return TestResult::DeviceNotCapable;
    }
  ...
  for (PerQueueData &queue : queues) {
    ASSERT_CL_SUCCESS(clEnqueueMemcpyINTEL(queue.queue, CL_FALSE, queue.copyDst,
                                           queue.copySrc, queue.copySize, 0,
                                           nullptr, nullptr));
  ...
  for (auto i = 0u; i < arguments.iterations; i++) {
    ...
    for (PerQueueData &queue : queues) {
      ASSERT_CL_SUCCESS(clEnqueueMemcpyINTEL(
          queue.queue, CL_FALSE, queue.copyDst, queue.copySrc, queue.copySize,
          0, nullptr, &queue.event));
    }
    for (PerQueueData &queue : queues) {
      ASSERT_CL_SUCCESS(clFlush(queue.queue));
    }
    ...
    for (PerQueueData &queue : queues) {
      ...
      statistics.pushValue(std::chrono::nanoseconds(timeNs), queue.copySize,
