...
static TestResult run(const UsmSharedMigrateGpuForFillArguments &arguments,
  ...
  ASSERT_ZE_RESULT_SUCCESS(zeMemAllocShared(
      ...
      arguments.bufferSize, 0, levelzero.device, &buffer));
  ...
  if (arguments.prefetchMemory) {
    ASSERT_ZE_RESULT_SUCCESS(zeCommandListAppendMemoryPrefetch(
        cmdList, bufferInt, arguments.bufferSize));
  }
  if (arguments.preferredLocation == UsmMemAdvisePreferredLocation::System) {
    ASSERT_ZE_RESULT_SUCCESS(zeCommandListAppendMemAdvise(
        cmdList, levelzero.device, buffer, arguments.bufferSize,
  ...
  ASSERT_ZE_RESULT_SUCCESS(zeCommandListAppendMemoryFill(
      cmdList, buffer, &pattern, 1, arguments.bufferSize, nullptr, 0, nullptr));
