...
static TestResult run(const KernelWithWorkArgumentsSplit &arguments,
  ...
  ASSERT_CL_SUCCESS(clEnqueueNDRangeKernel(
      opencl.commandQueue, kernel, 1, nullptr, &gws, &lws, 0, nullptr,
      arguments.useEvents ? &profilingEvents[0] : nullptr));
  ...
  for (auto i = 0u; i < arguments.iterations; i++) {
    ...
    for (uint32_t splitId = 0u; splitId < arguments.splitSize; splitId++) {
      globalWorkOffset =
          splitId * arguments.workgroupSize * workgorupsInOneSplit;
      ASSERT_CL_SUCCESS(clEnqueueNDRangeKernel(
          opencl.commandQueue, kernel, 1, &globalWorkOffset, &gws, &lws, 0,
          nullptr, arguments.useEvents ? &profilingEvents[splitId] : nullptr));
    }
    ASSERT_CL_SUCCESS(clFinish(opencl.commandQueue));
    timer.measureEnd();
    if (arguments.useEvents) {
      ...
      cl_ulong start{};
      cl_ulong end{};
      ASSERT_CL_SUCCESS(clGetEventProfilingInfo(
          profilingEvents.front(), CL_PROFILING_COMMAND_START, sizeof(cl_ulong),
          &start, nullptr));
      ASSERT_CL_SUCCESS(clGetEventProfilingInfo(
          profilingEvents.back(), CL_PROFILING_COMMAND_END, sizeof(cl_ulong),
          &end, nullptr));
      for (uint32_t splitId = 0u; splitId < arguments.splitSize; splitId++) {
...
static RegisterTestCaseImplementation<KernelWithWorkSplit>
