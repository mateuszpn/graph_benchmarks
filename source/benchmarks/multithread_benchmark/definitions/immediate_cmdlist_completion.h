...
struct ImmediateCommandListCompletionArguments : TestCaseArgumentContainer {
  ...
  ImmediateCommandListCompletionArguments()
      : numberOfThreads(*this, "numberOfThreads", "total number of threads"),
        threadsPerEngine(
            *this, "threadsPerEngine",
            "number of threads submitting commands to each engine"),
...
struct ImmediateCommandListCompletion
    : TestCase<ImmediateCommandListCompletionArguments> {
  ...
  std::string getHelp() const override {
    return "measures completion latency of AppendMemoryCopy issued from "
           "multiple threads to Immediate Command Lists."
           "Engines to be used for submissions are selected based on the "
           "enabled bits of engineMask."
           "'threadsPerEngine' number of threads submits commands to each "
           "selected engine."
           "If 'numberOfThreads' is greater than 'threadsPerEngine' x selected "
           "engine count, then the excess threads are "
           "assigned to selected engines one each, in a round-robin method."
           "if selected engineCount == 1, then all threads are assigned to "
           "that engine.";
  }
};
