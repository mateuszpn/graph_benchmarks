...
static TestResult run(const AppendLaunchKernelArguments &arguments,
  ...
  // Create kernel
  auto spirvModule =
      FileHelper::loadBinaryFile("api_overhead_benchmark_empty_kernel.spv");
  ...
  const ze_event_pool_desc_t eventPoolDesc = {
      ZE_STRUCTURE_TYPE_EVENT_POOL_DESC, nullptr, 0, arguments.appendCount};
  ...
  if (arguments.useEvent) {
    ASSERT_ZE_RESULT_SUCCESS(zeEventPoolCreate(
        levelzero.context, &eventPoolDesc, 0, nullptr, &eventPool));
    for (auto j = 0u; j < arguments.appendCount; ++j) {
      eventDesc.index = j;
      ASSERT_ZE_RESULT_SUCCESS(
          zeEventCreate(eventPool, &eventDesc, &events[j]));
    }
  }

  // Create command list and warmup
  const ze_group_count_t dispatchTraits{
  ...
  for (auto j = 0u; j < arguments.appendCount; ++j) {
    ASSERT_ZE_RESULT_SUCCESS(zeCommandListAppendLaunchKernel(
        cmdList, kernel, &dispatchTraits, events[j], 0, nullptr));
  }
  ...
  for (auto i = 0u; i < arguments.iterations; ++i) {
    ...
    timer.measureEnd();
    statistics.pushValue(timer.get() / arguments.appendCount,
...
static RegisterTestCaseImplementation<AppendLaunchKernel>
