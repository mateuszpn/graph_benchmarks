...
static TestResult run(const UsmSharedMigrateGpuForFillArguments &arguments,
  ...
  auto clEnqueueMigrateMemINTEL =
      (pfn_clEnqueueMigrateMemINTEL)clGetExtensionFunctionAddressForPlatform(
          opencl.platform, "clEnqueueMigrateMemINTEL");
  ...
  // Create buffer
  auto buffer = static_cast<cl_int *>(
      clSharedMemAllocINTEL(opencl.context, opencl.device, nullptr,
                            arguments.bufferSize, 0u, &retVal));
  ...
  if (arguments.prefetchMemory) {
    ASSERT_CL_SUCCESS(clEnqueueMigrateMemINTEL(opencl.commandQueue, buffer,
                                               arguments.bufferSize, 0, 0,
                                               nullptr, nullptr));
  }

  ASSERT_CL_SUCCESS(clEnqueueMemFillINTEL(opencl.commandQueue, buffer, &pattern,
                                          1, arguments.bufferSize, 0, nullptr,
                                          nullptr));
  ...
  for (auto i = 0u; i < arguments.iterations; i++) {
    ...
    ASSERT_CL_SUCCESS(clEnqueueMemFillINTEL(opencl.commandQueue, buffer,
                                            &pattern, 1, arguments.bufferSize,
                                            0, nullptr, nullptr));
    ASSERT_CL_SUCCESS(clFinish(opencl.commandQueue));
...
static RegisterTestCaseImplementation<UsmSharedMigrateGpuForFill>
