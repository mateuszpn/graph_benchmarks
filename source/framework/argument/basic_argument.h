...
struct Unsigned32BitArgumentBase : Argument {
  ...
  uint32_t getSizeOf() const { return sizeof(value); }

  const uint32_t *getAddressOf() const { return &value; }
...
struct IntegerArgumentBase : Argument {
  ...
  int64_t getSizeOf() const { return sizeof(value); }

  const int64_t *getAddressOf() const { return &value; }
...
struct PositiveIntegerArgument : IntegerArgumentBase {
  ...
  bool validate() const override { return this->value > 0; }
};
...
struct ByteSizeArgument : PositiveIntegerArgument {
  ...
  void parseImpl(const std::string &valueToParse) override {
    ...
    const size_t unitMultipliers[] = {1024, 1024 * 1024, 1024 * 1024 * 1024, 1,
                                      1};
    const auto unitCount = sizeof(units) / sizeof(units[0]);
    static_assert(unitCount ==
                  sizeof(unitMultipliers) / sizeof(unitMultipliers[0]));
    ...
    for (; currentUnit < unitCount; currentUnit++) {
      ...
      const auto expectedUnitPosition =
          valueToParseLower.length() - units[currentUnit].length();
...
struct BooleanArgument : Argument {
  ...
  bool validate() const override { return value == 0 || value == 1; }
