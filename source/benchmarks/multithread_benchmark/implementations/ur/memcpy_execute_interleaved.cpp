...
static TestResult run(const MemcpyExecuteArguments &arguments,
  ...
  if (!useEvents && !inOrderQueue) {
    std::cerr << "In order queue must be used when events are not used"
              << std::endl;
  ...
  // Create kernel
  auto spirvModule =
      FileHelper::loadBinaryFile("memory_benchmark_fill_with_ones.spv");
  if (spirvModule.size() == 0)
    return TestResult::KernelNotFound;

  ur_program_handle_t program;
  EXPECT_UR_RESULT_SUCCESS(urProgramCreateWithIL(
      ur.context, spirvModule.data(), spirvModule.size(), nullptr, &program));
  ...
  if (!useQueuePerThread) {
    EXPECT_UR_RESULT_SUCCESS(
        urQueueCreate(ur.context, ur.device, &queueProperties, &singleQueue));
  ...
  for (size_t i = 0; i < numThreads; i++) {
    for (size_t j = 0; j < numOpsPerThread; j++) {
      ...
      EXPECT_UR_RESULT_SUCCESS(
          urKernelSetArgPointer(kernel, 0, nullptr, usm[i][j]));
  ...
  auto worker = [&](size_t thread_id, Timer &timer) {
    ...
    for (size_t i = 0; i < numOpsPerThread; i++) {
      ...
      ur_event_handle_t *memcpySignalEventPtr =
          useEvents ? &events[i][0] : nullptr;
      ur_event_handle_t *kernelSignalEventPtr =
          useEvents ? &events[i][1] : nullptr;
      ur_event_handle_t *finalSignalEventPtr =
          useEvents ? &events[i][2] : nullptr;

      EXPECT_UR_RESULT_SUCCESS(
          urEnqueueUSMMemcpy(queue, false, usm_ptr, src_buffer.data(),
                             allocSize, 0, nullptr, memcpySignalEventPtr));
      EXPECT_UR_RESULT_SUCCESS(urEnqueueKernelLaunch(
          queue, kernel, n_dimensions, &global_offset, &arraySize, nullptr,
          useEvents, memcpySignalEventPtr, kernelSignalEventPtr));
      EXPECT_UR_RESULT_SUCCESS(urEnqueueUSMMemcpy(
          queue, false, host_dst, usm_ptr, allocSize, useEvents,
          kernelSignalEventPtr, finalSignalEventPtr));
    }
  ...
  for (size_t i = 0u; i < arguments.iterations; i++) {
      ...
      for (size_t i = 0; i < numOpsPerThread; i++) {
        for (size_t j = 0; j < allocSize / sizeof(int); j++) {
          if (dst_buffers[t][i][j] != 1) {
            std::cerr << "dst_buffers at: " << t << " " << i << " " << j
                      << " , is: " << dst_buffers[t][i][j] << std::endl;
            return TestResult::Error;
          }
        }
      }
    ...
    statistics.pushValue(avgTime, typeSelector.getUnit(),
...
[[maybe_unused]] static RegisterTestCaseImplementation<MemcpyExecute>
