...
void enqueueSvmCopy(ze_command_queue_handle_t queue,
...
static TestResult run(const SvmCopyArguments &arguments,
  ...
  auto queueFamiliesDesc =
      QueueFamiliesHelper::queryQueueFamilies(levelzero.device);
  for (auto copyEngineIndex = 1u; copyEngineIndex <= 8; copyEngineIndex++) {
    auto queueDesc = QueueFamiliesHelper::getPropertiesForSelectingEngine(
        levelzero.device,
        EngineHelper::getBlitterEngineFromIndex(copyEngineIndex));
  ...
  for (auto i = 0u; i < arguments.numberOfThreads; i++) {
    ...
    ASSERT_ZE_RESULT_SUCCESS(UsmHelper::allocate(
        UsmMemoryPlacement::Host, levelzero, bufferForCopySize, &source));
    ASSERT_ZE_RESULT_SUCCESS(UsmHelper::allocate(UsmMemoryPlacement::Device,
                                                 levelzero, bufferForCopySize,
                                                 &destination));
  ...
  for (auto i = 0u; i < arguments.numberOfThreads; i++) {
    ...
    cmdListDesc.commandQueueGroupOrdinal =
        queueDescriptors[i % queueDescriptors.size()].ordinal;
    ze_command_list_handle_t cmdList{};
    ASSERT_ZE_RESULT_SUCCESS(zeCommandListCreate(
        levelzero.context, levelzero.device, &cmdListDesc, &cmdList));
    ASSERT_ZE_RESULT_SUCCESS(
        zeCommandListAppendMemoryCopy(cmdList, dstAllocs[i], srcAllocs[i],
                                      bufferForCopySize, nullptr, 0, nullptr));
  ...
  // Warmup
  {
    ...
    for (auto i = 0u; i < arguments.numberOfThreads; i++) {
      threads.push_back(std::unique_ptr<std::thread>(new std::thread(
          enqueueSvmCopy, queues[i % queues.size()], cmdLists[i], &barrier)));
    }
    lock.unlock();
    for (auto i = 0u; i < arguments.numberOfThreads; i++) {
      threads[i]->join();
  ...
  for (auto i = 0u; i < arguments.iterations; i++) {
    ...
    for (auto j = 0u; j < arguments.numberOfThreads; j++) {
      threads.push_back(std::unique_ptr<std::thread>(new std::thread(
          enqueueSvmCopy, queues[j % queues.size()], cmdLists[j], &barrier)));
    }
    timer.measureStart();
    lock.unlock();
    for (auto j = 0u; j < arguments.numberOfThreads; j++) {
  ...
  // Cleanup
  for (auto i = 0u; i < arguments.numberOfThreads; i++) {
    ASSERT_ZE_RESULT_SUCCESS(UsmHelper::deallocate(UsmMemoryPlacement::Host,
                                                   levelzero, srcAllocs[i]));
    ASSERT_ZE_RESULT_SUCCESS(UsmHelper::deallocate(UsmMemoryPlacement::Device,
                                                   levelzero, dstAllocs[i]));
  }
