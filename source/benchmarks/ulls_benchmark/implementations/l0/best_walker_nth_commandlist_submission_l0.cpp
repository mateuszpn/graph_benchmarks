...
static TestResult
run(const BestWalkerNthCommandListSubmissionArguments &arguments,
  ...
  ASSERT_ZE_RESULT_SUCCESS(zeCommandQueueExecuteCommandLists(
      levelzero.commandQueue, static_cast<uint32_t>(arguments.cmdListCount),
      cmdLists.data(), nullptr));
  ...
  for (uint32_t i = 0; i < arguments.cmdListCount; i++) {
    ...
    ASSERT_ZE_RESULT_SUCCESS(
        zeMemFree(levelzero.context, cmdListData[i].buffer));
...
static RegisterTestCaseImplementation<BestWalkerNthCommandListSubmission>
