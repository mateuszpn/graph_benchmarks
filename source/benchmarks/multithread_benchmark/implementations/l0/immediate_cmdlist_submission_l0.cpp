...
static void issueToImmediateCmdList(ThreadSpecificData *threadData,
  ...
  volatile uint64_t *volatileBuffer =
      static_cast<uint64_t *>(threadData->hostMemory);
  *volatileBuffer = 0;
  ...
  EXPECT_ZE_RESULT_SUCCESS(zeCommandListAppendLaunchKernel(
      threadData->cmdList, threadData->kernel, &groupCount, threadData->event,
      0, nullptr));
...
static TestResult
getComputeEngineInfo(std::vector<EngineInfo> &supportedEngineInfo,
                     LevelZero &levelzero) {
...
static TestResult run(const ImmediateCommandListSubmissionArguments &arguments,
  ...
  for (auto i = 0u; i < arguments.numberOfThreads; i++) {
    ...
    ASSERT_ZE_RESULT_SUCCESS(UsmHelper::allocate(UsmMemoryPlacement::Host,
                                                 levelzero, bufferSize,
                                                 &threadData[i].hostMemory));
    ASSERT_ZE_RESULT_SUCCESS(
        zeContextMakeMemoryResident(levelzero.context, levelzero.device,
                                    threadData[i].hostMemory, bufferSize));
    ...
    ASSERT_ZE_RESULT_SUCCESS(
        zeKernelCreate(module, &kernelDesc, &threadData[i].kernel));
    ASSERT_ZE_RESULT_SUCCESS(
        zeKernelSetGroupSize(threadData[i].kernel, 1, 1, 1));
    ASSERT_ZE_RESULT_SUCCESS(zeKernelSetArgumentValue(
        threadData[i].kernel, 0, sizeof(threadData[i].hostMemory),
        &threadData[i].hostMemory));
    eventDesc.index = i;
    ASSERT_ZE_RESULT_SUCCESS(
  ...
  for (auto i = 0u; i < arguments.numberOfThreads; i++) {
    ASSERT_ZE_RESULT_SUCCESS(
        zeContextEvictMemory(levelzero.context, levelzero.device,
                             threadData[i].hostMemory, bufferSize));
    ASSERT_ZE_RESULT_SUCCESS(zeKernelDestroy(threadData[i].kernel));
    ASSERT_ZE_RESULT_SUCCESS(
        zeMemFree(levelzero.context, threadData[i].hostMemory));
...
static RegisterTestCaseImplementation<ImmediateCommandListSubmission>
