  ...
  UsmImmediateCopyCopyMultipleBlitsArguments()
      : sourcePlacement(*this, "src", "Placement of the source buffer"),
        destinationPlacement(*this, "dst",
                             "Placement of the destination buffer"),
...
struct UsmImmediateCopyMultipleBlits
    : TestCase<UsmImmediateCopyCopyMultipleBlitsArguments> {
  ...
  std::string getHelp() const override {
    return "allocates two unified shared memory buffers, divides them into "
           "chunks, copies "
           "each chunk using a different copy engine with an immediate command "
           "list and "
           " measures bandwidth. Results for each "
           "individual blitter engine is measured using GPU-based timings and "
           "reported "
