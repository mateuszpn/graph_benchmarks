...
struct MathOperationHelper {
  static bool requiresIntelGlobalAtomicsExtension(MathOperation operation,
                                                  DataType type);
  static bool isSupportedAsAtomic(MathOperation operation, DataType type,
                                  bool globalAtomicFloatsSupported,
                                  bool usesSlm);
  ...
  static MathOperationTestData generateTestData(DataType dataType,
                                                MathOperation operation,
                                                size_t loopIterations,
                                                size_t operationsPerLoop,
                                                size_t totalThreadsCount = 1);
  template <typename DataTypeT>
  static MathOperationTestData
  generateTestData(MathOperation operation, size_t loopIterations,
                   size_t operationsPerLoop, size_t totalThreadsCount = 1);
};

template <typename DataTypeT>
inline MathOperationTestData MathOperationHelper::generateTestData(
    MathOperation operation, size_t loopIterations, size_t operationsPerLoop,
    size_t totalThreadsCount) {
  const size_t operationsCount =
      totalThreadsCount * loopIterations * operationsPerLoop;
  ...
  DataTypeT &otherArgument =
      reinterpret_cast<DataTypeT &>(result.otherArgument);
  DataTypeT &expectedValue =
      reinterpret_cast<DataTypeT &>(result.expectedValue);
  ...
  case MathOperation::Add:
    ...
    otherArgument = 3;
    expectedValue =
        initialValue + otherArgument * static_cast<DataTypeT>(operationsCount);
    break;
  ...
  default:
    if constexpr (std::is_integral_v<DataTypeT>) {
      ...
      case MathOperation::Xor:
        initialValue = 0b1111001111001101;
        otherArgument = 0b1101100100101010;
        expectedValue = (operationsCount % 2 == 0)
                            ? initialValue
                            : initialValue ^ otherArgument;
        break;
      default:
