...
class AllTestsGtestListener : public ::testing::EmptyTestEventListener {
  ...
  void OnTestProgramStart(
      [[maybe_unused]] const ::testing::UnitTest &unitTest) override {
    if (!Configuration::get().noHeaders &&
        Configuration::get().printType != Configuration::PrintType::Csv) {
      std::cout << "Running " << Configuration::get().iterations
  ...
  void
  OnTestStart([[maybe_unused]] const ::testing::TestInfo &testCase) override {
    currentTestCaseErrorInfo = {};
  }
  void
  OnTestPartResult(const ::testing::TestPartResult &testPartResult) override {
  ...
  void OnTestEnd(const ::testing::TestInfo &testCase) override {
    if (testCase.result()->Failed()) {
      currentTestCaseErrorInfo.name << testCase.test_case_name() << "."
                                    << testCase.name();
...
template <typename ListenerType> void replaceGtestListener() {
