...
static TestResult run(const KernelWithWorkArguments &arguments,
  ...
  const auto outputBufferSize =
      sizeof(uint32_t) * arguments.workgroupCount * arguments.workgroupSize;
  ASSERT_ZE_RESULT_SUCCESS(
  ...
  // Create kernel
  auto spirvModule =
      FileHelper::loadBinaryFile(selectKernel(arguments.usedIds));
  ...
  for (auto commandIndex = 0u; commandIndex < arguments.measuredCommands;
    ...
    const ze_group_count_t groupCount{
        static_cast<uint32_t>(arguments.workgroupCount), 1u, 1u};
...
static RegisterTestCaseImplementation<KernelWithWork> registerTestCase(run,
                                                                       Api::L0);
