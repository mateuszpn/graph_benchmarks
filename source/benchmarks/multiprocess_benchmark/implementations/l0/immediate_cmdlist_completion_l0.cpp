...
static TestResult getEngineInfo(
    std::vector<EngineInfo> &supportedEngineInfo, LevelZero &levelzero,
    std::string_view engineGroup,
    const MultiProcessImmediateCmdlistCompletionArguments &arguments) {
...
static TestResult
run(const MultiProcessImmediateCmdlistCompletionArguments &arguments,
  ...
  if (supportedEngineInfo.size() * arguments.processesPerEngine >
      arguments.numberOfProcesses) {
    return TestResult::DeviceNotCapable;
  }

  // Prepare processes
  ProcessGroup processes{"immediate_cmdlist_copy_workload_l0",
                         arguments.numberOfProcesses};
  ...
  for (auto i = 0u; i < processes.size(); i++) {
    ...
    processes[i].setName("p:" + std::to_string(i) +
                         "|o:" + std::to_string(ordinal) +
                         "|i:" + std::to_string(engineIndex));
  ...
  processes.pushMeasurementsToStatistics(
      arguments.iterations, statistics, MeasurementUnit::Microseconds,
      MeasurementType::Cpu, pushIndividualProcessesMeasurements, true);

  return TestResult::Success;
}

static RegisterTestCaseImplementation<MultiProcessImmediateCmdlistCompletion>
