...
static TestResult run(const MatrixMultiplyArguments &arguments,
  ...
  {
    sycl::buffer<int32_t, 1> dataXBuf(dataX.data(),
                                      sycl::range<1>{totalElements});
    sycl::buffer<int32_t, 1> dataYBuf(dataY.data(),
                                      sycl::range<1>{totalElements});
    sycl::buffer<int32_t, 1> resultsBuf(resultsFromRun.data(),
                                        sycl::range<1>{totalElements});
    ...
    auto commandList = [&](sycl::handler &cgh) {
      ...
      auto resultsDev =
          resultsBuf.get_access<sycl::access_mode::discard_write>(cgh);

      cgh.parallel_for<class MatrixMultiplyKernel>(
          sycl::range<1>{totalElements}, [=](sycl::item<1> item) {
            resultsDev[item.get_id()] =
                dataXDev[item.get_id()] + dataYDev[item.get_id()];
          });
    };
  ...
  if (!std::equal(std::begin(resultsFromRun), std::end(resultsFromRun),
...
[[maybe_unused]] static RegisterTestCaseImplementation<MatrixMultiply>
