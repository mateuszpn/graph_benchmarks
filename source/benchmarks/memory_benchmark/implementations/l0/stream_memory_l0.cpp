...
static TestResult run(const StreamMemoryArguments &arguments,
  ...
  case StreamMemoryType::Read:
    ...
    ASSERT_ZE_RESULT_SUCCESS(UsmHelper::allocate(
        arguments.memoryPlacement, levelzero, 16u, &buffers[buffersCount++]));
    break;
  ...
  const uint32_t groupSizeX =
      std::min(levelzero.getDeviceComputeProperties().maxGroupSizeX, gws);
  ...
  for (auto i = 0u; i < buffersCount; i++) {
    ASSERT_ZE_RESULT_SUCCESS(BufferContentsHelperL0::fillBuffer(
        levelzero, buffers[i], bufferSizes[i], arguments.contents, false));
...
static RegisterTestCaseImplementation<StreamMemory> registerTestCase(run,
                                                                     Api::L0);
