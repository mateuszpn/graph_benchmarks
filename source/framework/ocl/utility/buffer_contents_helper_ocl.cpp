...
cl_int BufferContentsHelperOcl::fillBuffer(cl_command_queue queue,
                                           cl_mem buffer, size_t bufferSize,
...
cl_int BufferContentsHelperOcl::fillUsmBufferOrHostPtr(
    cl_command_queue queue, void *ptr, size_t ptrSize,
    UsmMemoryPlacement placement, BufferContents contents) {
  switch (placement) {
...
cl_int BufferContentsHelperOcl::fillBufferWithRandomBytes(
    cl_command_queue queue, cl_mem buffer, size_t bufferSize) {
  auto cpuBuffer = std::make_unique<uint8_t[]>(bufferSize);
  BufferContentsHelper::fillWithRandomBytes(cpuBuffer.get(), bufferSize);
  CL_SUCCESS_OR_RETURN(clEnqueueWriteBuffer(queue, buffer, CL_BLOCKING, 0,
                                            bufferSize, cpuBuffer.get(), 0,
                                            nullptr, nullptr));
  return CL_SUCCESS;
}

cl_int BufferContentsHelperOcl::fillBufferWithZeros(cl_command_queue queue,
                                                    cl_mem buffer,
                                                    size_t bufferSize) {
  const cl_uint pattern[] = {0};
  CL_SUCCESS_OR_RETURN(clEnqueueFillBuffer(queue, buffer, pattern,
...
cl_int BufferContentsHelperOcl::fillBufferWithIncreasingBytes(
    cl_command_queue queue, cl_mem buffer, size_t bufferSize) {
  auto cpuBuffer = std::make_unique<uint8_t[]>(bufferSize);
...
cl_int BufferContentsHelperOcl::fillUsmBuffer(cl_command_queue queue,
                                              void *usmBuffer,
                                              size_t bufferSize,
                                              BufferContents contents) {
  switch (contents) {
...
cl_int BufferContentsHelperOcl::fillUsmBufferWithRandomBytes(
    cl_command_queue queue, void *usmBuffer, size_t bufferSize) {
  // Get API calls
  cl_device_id device = {};
  CL_SUCCESS_OR_RETURN(clGetCommandQueueInfo(queue, CL_QUEUE_DEVICE,
                                             sizeof(device), &device, nullptr));
  cl_context context = {};
  CL_SUCCESS_OR_RETURN(clGetCommandQueueInfo(
      queue, CL_QUEUE_CONTEXT, sizeof(context), &context, nullptr));
  cl_platform_id platform = {};
  CL_SUCCESS_OR_RETURN(clGetDeviceInfo(device, CL_DEVICE_PLATFORM,
  ...
  auto clEnqueueMemcpyINTEL =
      (pfn_clEnqueueMemcpyINTEL)clGetExtensionFunctionAddressForPlatform(
          platform, "clEnqueueMemcpyINTEL");

  // Create staging allocation
  cl_int retVal = {};
  void *stagingAlloc =
      clHostMemAllocINTEL(context, nullptr, bufferSize, 0, &retVal);
  CL_SUCCESS_OR_RETURN(retVal);
  fillWithRandomBytes(static_cast<uint8_t *>(stagingAlloc), bufferSize);

  // Copy to destination allocation
  CL_SUCCESS_OR_RETURN(clEnqueueMemcpyINTEL(queue, CL_NON_BLOCKING, usmBuffer,
                                            stagingAlloc, bufferSize, 0,
                                            nullptr, nullptr));
  CL_SUCCESS_OR_RETURN(clFinish(queue));
...
cl_int BufferContentsHelperOcl::fillUsmBufferWithZeros(cl_command_queue queue,
                                                       void *usmBuffer,
                                                       size_t bufferSize) {
  // Get API calls
  cl_device_id device = {};
  ...
  // Fill with zeros
  const cl_uint pattern[] = {0};
  CL_SUCCESS_OR_RETURN(clEnqueueMemFillINTEL(queue, usmBuffer, pattern,
                                             sizeof(pattern), bufferSize, 0,
                                             nullptr, nullptr));
  CL_SUCCESS_OR_RETURN(clFinish(queue));
