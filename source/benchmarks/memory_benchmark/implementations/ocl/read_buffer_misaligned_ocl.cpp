...
static TestResult run(const ReadBufferMisalignedArguments &arguments,
  ...
  // Create buffer
  const cl_mem buffer = clCreateBuffer(opencl.context, CL_MEM_READ_WRITE,
                                       arguments.size, nullptr, &retVal);
  ASSERT_CL_SUCCESS(retVal);
  ASSERT_CL_SUCCESS(BufferContentsHelperOcl::fillBuffer(
      opencl.commandQueue, buffer, arguments.size,
      BufferContents::IncreasingBytes));
  auto cpuBuffer = CpuAllocationHelper::allocateMisalignedAllocation(
      arguments.size, MemoryConstants::cachelineSize,
      arguments.misalignmentFromCacheline);
...
static RegisterTestCaseImplementation<ReadBufferMisaligned>
