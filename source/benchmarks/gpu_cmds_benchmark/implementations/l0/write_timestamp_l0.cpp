...
static TestResult run(const WriteTimestampArguments &arguments,
  ...
  for (auto commandIndex = 0u; commandIndex < arguments.measuredCommands;
       commandIndex++) {
    ASSERT_ZE_RESULT_SUCCESS(zeCommandListAppendWriteGlobalTimestamp(
        cmdList, addressForWrite, nullptr, 0, nullptr));
  }
  ASSERT_ZE_RESULT_SUCCESS(zeCommandListAppendWriteGlobalTimestamp(
...
static RegisterTestCaseImplementation<WriteTimestamp> registerTestCase(run,
                                                                       Api::L0);
