...
Type getInfo(Handle handle, Fn &&getInfoCb, Info info) {
  if constexpr (std::is_same_v<Type, std::string>) {
    size_t paramSize = 0;
    EXPECT_UR_RESULT_SUCCESS(getInfoCb(handle, info, 0, nullptr, &paramSize));

    std::string result(paramSize, '\0');
    EXPECT_UR_RESULT_SUCCESS(
        getInfoCb(handle, info, paramSize, result.data(), &paramSize));
    return result;
  } else {
    Type result;
    EXPECT_UR_RESULT_SUCCESS(
        getInfoCb(handle, info, sizeof(Type), &result, nullptr));
    return result;
  }
}
...
std::string getDriverVersion(ur_device_handle_t device) {
  return getInfo<std::string>(device, urDeviceGetInfo,
                              UR_DEVICE_INFO_DRIVER_VERSION);
}

uint32_t getDeviceClockRate(ur_device_handle_t device) {
  return getInfo<uint32_t>(device, urDeviceGetInfo,
...
std::string getPlatformVendorName(ur_platform_handle_t platform) {
  return getInfo<std::string>(platform, urPlatformGetInfo,
...
ur_adapter_backend_t getAdapterBackend(ur_adapter_handle_t adapter) {
  return getInfo<ur_adapter_backend_t>(adapter, urAdapterGetInfo,
...
void printDeviceInfo() {
  ...
  std::cout << "  Platform: " << getPlatformVendorName(ur.platform)
            << std::endl;
  ...
  std::cout << "\t\tvendorId:     0x" << std::hex
            << getDeviceVendorId(ur.device) << std::endl;
  std::cout << "\t\tdeviceId:     0x" << std::hex << getDeviceId(ur.device)
            << std::endl;
  std::cout << "\t\tclockFreq:    " << std::dec << getDeviceClockRate(ur.device)
            << std::endl;
  std::cout << "\t\tconfig:       " << getDeviceNumSlices(ur.device) << "x"
            << getDeviceNumSubslicesPerSlice(ur.device) << "x"
            << getDeviceNumEUPerSubslice(ur.device) << std::endl;
  std::cout << "\t\teuCount:      "
            << getDeviceNumSlices(ur.device) *
                   getDeviceNumSubslicesPerSlice(ur.device) *
                   getDeviceNumEUPerSubslice(ur.device)
            << std::endl;
  std::cout << "\t\tthreadsPerEu: " << getDeviceNumThreadPerEU(ur.device)
            << std::endl;
...
static void printAvailableDevices() {
  ...
  EXPECT_UR_RESULT_SUCCESS(
      urPlatformGet(&ur.adapter, 1, 0, nullptr, &platform_count));

  std::vector<ur_platform_handle_t> platforms(platform_count);
  EXPECT_UR_RESULT_SUCCESS(
      urPlatformGet(&ur.adapter, 1, platform_count, platforms.data(), nullptr));

  std::cout << "UR platforms: " << platform_count << std::endl;
  std::cout << "Adapter: " << getAdapterBackend(ur.adapter) << std::endl;

  for (uint32_t platform_index = 0; platform_index < platform_count;
       platform_index++) {
    ...
    EXPECT_UR_RESULT_SUCCESS(
        urDeviceGet(platform, UR_DEVICE_TYPE_ALL, 0, nullptr, &device_count));

    std::vector<ur_device_handle_t> devices(device_count);
    EXPECT_UR_RESULT_SUCCESS(urDeviceGet(
        platform, UR_DEVICE_TYPE_ALL, device_count, devices.data(), nullptr));
    ...
    for (uint32_t device_index = 0; device_index < device_count;
      ...
      std::cout << "      Type: " << getDeviceType(ur.device) << std::endl;
      std::cout << "      select this device with --urPlatformIndex="
                << platform_index << " --urDeviceIndex=" << device_index;
      std::cout << std::endl;
    }
