...
static TestResult run(const UsmSharedMigrateGpuArguments &arguments,
  ...
  ASSERT_ZE_RESULT_SUCCESS(UsmHelper::allocate(
      arguments.bufferPlacement, levelzero, arguments.bufferSize, &bufferVoid));
  int32_t *buffer = static_cast<int32_t *>(bufferVoid);
  const size_t elementsCount = arguments.bufferSize / sizeof(uint32_t);

  // Create kernel
  const auto kernelBinary = FileHelper::loadBinaryFile(
      "multitile_memory_benchmark_fill_with_ones.spv");
  ...
  ASSERT_ZE_RESULT_SUCCESS(zeModuleCreate(levelzero.context,
                                          levelzero.getDevice(queuePlacement),
  ...
  ASSERT_ZE_RESULT_SUCCESS(zeCommandListCreate(
      levelzero.context, levelzero.getDevice(queuePlacement), &cmdListDesc,
      &cmdList));
