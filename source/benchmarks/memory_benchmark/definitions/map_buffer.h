...
struct MapBuffer : TestCase<MapBufferArguments> {
  ...
  std::string getTestCaseName() const override { return "MapBuffer"; }

  std::string getHelp() const override {
    return "allocates an OpenCL buffer and measures map bandwidth. Mapping "
           "operation means "
           "memory transfer from GPU to CPU or a no-op, depending on map "
           "flags.";
  }
};
