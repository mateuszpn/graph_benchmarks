...
// Class handles regular OpenCL boilerplate code, such as querying driver
...
struct Opencl {
  ...
  Opencl(const QueueProperties &queueProperties)
      : Opencl(queueProperties, ContextProperties::create()) {}
  Opencl(const QueueProperties &queueProperties,
         const ContextProperties &contextProperties);
  ~Opencl() noexcept(false);

  // Creating base OpenCL queue/context. Objects are saved inside the Opencl
  // object and released in its destructor
  ...
  // Getter methods for cl_device objects store inside the Opencl object. Single
  // device version will throw, if multiple devices are requested.
  cl_device_id getDevice(DeviceSelection deviceSelection);
  std::vector<cl_device_id> getDevices(DeviceSelection deviceSelection,
                                       bool requireSuccess);

  // Get helper used to query if certain extensions are supported by the OpenCL
  // implementation
