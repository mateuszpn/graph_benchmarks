...
inline void printMessageLine(const char *label, Args &&...args) {
  static_assert(sizeof...(args) > 0,
                "A textual message of the FATAL_ERROR is required");
  printToOstream(std::cerr, label, ": ", std::forward<Args>(args)...);
  std::cerr << std::endl;
}

inline std::string composeErrorStringForMacro(const char *file, int line,
                                              const char *macroName,
                                              const char *macroArg,
                                              const char *macroArgValue,
                                              const char *macroArgValueEnum) {
...
#define NON_FATAL_ERROR(macroName, macroArg, macroArgValue, macroArgValueEnum) \
  GTEST_NONFATAL_FAILURE_(                                                     \
      composeErrorStringForMacro(__FILE__, __LINE__, macroName, macroArg,      \
                                 macroArgValue, macroArgValueEnum)             \
          .c_str());
...
#define FATAL_ERROR_UNLESS(condition, message)                                 \
