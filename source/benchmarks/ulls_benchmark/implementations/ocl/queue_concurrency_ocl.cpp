...
static TestResult run(const QueueConcurrencyArguments &arguments,
  ...
  ASSERT_CL_SUCCESS(
      clSetKernelArg(slowKernel, 0, sizeof(kernelTime), &kernelTime));
  cl_uint fastTime = 1u;
  ...
  // Warmup, kernel
  ASSERT_CL_SUCCESS(clEnqueueNDRangeKernel(opencl.commandQueue, slowKernel, 1,
                                           nullptr, &gws, &lws, 0, nullptr,
                                           &events[0]));
  ASSERT_CL_SUCCESS(clEnqueueNDRangeKernel(opencl.commandQueue, fastKernel, 1,
                                           nullptr, &gws, &lws, 0, nullptr,
                                           &events[1]));
  ...
  for (auto i = 0u; i < arguments.iterations; i++) {
    ...
    for (auto j = 2u; j < arguments.kernelCount * 2; j += 2) {
      ...
      ASSERT_CL_SUCCESS(clEnqueueNDRangeKernel(opencl.commandQueue, slowKernel,
                                               1, nullptr, &gws, &lws, 1,
                                               &events[j - 1], &events[j]));
      ASSERT_CL_SUCCESS(clEnqueueNDRangeKernel(opencl.commandQueue, fastKernel,
                                               1, nullptr, &gws, &lws, 0,
                                               nullptr, &events[j + 1]));
    }
    ASSERT_CL_SUCCESS(clFinish(opencl.commandQueue));
...
static RegisterTestCaseImplementation<QueueConcurrency>
