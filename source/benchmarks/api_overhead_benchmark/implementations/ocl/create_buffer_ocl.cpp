...
static TestResult run(const CreateBufferArguments &arguments,
  ...
  if (arguments.copyHostPtr) {
    ...
    for (auto i = 0u; i < arguments.iterations; ++i) {
      srcCpuBuffers[i] =
          std::make_unique<int[]>(arguments.bufferSize / sizeof(int));
    }
  ...
  // Warmup
  {
    cl_mem buffer = clCreateBuffer(opencl.context, memFlags,
                                   arguments.bufferSize, hostPtr, &retVal);
...
static RegisterTestCaseImplementation<CreateBuffer>
