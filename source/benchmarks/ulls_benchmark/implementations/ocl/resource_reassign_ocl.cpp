...
static TestResult run(const ResourceReassignArguments &arguments,
  ...
  if (QueueFamiliesHelper::getQueueCountForEngineGroup(
          opencl.device, EngineGroup::Compute) < (arguments.queueCount + 1)) {
    return TestResult::DeviceNotCapable;
  }

  std::array<cl_command_queue, 4> queues{};
  for (size_t i = 0; i < arguments.queueCount + 1; i++) {
    queues[i] = opencl.createQueue(QueueProperties::create().setForceEngine(
        static_cast<Engine>(static_cast<uint32_t>(Engine::Ccs0) + i)));
  }
  ...
  // Warmup
  ASSERT_CL_SUCCESS(clEnqueueNDRangeKernel(queues[0], stress, 1, nullptr, &gws,
  ...
  for (size_t i = 1; i < arguments.queueCount + 1; i++) {
    ASSERT_CL_SUCCESS(clEnqueueNDRangeKernel(
        queues[i], stress, 1, nullptr, &gws, nullptr, 0, nullptr, nullptr));
  ...
  for (auto j = 0u; j < arguments.iterations; j++) {
    ...
    ASSERT_CL_SUCCESS(clEnqueueNDRangeKernel(
        queues[0], stress, 1, nullptr, &gws, nullptr, 0, nullptr, nullptr));
    ASSERT_CL_SUCCESS(clFinish(queues[0]));
...
static RegisterTestCaseImplementation<ResourceReassign>
