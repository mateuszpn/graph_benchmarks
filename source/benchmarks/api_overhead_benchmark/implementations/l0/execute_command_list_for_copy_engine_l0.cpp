...
static TestResult run(const ExecuteCommandListArguments &arguments,
  ...
  if (arguments.useFence) {
    ...
    ASSERT_ZE_RESULT_SUCCESS(
        zeFenceCreate(levelzero.commandQueue, &fenceDesc, &fence));
  }
  ...
  ASSERT_ZE_RESULT_SUCCESS(UsmHelper::allocate(UsmMemoryPlacement::Host,
                                               levelzero, bufferSize, &source));
  ASSERT_ZE_RESULT_SUCCESS(zeContextMakeMemoryResident(
      levelzero.context, levelzero.device, source, bufferSize));
  memset(static_cast<uint8_t *>(source), 0x7c, bufferSize);
  ASSERT_ZE_RESULT_SUCCESS(UsmHelper::allocate(
      UsmMemoryPlacement::Device, levelzero, bufferSize, &destination));
  ASSERT_ZE_RESULT_SUCCESS(zeContextMakeMemoryResident(
      levelzero.context, levelzero.device, destination, bufferSize));
  ...
  ASSERT_ZE_RESULT_SUCCESS(zeCommandListAppendMemoryCopy(
      cmdList, destination, source, bufferSize, nullptr, 0, nullptr));
  ASSERT_ZE_RESULT_SUCCESS(zeCommandListClose(cmdList));

  // Warmup
  ASSERT_ZE_RESULT_SUCCESS(zeCommandQueueExecuteCommandLists(
      levelzero.commandQueue, 1, &cmdList, fence));
  if (arguments.useFence) {
    ASSERT_ZE_RESULT_SUCCESS(
...
static RegisterTestCaseImplementation<ExecuteCommandListForCopyEngine>
