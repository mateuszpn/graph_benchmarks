...
static TestResult run(const ReadDeviceMemBufferArguments &arguments,
  ...
  QueueProperties queueProperties =
      QueueProperties::create().setProfiling(true);
  ...
  const size_t singleSendSizeInBytes =
      subgroupSize * vectorSize * sizeof(float);
  ...
  ASSERT_CL_SUCCESS(clGetDeviceInfo(opencl.device, CL_DEVICE_MAX_COMPUTE_UNITS,
                                    sizeof(euNum), &euNum, nullptr));

  const bool useLargeGRF = gpuGen > IntelGen::Gen12lp ? true : false;
  const std::string largeGrfOpt =
      useLargeGRF ? " -cl-intel-256-GRF-per-thread " : " ";
  const std::string buildOptions =
      std::string("-cl-std=CL2.0 -cl-mad-enable -cl-fast-relaxed-math ") +
      ...
      " -D VECTOR_SIZE=" + std::to_string(vectorSize) + largeGrfOpt +
      std::string(" ");

  // Create buffer
  const cl_mem_flags compressionHint = CompressionHelper::getCompressionFlags(
      arguments.compressed, arguments.noIntelExtensions);
  ...
  const cl_mem source =
      clCreateBuffer(opencl.context, memFlags | CL_MEM_COPY_HOST_PTR,
                     arguments.size, srcCpuBuffer.get(), &retVal);

  const cl_mem destination = clCreateBuffer(opencl.context, memFlags,
                                            arguments.size, nullptr, &retVal);
  ASSERT_CL_SUCCESS(retVal);

  const uint32_t clearGpuBuffSize = 64 * megaByte;
  const cl_mem clearGpuBuff = clCreateBuffer(
      opencl.context, memFlags, clearGpuBuffSize, nullptr, &retVal);
  ASSERT_CL_SUCCESS(retVal);

  // Check buffers compression
  auto compressionStatus = CompressionHelper::verifyCompression(
      source, arguments.compressed, arguments.noIntelExtensions);
  if (compressionStatus != TestResult::Success) {
    ASSERT_CL_SUCCESS(clReleaseMemObject(source));
    return compressionStatus;
  }
  compressionStatus = CompressionHelper::verifyCompression(
      destination, arguments.compressed, arguments.noIntelExtensions);
  if (compressionStatus != TestResult::Success) {
    ASSERT_CL_SUCCESS(clReleaseMemObject(source));
  ...
  const char *programSrc =
      "__kernel void ClearCaches(__global unsigned int *pBuffer, unsigned int "
      "buffSize) {"
      ...
      "\n__kernel void ReadOnly(__global float4 *const pSrcBuffer, __global "
      "float4 *pDstBuffer,"
      "        unsigned int sliceMask, unsigned int sliceSize, unsigned int "
      "slotMask) {"
      "   const uint gid = get_global_id(0);"
      "   const uint sid = ((get_group_id(0) * get_num_sub_groups()) + "
      "get_sub_group_id()) % slotMask;"
      "   uint startOffset = 0;"
      ...
      "       startOffset = (((THREAD_TILE_SIZE * sid) + slotOffset) / "
      "SEND_SIZE);"
      "       __attribute__((opencl_unroll_hint(NUM_SENDS)))"
      "       do {"
      "           _input_data = "
      "as_float4(intel_sub_group_block_read4((__global const uint *)((__global "
      "uint *const)(&pSrcBuffer[startOffset]))));"
  ...
  cl_program program{};
  if (TestResult result = ProgramHelperOcl::buildProgramFromSource(
          opencl.context, opencl.device, programSrc, programSrcLen,
          buildOptions.c_str(), program);
  ...
  ASSERT_CL_SUCCESS(
      clSetKernelArg(clearCacheKernel, 0, sizeof(clearGpuBuff), &clearGpuBuff));
  ASSERT_CL_SUCCESS(clSetKernelArg(clearCacheKernel, 1, sizeof(buffSizeInInts),
                                   &buffSizeInInts));
  ASSERT_CL_SUCCESS(
      clEnqueueNDRangeKernel(opencl.commandQueue, clearCacheKernel, 1, nullptr,
                             &clearGws, NULL, 0, nullptr, nullptr));
  ASSERT_CL_SUCCESS(clFinish(opencl.commandQueue));
  ASSERT_CL_SUCCESS(retVal);

  cl_uint sliceSize = 0, sliceMask = 1, slotMask = 1;
  const cl_uint numThreadsPerEu = (gpuGen >= IntelGen::XeHpCore) ? 8 : 7;
  const cl_uint numHwThreads =
      (useLargeGRF ? numThreadsPerEu / 2 : 7) * static_cast<cl_uint>(euNum);
  ...
  if ((2 * numHwThreads * threadTileSizeInSubgroup) <
      static_cast<cl_uint>(arguments.size)) {
    ...
    sliceMask /= 2;
    sliceMask -= 1;
  } else if (arguments.size > 256 * kiloByte &&
             arguments.size <= 32 * megaByte) {
    // hit cache
  ...
  ASSERT_CL_SUCCESS(
      clSetKernelArg(kernel, 1, sizeof(destination), &destination));
  ...
  for (auto i = 0u; i < arguments.iterations; i++) {
    ...
    ASSERT_CL_SUCCESS(
        clEnqueueNDRangeKernel(opencl.commandQueue, clearCacheKernel, 1,
                               nullptr, &clearGws, NULL, 0, nullptr, nullptr));
    ASSERT_CL_SUCCESS(clFinish(opencl.commandQueue));

    ASSERT_CL_SUCCESS(clEnqueueNDRangeKernel(
        opencl.commandQueue, kernel, 1, nullptr, &gws, &lws, 0, nullptr, &evt));
    ASSERT_CL_SUCCESS(clWaitForEvents(1, &evt));

    cl_ulong timeNs{};
    ASSERT_CL_SUCCESS(
        ProfilingHelper::getEventDurationInNanoseconds(evt, timeNs));
    const size_t totalAccessedMemory =
        (numHwThreads * threadTileSizeInSubgroup * numOfLoops);
    statistics.pushValue(std::chrono::nanoseconds(timeNs), totalAccessedMemory,
...
static RegisterTestCaseImplementation<ReadDeviceMemBuffer>
