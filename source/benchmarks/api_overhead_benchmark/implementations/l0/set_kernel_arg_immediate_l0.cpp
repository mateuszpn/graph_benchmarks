...
static TestResult run(const KernelSetArgumentValueImmediateArguments &arguments,
  ...
  auto spirvModule = FileHelper::loadBinaryFile(
      std::string("api_overhead_benchmark_") +
      std::to_string(arguments.argumentSize) + "bytes_argument.spv");
  ...
  case 8:
    ASSERT_ZE_RESULT_SUCCESS(zeKernelSetArgumentValue(
        kernel, 0, sizeof(st_input_8), &kernelArgument8));
    break;
  case 64:
    ASSERT_ZE_RESULT_SUCCESS(zeKernelSetArgumentValue(
        kernel, 0, sizeof(st_input_64), &kernelArgument64));
    break;
  case 256:
    ASSERT_ZE_RESULT_SUCCESS(zeKernelSetArgumentValue(
        kernel, 0, sizeof(st_input_256), &kernelArgument256));
    break;
  case 512:
    ASSERT_ZE_RESULT_SUCCESS(zeKernelSetArgumentValue(
        kernel, 0, sizeof(st_input_512), &kernelArgument512));
    break;
  case 1024:
    ASSERT_ZE_RESULT_SUCCESS(zeKernelSetArgumentValue(
        kernel, 0, sizeof(st_input_1024), &kernelArgument1024));
    break;
  case 2048:
    ASSERT_ZE_RESULT_SUCCESS(zeKernelSetArgumentValue(
        kernel, 0, sizeof(st_input_2048), &kernelArgument2048));
    break;
  default:
...
static RegisterTestCaseImplementation<KernelSetArgumentValueImmediate>
