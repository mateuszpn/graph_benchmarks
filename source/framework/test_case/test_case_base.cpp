...
bool TestCaseBase::parseArguments(TestCaseArgumentContainer &arguments,
...
std::string TestCaseBase::getTestCaseNameWithConfig(
    const TestCaseArgumentContainer &arguments, bool commandLine) const {
  std::ostringstream result{};

  if (commandLine) {
    result << "--test=" << getTestCaseName()
           << " --api=" << std::to_string(arguments.api);
  } else {
...
bool TestCaseBase::matchesWithArgFilter(
  ...
  for (const std::string &argFilter : Configuration::get().argFilter.get()) {
    ...
    const auto matches = [&filter = filter](Argument *arg) {
      return (arg->toString() == filter);
    };
    const bool requirementMet =
        isFilterNegated ? std::none_of(args.begin(), args.end(), matches)
...
void TestCaseBase::printTestMapWarning() const {
  ...
  std::cerr << "WARNING: \"" << getTestCaseName()
            << "\" is not added to the test map. This is an issue in the "
               "benchmark causing single-test mode to not work.\n";
}

void TestCaseBase::printTestCaseNameLengthWarning(
    const std::string &testCaseNameWithConfig) const {
  const static size_t columnWidth =
      BenchmarkInfo::get().getTestCaseNameColumnWidth();
  ...
  if (currentWidth > maxWidth && Configuration::get().verbose) {
    ...
    std::cerr << "WARNING: current TestCase column width of " << columnWidth
              << " is too small. Consider changing it to " << maxWidth << ". "
              << "This is an issue in the benchmark which may cause the output "
                 "to appear weird, but does not break any functionality.\n";
  }
}
