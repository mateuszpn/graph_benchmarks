...
void printDeviceInfo() {
  ...
  ZE_RESULT_SUCCESS_OR_ERROR(
      zeDriverGetProperties(levelzero.driver, &driverProperties));
  // Version recorded by L0 GPU driver in following format:
  // 31 - 24: Major
  // 23 - 16: Minor
  // 15 - 0: Build
  std::string driverVersion =
      std::to_string((driverProperties.driverVersion & 0xFF000000) >> 24) +
      "." +
  ...
  std::cout << "\t\tvendorId:     0x" << std::hex << deviceProperties.vendorId
            << std::endl;
  std::cout << "\t\tdeviceId:     0x" << std::hex << deviceProperties.deviceId
            << " (intelProduct=" << std::to_string(intelProduct)
            << ", intelGen=" << std::to_string(intelGen) << ")\n";
  std::cout << "\t\tclockFreq:    " << std::dec
            << deviceProperties.coreClockRate << std::endl;
  std::cout << "\t\tconfig:       " << deviceProperties.numSlices << "x"
            << deviceProperties.numSubslicesPerSlice << "x"
            << deviceProperties.numEUsPerSubslice << std::endl;
  std::cout << "\t\teuCount:      "
            << deviceProperties.numSlices *
                   deviceProperties.numSubslicesPerSlice *
                   deviceProperties.numEUsPerSubslice
            << std::endl;
  std::cout << "\t\tthreadsPerEu: " << deviceProperties.numThreadsPerEU
            << std::endl;
...
static void printAvailableDevices() {
  ...
  for (uint32_t driverIndex = 0; driverIndex < driverCount; driverIndex++) {
    ...
    ZE_RESULT_SUCCESS_OR_ERROR(
        zeDriverGetProperties(driver, &driverProperties));
    std::cout << "  Driver " << driverIndex << ": version 0x" << std::hex
              << driverProperties.driverVersion << std::dec << " ";
    ...
    ZE_RESULT_SUCCESS_OR_ERROR(
        zeDeviceGet(driver, &deviceCount, devices.get()));
    for (uint32_t deviceIndex = 0; deviceIndex < deviceCount; deviceIndex++) {
      ...
      std::cout << ", select this device with --l0DriverIndex=" << driverIndex
                << " --l0DeviceIndex=" << deviceIndex;
      std::cout << std::endl;
    }
  }
